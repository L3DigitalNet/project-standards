...
A raw regex match is not yet a spec-local ID — skip it when any of four conditions holds: its prefix is standards/acronym noise that matches the shape but is not an ID (`HTTP-…`, `AES-…`, `ISO-…`, `SHA-…`, `RPO`, `RTO`, …), carried in the spec registry's `NOT_AN_ID` denylist; its prefix is the built-in `ADR` reference prefix (ADR ids are minted by the sibling [ADR Standard](../../../../adr/README.md), not here); its prefix is listed in the selected package's `reference_prefixes` option (external namespaces the spec only cites, e.g. a backlog `RQ-123` or a zero-version license family like `MIT`); or its digits are immediately followed by `.`+digit — a version/SPDX shape such as `MPL-2.0` (a real ID at a sentence end, `FR-007.`, is `.`+space and never matches). Only the remaining tokens are spec-local IDs.
...
5. Shared **boilerplate is identical**. Project Specification 1.10 lint checks exact Lifecycle and Quality prose plus the selected profile's structurally isolated Appendix A, B, and D blocks as `SL-BOILERPLATE`.
6. Every `FR-`, `NFR-`, `IR-`, and `DR-` Requirement cell begins with `The system shall`. Project Specification 1.10 lint reports a divergent row as `SL-REQUIREMENT-PHRASING` with the requirement ID and physical line.
7. A given logical **table has the same column count** across tiers (FR = 5 cols, NG = 3, WH = 4, Boundaries = 2, OQ = 7, DEV = 5, Revision History = 4, Appendix A = 3).
...
project-standards spec lint [--strict] [--json] [FILE ...]
```
...
For selected 1.10, ordinary lint returns `0` with warnings and strict lint returns `1` when any warning remains. `SL-BOILERPLATE` names a divergent Lifecycle, Quality, or Appendix A/B/D surface; restore it exactly from the selected profile template. `SL-REQUIREMENT-PHRASING` names and locates a mandatory row whose Requirement cell must begin with `The system shall`. Clean human output names `shared-boilerplate` and `mandatory-phrasing`; JSON adds the same fixed names in `checks`. Semantic review still owns requirement meaning, rationale, acceptance criteria, priority, and cross-document coherence.
