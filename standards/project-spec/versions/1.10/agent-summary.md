# Project Specification 1.10 summary

The canonical [README](README.md) is authoritative and wins if this summary conflicts with it.

- Package version `1.10`; independent document `contract_version` `1.1`; consumer availability.
- Enable with `project-standards standards enable project-spec --version 1.10`, review with `project-standards reconcile`, and apply with `project-standards reconcile --apply`.
...
- `spec import SOURCE --output TARGET --id SPEC-XXXX` is a read-only preservation preview. Source and target must be explicit, contained, and distinct; the source is never changed. Exact canonical headings map, while preamble, duplicate, near, and unmapped blocks remain byte-exact in adaptive-fenced review content with owner-decision diagnostics.
- Import apply additionally requires `--apply --expected-plan-digest DIGEST`. It regenerates one current typed plan, refuses a mismatch before the executor, and passes that same in-memory plan once on a match. No persisted plan handle exists. Human and `--json` output report mappings, review, diagnostics, digest, and `written`; import exits `0` for preview/apply and `2` for refusals.
...
- 1.10's `templates/` are byte-identical to this repository's canonical template source, so the selected registry and every unselected `spec new` / `spec upgrade` agree on the same Appendix A/B/D text. The 1.9 payload's Full template did not.
