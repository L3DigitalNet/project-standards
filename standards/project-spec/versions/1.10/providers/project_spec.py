    ...
    *,
    ...
    for raw in _documents(request):
        try:
            ...
            continue
        raw_findings = (
            lint_document(document, registry, conformance=True)
            if lint
            else validate_document(document, registry)
        )
...
def run_lint(request: Mapping[str, object], resources: Mapping[str, bytes]) -> dict[str, object]:
    ...
    return {
        **_run_findings(request, resources, lint=True),
        "checks": ["shared-boilerplate", "mandatory-phrasing"],
    }
...
# nothing at all, so a 1.7 consumer that sets no labels renders byte-identical
# bytes. The label alphabet enforced above is what makes the single-quoted YAML
# scalar exact — no label can carry a quote or backslash that would need escaping.
...
def run_render_workflow(
    ...
    return {
        "content": (
            ...
            '    with:\n      standards-ref: "v5"\n      strict-lint: true\n'
            f"{_runner_labels_entry(_config(request))}"
        )
    }
...
def run_migrate(
    ...
    if isinstance(raw_signatures, Mapping):
        ...
        for signature_id, target, ownership, disposition in (
            ...
            relinquished = config.get("workflow_ownership") == "consumer-owned"
                ...
                claims.append(
                    {
                        ...
                        "ownership": "consumer-owned" if relinquished else ownership,
                        "disposition": "preserve" if relinquished else disposition,
                    }
                )
            elif relinquished and isinstance(digest, str):
                claims.append(
                    {
                        "signature_id": signature_id,
                        "target": target,
                        ...
                        "intent_pointer": "/spec/workflow_ownership",
                    }
                )
    return {
        "schema_version": "1.0",
...
def run_import(request: Mapping[str, object], resources: Mapping[str, bytes]) -> dict[str, object]:
    ...
    # predecessor surface above must remain exact while 1.10 is still mutable.
