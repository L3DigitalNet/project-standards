...
5. Shared **boilerplate is identical**. Project Specification 1.11 lint checks exact Lifecycle and Quality prose plus the selected profile's structurally isolated Appendix A, B, and D blocks as `SL-BOILERPLATE`.
6. Every `FR-`, `NFR-`, `IR-`, and `DR-` Requirement cell begins with `The system shall`. Project Specification 1.11 lint reports a divergent row as `SL-REQUIREMENT-PHRASING` with the requirement ID and physical line.
