# Adopt Project Specification 1.11
...
project-standards standards enable project-spec --version 1.11
...
- `contract_version` selects the document contract independently of package version `1.11`.
...
Package 1.11 runs two profile-aware lint checks:

- `SL-BOILERPLATE` identifies Lifecycle, Quality, or the selected Light, Standard, or Full template's complete Appendix A, B, or D block. Restore the named surface exactly from that profile's immutable 1.11 template.
...
| `SL-BOILERPLATE` | Restore the named Lifecycle, Quality, or Appendix A, B, or D surface exactly from the selected 1.11 profile template. |
