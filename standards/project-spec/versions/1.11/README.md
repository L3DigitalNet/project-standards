...
- [Project Specification Standard](#project-specification-standard)
  ...
  - [4. Templates](#4-templates)
    - [What 1.10 changed](#what-110-changed)
    - [What 1.11 changed](#what-111-changed)
  - [5. Tooling](#5-tooling)
    - [Purpose](#purpose)
...
The three shipped templates are the only bytes that move, and they move back into agreement with this repository's canonical template source. Two corpora exist and must not drift apart: `src/project_standards/specs/templates/` backs `Registry.canonical_surfaces` and every unselected `spec new` and `spec upgrade`, while the selected V5 path builds its registry from the payload's own `template-*` resources. Between the 1.9 cut and this one the source templates gained guarded `project-standards spec upgrade` guidance and the 1.9 payload copies did not, so all three diverged — and in `spec-full-template.md` the divergence landed inside Appendix D, one of the blocks `spec lint` checks as `SL-BOILERPLATE`. A specification scaffolded under one corpus and linted under the other therefore reported an Appendix D finding whose canonical wording was absent from the payload its author was given (project-standards issue #199).

Resyncing alone was not enough. The source's own Appendix D closed on a code span holding nothing but an angle group, and `spec lint` treats such a span as a field to fill — inside a checked surface that is unwinnable, because filling it breaks the surface and leaving it reports an unfilled placeholder. This cut reworded that clause to name the target profile in prose, in the canonical source and in the payload together, so a Full specification filled in from the shipped template is clean under both rules.

1.10's `templates/` is byte-identical to that source, and `tests/package_contract/test_project_spec_1_10.py` pins the equality for whichever payload the catalog marks `default`, so a later source edit cannot silently reopen the gap. No option, provider, schema, artifact, or lint rule changes; everything else in the package is 1.9's.
...
- **`lint`** _(core)_ — advisory authoring quality on top of a valid spec: `<angle-bracket>` placeholders, un-deleted template guidance, status-aware traceability, and exact conformance to the selected profile's shared boilerplate and mandatory requirement phrasing. Ordinary lint reports warnings and exits `0`; `--strict` exits `1` when any warning remains. → **G2, G4, G5, G7**
...
Project Specification 1.11 reports `SL-BOILERPLATE` when Lifecycle, Quality, or the selected profile's Appendix A, B, or D block differs from its immutable template. Restore that complete canonical surface. It reports `SL-REQUIREMENT-PHRASING` when an `FR-`, `NFR-`, `IR-`, or `DR-` Requirement cell does not begin with the exact words `The system shall`; rewrite the statement without changing its intent, acceptance criteria, or priority. A clean human lint names `shared-boilerplate` and `mandatory-phrasing`. JSON adds the same fixed names in a per-document `checks` array, so exact-key consumers must accept this additive field when selecting 1.11.
