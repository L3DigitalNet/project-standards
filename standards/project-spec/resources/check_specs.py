...
NOT_AN_ID = {"HTTP", "AES", "SHA", "UTF", "ISO", "IEEE", "IEC", "WCAG", "RPO", "RTO",
...
def fm_keys(fm: str) -> list[str]:
    return [m.group(1) for line in fm.splitlines()
...
def check_file(name: str, path: pathlib.Path, canonical: set[str],
               fm_reference: list[str], defined_in_acc: dict[str, dict[str, str]]
    ...
    if not prof or prof.group(1) != name:
        problems.append(f"[frontmatter] profile is '{prof.group(1) if prof else None}', expected '{name}'")
    sid = re.search(r"^spec_id:\s*(\S+)", fm, re.M)
    if not sid or sid.group(1) != "SPEC-____":
        problems.append(f"[frontmatter] spec_id is '{sid.group(1) if sid else None}', expected sentinel 'SPEC-____'")
    ...
    omit_lines = [l for l in body.splitlines()
                  if l.lstrip().startswith(">") and "tier" in l and "omitted" in l]
    covered: set[int] = set()
    for l in omit_lines:
        ranges: list[tuple[str, str]] = re.findall(r"§(\d+)\s*[–-]\s*§?(\d+)", l)  # §3–§6
        for a, b in ranges:
    ...
    for pfx, definedin in declared.items():
        ...
        if mm and mm.group(1) not in canonical and mm.group(1).split(".")[0] not in canonical:
            problems.append(f"[id-registry] Appendix A '{pfx}-' Defined In '{definedin}' not in registry")
        ...
        if lines[i].strip().startswith("|") and i + 1 < len(lines) \
            ...
            while j < len(lines) and lines[j].strip().startswith("|"):
                if j != i + 1 and lines[j].count("|") != cols:
                    problems.append(f"[table] L{j + 1}: {lines[j].count('|')} pipes vs header {cols}")
                j += 1
            i = j
        else:
            i += 1
...
def main() -> int:
    ...
    for pfx, per_file in sorted(defined_in_acc.items()):
        if len({v for v in per_file.values()}) > 1:
            xfile.append(f"[cross-file] '{pfx}-' Defined In differs: "
    ...
    print("\n" + ("PASS — templates are consistent and tooling-interchangeable"
                  if total == 0 else f"FAIL — {total} problem(s)"))
