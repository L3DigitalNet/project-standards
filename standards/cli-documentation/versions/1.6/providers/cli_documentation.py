...
def _workflow(config: Mapping[str, object]) -> str:
    ...
    if language == "go":
        lines.append("  CLI_DOCS_GO_PACKAGE: ${{ vars.CLI_DOCS_GO_PACKAGE }}")
    lines.extend(
        [
            "",
            "jobs:",
    ...
    if language == "go":
        lines.extend(
            [
                '          if [[ "$CLI_DOCS_GO_PACKAGE" != "." && ! "$CLI_DOCS_GO_PACKAGE" =~ ^\\./[A-Za-z0-9][A-Za-z0-9._-]*(/[A-Za-z0-9][A-Za-z0-9._-]*)*$ ]]; then',
                "            printf '%s\\n' 'CLI_DOCS_GO_PACKAGE must be . or one explicit ./relative/package path' >&2",
                "            exit 2",
                "          fi",
            ]
        )
    if language == "python" and setup == "uv":
        ...
        lines.extend(
            [
                ...
                "          cache: true",
                "      - name: Build and verify Go executable",
                "        shell: bash",
                "        run: |",
                '          install_dir="$(mktemp -d)"',
                "          trap 'rm -rf -- \"$install_dir\"' EXIT",
                '          go build -trimpath -o "$install_dir/$CLI_DOCS_COMMAND" "$CLI_DOCS_GO_PACKAGE"',
                '          command_path="$(command -v -- "$install_dir/$CLI_DOCS_COMMAND")"',
                '          "$command_path" --help',
...
def _referenced_usage_index(
    config: Mapping[str, object],
    snapshots: Mapping[str, object],
) -> str | None:
    path = config.get("usage_index_path")
    if path is None:
        return None
    if not isinstance(path, str) or not path:
        raise ValueError("usage_index_path must be a repository-relative path")
    ...
    for raw in referenced:
        entry = _table(raw, name="referenced usage index")
        if (
            entry.get("standard_id") == "cli-documentation"
            and entry.get("extension_id") == "usage-index"
            and entry.get("path") == path
        ):
            matches.append(entry)
    if len(matches) != 1:
        raise ValueError("usage_index_path requires one locked referenced input")
    return path


def run_render_usage(
    request: Mapping[str, object],
    resources: Mapping[str, bytes],
) -> dict[str, object]:
    """Render the create-only usage scaffold and its optional index link."""
    template = resources.get("usage-template")
    if template is None:
        raise ValueError("usage template resource is unavailable")
    try:
        content = template.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise ValueError("usage template resource is not UTF-8") from exc
    config = _table(request.get("config"), name="config")
    snapshots = _table(request.get("snapshots"), name="snapshots")
    index_path = _referenced_usage_index(config, snapshots)
    ...
    if not isinstance(target, str) or "/" not in target:
        raise ValueError("planned usage target is unavailable")
    relative = posixpath.relpath(index_path, start=posixpath.dirname(target))
    # Filesystem containment does not make a path safe Markdown syntax; encode
    # delimiters so the consumer-owned path cannot escape the link destination.
    link = quote(relative, safe="/-._~")
    return {
        "content": f"{content.rstrip()}\n\n[Multi-CLI usage index]({link})\n",
    }
