...
| Go command | Primary implementation doc | Built executable naming, explicit `-o` output, and `go install` destination behavior | Compiled-command CI and installation guidance |
| Cobra | Primary implementation docs and source | Whole-tree Markdown, man pages, and multi-shell completion generation | Parser-driven Go documentation and generated-artifact drift checks |
| GoReleaser | Primary release-tool docs | Archives and package-manager mappings for man pages and completions | Evidence that ancillary documentation may be included or installed by release channel |
...
This preserves the two original research inputs behind the standard—the CLI documentation-framework synthesis and the prior in-repo CLI documentation-structure research—while adding a Go-specific source review for executable, generated-document, completion, packaging, and CI behavior. The original inputs independently converged on canonical man-style sections, modern Markdown notation in Markdown sources, and a clear separation between help text and full documentation; the Go review changes the language mapping, not that shared content model. `[S01]` `[S04]` `[S20]`–`[S25]`
...
The tooling landscape splits into a run-in-place half (man-page generation from `--help`) and an installed-distribution half (parser-to-docs generation and built-command testing). Python and Go use different artifact mechanics under the same observable documentation contract.
...
| `go build -o` | Built Go executable testing | Builds one explicit `main` package to the exact documented command name. It proves a compiled artifact without relying on `go run` or the package directory's inferred name. `[S20]` |
| Cobra `doc` package | Go command tree → Markdown/man pages | `GenMarkdownTree` and `GenManTree` provide the Go mapping for the Packaged-deep and generated-man-page rules. `[S21]` |
| Cobra completion APIs | Go command tree → Bash/Zsh/Fish/PowerShell | Makes completion scripts parser-derived surfaces and exposes shell-specific behavior that documentation must preserve. `[S22]` |
| GoReleaser package integrations | Built binary plus ancillary docs → package channels | Shows that man pages and completions can be bundled or installed by the selected channel without making GoReleaser a required tool. `[S23]` |
...
— and two checks that were considered but folded into other rules rather than kept as separate table rows: **color normalization** and **width normalization** are runtime capabilities, so README §9 requires each snapshot to normalize only the controls its renderer supports. A **broken-link / docs-build** check (useful once a Packaged-deep adopter actually stands up a generated multi-page docs site) was left out of the mandated baseline because docs-site hosting itself is optional (`MAY`) under the Packaged-deep profile — an adopter who does build a hosted site should add a link/build check for it, but the standard does not require the site in the first place.

## Go executables: why the command inventory is explicit

Go source layout does not provide the same public-command registry as Python's `[project.scripts]`. `go build` accepts package arguments and writes an executable for a single `main` package; `-o` supplies the artifact name. `go install` places executables under `GOBIN` or its documented fallback. Neither behavior says that every source directory containing `package main` is intended for users. `[S20]`

The standard therefore treats the release/build configuration as the Go distribution authority. CI names one explicit relative package and one reviewed executable basename, then builds the artifact to a temporary path. This is a policy inference from the tool behavior: it prevents incidental development commands from becoming documentation obligations and avoids accepting package patterns, multiple outputs, or traversal through a shell-facing repository variable.

`go run` remains useful during development, but it cannot prove the final executable name, package selection, distribution wiring, or linker-injected version behavior. The built executable is the correct parity boundary for `--help` and `--version`.

## Go ancillary documentation and shell completion

Cobra's command model can generate a whole Markdown tree, man pages, and completion scripts for Bash, Zsh, Fish, and PowerShell. `[S21]` `[S22]` This makes those files derived interfaces rather than independent prose. Once committed or shipped, regeneration plus byte or semantic comparison is the maintainable drift guard.

Distribution channels differ. `go install` installs executable packages, while release archives and OS package integrations can carry or install man pages and completion files. GoReleaser documents Homebrew and AUR mappings for those ancillary files, but the standard deliberately does not require GoReleaser: the stable requirement is to document what each selected channel actually does. `[S20]` `[S23]`

## Go workflow trust boundary

The provider keeps executable selection outside desired package configuration, matching the existing Python contract. GitHub repository variables supply the reviewed command basename and Go package path through the `vars` context. `[S25]` The rendered workflow validates both values, permits only `.` or one explicit contained relative package, quotes every shell expansion, reads the toolchain from `go.mod`, and pins `actions/setup-go` to a reviewed commit. `[S24]`
...
- The packaged/`src`-layout research report this standard's packaged-CLI half is grounded in: [`docs/research/2026-07-07-cli-usage-docs-packaged-src-layout-python.md`](../../../../../docs/research/2026-07-07-cli-usage-docs-packaged-src-layout-python.md).
