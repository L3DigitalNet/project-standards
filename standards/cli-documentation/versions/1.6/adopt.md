...
Optional settings whose value would be absent are omitted because TOML has no `null` value.
...
For one installed Go command with CI generation, use:
...
command_name = "toolname"
workflow_path = ".github/workflows/cli-docs-check.yml"
ci = { enabled = true, runner = "ubuntu-latest", language = "go", setup = "go" }
```
...
The rendered workflow takes its executable name from the reviewed, consumer-owned GitHub repository variable `CLI_DOCS_COMMAND`. It validates that value as a command basename before use. The Python profile builds a wheel, creates a throwaway virtual environment, installs the wheel, and invokes the installed wrapper from that environment. The generic profile resolves an already-installed command from the same repository variable.

The Go profile additionally reads `CLI_DOCS_GO_PACKAGE`. Set it to `.` for a root `main` package or one explicit repository-relative package such as `./cmd/toolname`. Patterns, absolute paths, traversal, whitespace, and multiple packages are rejected. The workflow reads the Go version from `go.mod`, builds the selected package with `go build -trimpath -o` into a temporary directory, invokes the resulting binary with `--help` and `--version`, and removes the temporary directory. Both action references are pinned to reviewed commits.
...
For a repository with several CLIs, point the fresh usage scaffold at one existing consumer-owned Markdown index:
...
profile = "script"
usage_ownership = "managed"
usage_index_path = "docs/script-docs/README.md"
ci = { enabled = false }
```

The configured path must be one contained repository-relative regular file. It must exist before reconciliation and must not be a directory, symlink, or package-owned output. Reconciliation records its digest as a referenced input, never writes or removes it, and adds a relative navigation link when creating `docs/usage.md`.

The generated usage surface remains one create-only file. After the scaffold exists, changing or removing `usage_index_path` updates referenced-input state but preserves the existing `docs/usage.md` bytes; update its navigation link manually when needed. The package does not generate per-command usage artifacts or a per-command CI matrix.
...
- The documented executable inventory comes from the distribution's explicit entry-point or release/build configuration; incidental source directories do not create public commands.
...
- CI smoke tests invoke the built wheel wrapper or compiled executable and normalize color and terminal width when the selected runtime supports those controls.
- If the CLI exposes shell completion, document supported shells, generation, temporary activation, persistent or package-managed installation, and shell-specific limitations; regenerate committed scripts in CI.
...
| Go package selection is unsafe or ambiguous | Set `CLI_DOCS_GO_PACKAGE` to `.` or one explicit `./relative/package` that builds exactly one `main` package. |
| Go build succeeds but the smoke test fails | Run the built executable, not `go run`; confirm `--help`, `--version`, and any linker-supplied version metadata are present in the artifact. |
