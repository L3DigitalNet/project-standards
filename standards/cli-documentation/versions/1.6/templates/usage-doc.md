...
| `2`  | Usage error, if this tool uses it |

Replace the example codes with the executable's exact exit contract.
...
If supported, list each shell, the exact generation command, temporary activation, persistent or package-managed installation, and shell-specific limitations. Remove this section when the command exposes no completion surface.
