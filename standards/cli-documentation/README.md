...
- **Status:** Active — registered for adoption; contract version 1.0 (a copy-adopted label; selected by consumers via `cli_documentation.version` — see [`meta/versioning.md`](../../meta/versioning.md))
- **Owner:** Project standards / repository template
- **Last updated:** 2026-07-07
- **Last source check:** 2026-07-07
- **Scope:** User-facing usage documentation for command-line tools — help text, the canonical Markdown usage reference, man pages, and the CI checks that keep those surfaces in sync — reusable across single-file scripts and packaged, installable CLIs.
...
- [CLI Documentation Standard](#cli-documentation-standard)
  ...
  - [1. Purpose](#1-purpose)
  - [2. Scope & sibling relations](#2-scope--sibling-relations)
  - [3. Profiles](#3-profiles)
  - [4. Usage-doc structure & notation](#4-usage-doc-structure--notation)
  ...
  - [14. Update process / review cadence](#14-update-process--review-cadence)
...
The following tool behaviors are assumed current as of the last source check. They shape the CI rules in [§9](#9-ci-drift-prevention); reverify them when a referenced tool changes.
...
- **The implementation toolchain** — uv, Ruff, BasedPyright, pytest, and packaging are owned by the [Python Tooling SSOT Standard](../python-tooling/README.md); how the CLI code is shaped is owned by the [Python Coding Standard](../python-coding/README.md).
...
## 4. Usage-doc structure & notation
...
The standard ships copy-adopt scaffolds. Fill the placeholders and delete guidance comments before committing.
...
| [`templates/cli-docs-check.yml`](templates/cli-docs-check.yml) | A copy-adopt CI workflow: fresh install plus a `NO_COLOR`/width-normalized installed-`--help` smoke test. |

---

## 12. Adoption

The full adoption procedure — selecting a profile, what `project-standards adopt cli-documentation` materializes versus what is copied manually, filling the usage scaffold, wiring the CI workflow, and the authoring/review checklist — is in [`adopt.md`](adopt.md); it is not repeated here. The single-file README template is manual-copy, because script repositories rarely run `adopt`.

---

## 13. Exceptions process

Choosing a smaller profile, omitting an optional section, or shipping a man page as best-effort is **tailoring**, not an exception — the profiles and checks are built to accept it. A true exception is deviating from a MUST this standard states: documenting a command surface under a name other than its entry-point key, hand-maintaining per-command pages under the deep profile, or omitting a leaf command from the usage reference. Record a genuine exception in the adopter's usage doc or repo docs, stating what the standard requires, what the project does instead, which CI drift guards are consequently disabled, and why tailoring does not already cover it — the same shape the [Project Specification Standard](../project-spec/README.md#7-exceptions-process) uses. An exception forfeits the drift guarantees the standard exists to provide, so prefer tailoring wherever it suffices.

---

## 14. Update process / review cadence
