...
- Policy decisions are explicit local choices for this project ecosystem—informed by sources, but not mandated by them. The ADR-specific choices are: the `id` embeds the `repo-name` while the filename omits it; ADRs live under `docs/adr/`; the MADR body-section check is opt-in and off by default; authors must bound a decision explicitly without adding a required MADR heading; and partial amendment is recorded reciprocally under the sanctioned `project` frontmatter namespace. MADR defines supersession only, so the amendment vocabulary is local policy throughout.
...
These are authoring and review requirements within the existing MADR sections. Package 1.5 adds no required heading, no required frontmatter field, and no semantic prose validator; reciprocity of `project.amends` and `project.amended_by` remains an authoring responsibility in this release.
