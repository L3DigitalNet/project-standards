...
`contract_version` selects the ADR document/body contract independently of package version `1.3`. Set `require_sections = true` to require `## Context and Problem Statement`, `## Considered Options`, and `## Decision Outcome` on `doc_type: adr` snapshots.
