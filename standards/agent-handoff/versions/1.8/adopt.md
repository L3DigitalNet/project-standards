# Adopt Agent Handoff 1.8

Agent Handoff 1.8 is reconciled by the V5 control plane. Do not copy templates, merge legacy fragments, or retain `.agents/agent-handoff/manifest.json` as a second ownership authority.
...
For automatic startup, provide either a usable `python3` version 3.14 or newer on `PATH`, or `uv` with an already-installed Python 3.14 interpreter. The launcher uses `uv run --no-project --python 3.14 --no-python-downloads` only when the `python3` probe fails, so it ignores consumer-project Python constraints and never downloads an interpreter during startup. The hook source is payload data with mode `100644`; reconciliation installs the managed artifact with its declared mode `0755`.
...
`contract_version = "1.0"` remains supported independently from the selected 1.8 package for migrated consumers.
...
If automatic startup reports that Python 3.14 or `uv` is required, expose a supported `python3`, make Python 3.14 available to `uv`, or select manual startup. The failure path writes no SessionStart context to stdout.
