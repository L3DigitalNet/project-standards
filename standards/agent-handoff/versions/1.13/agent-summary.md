...
- The skill is installed twice, under `.agents/skills/agent-handoff/` for Codex and `.claude/skills/agent-handoff/` for Claude Code, as byte-identical managed copies. Never edit or delete one to deduplicate them.
...
- Independently configured Python and Markdown tools must exclude the locked hook and skill surfaces documented in the [adoption guide](adopt.md#exclude-locked-artifacts-from-independent-tools); do not format, lint, or type-check managed payload bytes.
