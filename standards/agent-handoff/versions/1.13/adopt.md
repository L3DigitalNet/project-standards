# Adopt Agent Handoff 1.13

Agent Handoff 1.13 is reconciled by the V5 control plane. Do not copy templates, merge legacy fragments, or retain `.agents/agent-handoff/manifest.json` as a second ownership authority.
...
`contract_version = "1.0"` remains supported independently from the selected 1.13 package for migrated consumers.
...
1.13 adds a second installed copy of every skill file under `.claude/skills/agent-handoff/`. Claude Code discovers project skills only there and has never read `.agents/skills/`, which is Codex's convention, so before 1.13 the packaged skill was invisible to Claude Code sessions. No option, provider, contribution, or existing artifact target changes, and no skill byte changes: reconciling 1.13 adds files and rewrites none.

The new tree is package-managed like the old one. Commit it, and extend any formatter, linter, or size-guard exclusion that names `.agents/skills/agent-handoff/**` to name `.claude/skills/agent-handoff/**` as well — see the exclusions above.
