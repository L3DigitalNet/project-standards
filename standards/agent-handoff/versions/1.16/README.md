...
1.16 re-cites the cross-file contract comment that pins `_SKILL_TARGETS` against the payload's `[[artifacts]]` set: it now names the family-generic `tests/package_contract/test_provider_registry.py` instead of a per-family registry test (#196). No option, policy value, template, hook, provider, contribution, or artifact target changes.
