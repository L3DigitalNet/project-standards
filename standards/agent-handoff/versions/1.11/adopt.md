# Adopt Agent Handoff 1.11

Agent Handoff 1.11 is reconciled by the V5 control plane. Do not copy templates, merge legacy fragments, or retain `.agents/agent-handoff/manifest.json` as a second ownership authority.
...
`contract_version = "1.0"` remains supported independently from the selected 1.11 package for migrated consumers.
...
1.11 changes documentation only. No option, artifact, contribution, or provider byte differs from 1.10, so a 1.10 consumer that selects 1.11 sees no reconciliation change.
