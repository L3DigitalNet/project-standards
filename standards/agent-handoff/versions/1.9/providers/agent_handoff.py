...
_SESSION_START_SCRIPT = (
    'if python3 -c "import sys; raise SystemExit(sys.version_info < (3, 14))" '
    '</dev/null >/dev/null 2>&1; then exec python3 "$1"; '
    "elif command -v uv >/dev/null 2>&1 && "
    'uv run --no-project --python 3.14 --no-python-downloads python3 -c "import sys; '
    'raise SystemExit(sys.version_info < (3, 14))" </dev/null >/dev/null 2>&1; then '
    'exec uv run --no-project --python 3.14 --no-python-downloads python3 "$1"; '
    'else printf "%s\\n" "agent-handoff: requires Python 3.14+ or uv with an installed '
    'Python 3.14" >&2; exit 127; fi'
)


def _session_start_command(hook: str) -> str:
    """Return a launcher that preserves SessionStart stdin for the hook.

    Both version probes read from `/dev/null`; reading the event to inspect Python
    would leave the selected hook with empty input and no startup context. Their
    output is suppressed so a policy shim or failed uv resolution cannot corrupt
    the harness transport before the hook starts.
    """
    return f"sh -c '{_SESSION_START_SCRIPT}' agent-handoff \"{hook}\""
...
def _claude(config: Mapping[str, object]) -> dict[str, object]:
    ...
    executable, and absent it hands `command` to a shell. The launcher is an
    `sh -c` wrapper with a conditional and a `$1`, so it only runs under shell
    form. 1.1 through 1.7 carried `args: []` harmlessly because `command` was a
    bare script path; 1.8 made the two fields contradict and the hook never ran.

    Rejected: keeping exec form with `command = "sh"` and the wrapper moved into
    `args`. It works, but it splits the launcher across two fields, diverges the
    Claude rendering from the Codex one that must stay a single command string,
    and hides the hook path from every reader that inspects `command`.
    """
...
def _codex(config: Mapping[str, object]) -> str:
    command = _session_start_command(
        "$(git rev-parse --show-toplevel)/.agents/hooks/agent-handoff/session_start.py"
    )
    return (
        "[[hooks.SessionStart]]\n"
        'matcher = "startup|resume|clear|compact"\n'
