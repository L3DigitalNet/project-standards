...
Agent Handoff package `1.9` defines repository-local project knowledge, bounded session continuity, and deterministic conformance checks for coding agents. The package supports consumer contracts `1.0` and `1.1` independently from its payload version. An adopting repository is the complete authority boundary: the standard never requires a separate checkout and never creates, reads, or stores consumer state outside that repository.

## Runtime prerequisite

Automatic startup requires either a usable `python3` version 3.14 or newer on `PATH`, or `uv` with an already-installed Python 3.14 interpreter. The launcher probes `python3` without consuming the SessionStart event, then falls back to `uv run --no-project --python 3.14 --no-python-downloads`; it never inherits the consumer project's Python constraint or downloads an interpreter during startup.
...
| `.agents/hooks/agent-handoff/session_start.py` | Standard | Stored in the payload as data mode `100644`; installed only for automatic mode at the declared managed artifact mode `0755`; hash-tracked |
...
- `.agents/hooks/agent-handoff/session_start.py` when automatic startup is selected;
- `.agents/skills/agent-handoff/**` for the skill and its interface metadata.
...
Automatic profiles require the selected harness's normal project trust and hook review. Agent Handoff documents that prerequisite but never changes user or global trust state. If neither supported interpreter route is available, the launcher writes no context to stdout and emits one prerequisite diagnostic on stderr; expose Python 3.14, install `uv` with Python 3.14 already available, or switch to manual startup.
