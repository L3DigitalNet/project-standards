# Adopt Agent Handoff 1.12

Agent Handoff 1.12 is reconciled by the V5 control plane. Do not copy templates, merge legacy fragments, or retain `.agents/agent-handoff/manifest.json` as a second ownership authority.
...
`contract_version = "1.0"` remains supported independently from the selected 1.12 package for migrated consumers.
...
When Project Standards Markdown Tooling is also enabled, prefer its typed `exclusions` option over hand-written ignore files. One record covers Prettier and markdownlint together and carries its own justification:
...
1.12 changes documentation only. No option, artifact, contribution, or provider byte differs from 1.11, so a 1.11 consumer that selects 1.12 sees no reconciliation change.
