...
`gh-workflow ledger` is not read-only. It writes `docs/GH-WORKFLOWS.md` in the repository, so review the resulting diff and decide deliberately whether the file is committed or ignored. See the [version-specific guide](versions/1.0/adopt.md) for exact options, first-run expectations, the ledger's ownership rules, and troubleshooting.
