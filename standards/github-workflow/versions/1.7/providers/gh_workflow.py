...
def _block_body(organization: str) -> str:
    ...
    ten-subcommand routing table, operator-sufficient authority, the T0-or-draft
    admission rule, the three-way governing-work declaration, terminal pairing, the
    three standing prohibitions, and the finding-disposition rule. What stays out is
    still deliberate — flag tables, field vocabularies, issue-body headings, exit
    codes, the T0 predicate itself, and the summary and receipt layouts — because
    every session in a consuming repository pays for this text whether or not GitHub
    work is in scope, and each omitted item is either recoverable from the binary's
    own refusals or needed only after a judgment call that has already sent the agent
    to SKILL.md.

    The byte length varies with the organization login, so a test asserts a ceiling
    rather than an equality. NFR-003 holds that ceiling at 2,400 bytes across 1.7's
    added content: measured 2,392 B at a 39-character login (GitHub's maximum), which
    is 8 bytes of headroom — prose added here without removing prose will exceed it.
    """
    return (
        ...
        "every GitHub work-state action through the complete table below. Every row is "
        "a `gh-workflow` subcommand unless it says raw `gh`. Load the `github-workflow` "
        "skill for triage, an organization-schema audit, T0 or relationship judgment, "
        "or uncommon recovery.\n"
        "\n"
        "| Action | Command |\n"
        "| --- | --- |\n"
        ...
        "--issue N --workflow VALUE` |\n"
        "| Check or read an issue or PR | `check --issue N` / `check --pr N [--through "
        "PHASE]` / `receipt --issue N` / `receipt --pr N` |\n"
        ...
        "`.agents/skills/github-workflow/bin/gh-workflow` (and its `.claude/` twin); "
        "refusals name the valid values, so invoke it rather than guessing. These rules "
        "bind even when the skill was never loaded:\n"
        "\n"
        "- An operator instruction is sufficient authority for the action it names. You "
        "author acceptance criteria and admit work to `Ready` yourself; open state never "
        "implies `Ready`.\n"
        "- A T0 commit — trivial prose repair, no protected surface, one "
        "`Workflow-Admission: T0` trailer — is the only autonomous direct push; all "
        "other work starts as a draft PR.\n"
        "- Every PR declares `Final: #N`, `Supporting: #N`, or `Standalone` under "
        "`## Governing work`.\n"
        "- Keep terminal state paired: `Done` closes as completed, `Dropped` as not "
        "planned, and reopen returns a nonterminal `Workflow` value.\n"
        "- Never create shadow state labels, mutate organization schema through this "
        "package, or bypass live enforcement.\n"
        "- A related finding you can address this session needs no issue: fix it in "
        "place when this repository owns it, file it against the owning upstream "
        "repository, and ask the operator only when it needs its own session.\n"
        "\n"
