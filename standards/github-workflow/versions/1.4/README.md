# GitHub Workflow Standard 1.4

- **Status:** Active; immutable package version 1.4.
...
From 1.4 the ledger escapes an underscore in a title only where Markdown could read it as an emphasis marker: at a word edge, next to punctuation, or in a run of two or more. An intraword underscore — a `snake_case` symbol name in an issue title — is emitted literally. Prettier, which owns markdown formatting in a consuming repository, strips the redundant escape, so the earlier unconditional form made every regeneration of such a ledger fail the consumer's own `prettier --check` until someone ran `prettier --write` by hand. The rendered characters are unchanged either way, and `|` — which would silently add a column — keeps its escape.
