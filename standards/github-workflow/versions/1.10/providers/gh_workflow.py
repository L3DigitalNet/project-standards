...
# format that Claude Code has never read, so from 1.5 it is declared under `.agents/` only
...
# test_provider_registry.py` pins the equality for every advertised payload at once,
# so the check survives a cut that copies this table forward unread (issue #196).
...
def _block_body(organization: str) -> str:
    ...
    From 1.7 the block carries the complete high-frequency contract (FR-007): the
    routing table, operator-sufficient authority, the admission rule, the three-way
    governing-work declaration, terminal pairing, the standing prohibitions, and the
    finding-disposition rule. What stays out is still deliberate — flag tables, field
    vocabularies, issue-body headings, exit codes, the T0 predicate itself, and the
    summary and receipt layouts — because every session in a consuming repository pays
    for this text whether or not GitHub work is in scope, and each omitted item is
    either recoverable from the binary's own refusals or needed only after a judgment
    call that has already sent the agent to SKILL.md.

    1.9 replaces the two-class admission bullet with the four `Workflow-Admission`
    classes, the handoff path set, and the mixed-commit rule (ADR 0031 D2, issue #218).
    The block is the only surface a delegated worker is guaranteed to read, so the
    exempt paths are enumerated here rather than left to pr-standard.md.

    The byte length varies with the organization login, so a test asserts a ceiling
    rather than an equality, and NFR-003 holds that ceiling at 2,400 bytes. Paying for
    the admission classes inside it displaced content rather than raising it: the
    former `Wait for CI` row is gone, since it was the one route that mutates nothing
    and costs at most one extra call when forgotten, and SKILL.md's routing table still
    carries it. Measured 2,394 B at a 39-character login (GitHub's maximum), so prose
    added here without removing prose will exceed the ceiling.
    """
    return (
        ...
        "every GitHub work-state action through the table below; every row is a "
        "`gh-workflow` subcommand unless it says raw `gh`. Load the `github-workflow` "
        "skill for triage, a schema audit, T0 or relationship judgment, or rare "
        "recovery.\n"
        "\n"
        "| Action | Command |\n"
        "| --- | --- |\n"
        ...
        "| Close or reopen an issue | `close --issue N --as done\\|dropped` / `reopen "
        "--issue N --workflow V` |\n"
        "| Check or read an issue or PR | `check`/`receipt --issue N` or `--pr N`; "
        "`check --pr N --through PHASE` |\n"
        ...
        "\n"
        "All ten accept `--output human|json`; the binary is at "
        "`.agents/skills/github-workflow/bin/gh-workflow` and its `.claude/` twin, and "
        "its refusals name valid values. These rules bind even when the skill was "
        "never loaded:\n"
        "\n"
        "- An operator instruction is sufficient authority for the action it names. You "
        "author acceptance criteria and admit work to `Ready`; open state never implies "
        "it.\n"
        "- Every commit on a governed branch carries one `Workflow-Admission` trailer: "
        "`T0` (trivial prose repair, no "
        "protected surface), `PR #N` (written by `merge`), `handoff` (touches only "
        "`docs/handoff/**`, `docs/STATUS.md`, `docs/TODO.md`), or `release`. A commit "
        "mixing handoff and other paths is not handoff: like all other work it starts "
        "as a draft PR declaring `Final: #N`, `Supporting: #N`, or `Standalone` under "
        "`## Governing work`.\n"
        "- Keep terminal state paired: `Done` closes as completed, `Dropped` as not "
        "planned; reopen returns a nonterminal `Workflow` value.\n"
        "- Never create shadow state labels, mutate organization schema, or bypass "
        "live enforcement.\n"
        "- A related finding you can address this session needs no issue: fix it here "
        "when this repository owns it, file it against the owning upstream "
        "repository, ask the operator only when it needs its own session.\n"
        "\n"
...
# The admission options ADR 0031 D1 adds, with the defaults `config.schema.json`
# declares. Every one is a scalar because the tool parses `policy.toml` with a bounded
# reader that accepts only comments, table headers, and double-quoted assignments; a
# list or an inline table here would need the Go parser changed in the same cut.
#
# The empty defaults are meaningful, not placeholders: no `integration_branch` is the
# two-branch topology, and no `admission_floor` classifies the whole history.
_ADMISSION_OPTIONS: tuple[tuple[str, str], ...] = (
    ("integration_branch", ""),
...
def _policy_document(
    resources: Mapping[str, bytes], config: Mapping[str, object], version: str
) -> str:
    ...
    rendered = (
        ...
        .replace("@organization@", _organization(config))
        .replace("@package_version@", version)
    )
    for name, value in _admission_values(config).items():
...
def run_render_semantic(
    ...
    if adapter == "whole-file" and target == _POLICY_TARGET:
        return {"content": _policy_document(resources, config, _version(request))}
...
def _findings(
    ...
    findings.extend(
        _rendered_findings(
            ...
            _policy_document(resources, _config(request), _version(request)).encode("utf-8"),
        )
    )
...
def run_upgrade(request: Mapping[str, object], resources: Mapping[str, bytes]) -> dict[str, object]:
    ...
    content = _policy_document(resources, _config(request), _version(request)).encode("utf-8")
    ...
    return {
        ...
        "actions": [action],
        "diagnostics": _residual_ledger_diagnostics(_snapshots(request)),
    }


def _residual_ledger_diagnostics(snapshots: Mapping[str, object]) -> list[dict[str, object]]:
    """Report the removed ledger's orphaned file, but only on the evidence of it.

    Through 1.6 this advisory was emitted unconditionally, because the upgrade
    snapshot covers the package's declared targets and `docs/GH-WORKFLOWS.md` was
    never one (spec DR-003), so the provider usually cannot see whether the consumer
    has a copy. That was defensible while 1.5 was the upgrade everyone was taking; at
    1.7 the removal is two versions old and the unconditional warning fires on every
    1.6 → 1.7 upgrade in a repository that never ran `ledger`, which is noise that
    trains operators to skim diagnostics. FR-025 therefore scopes it to its source:
    the notice appears only when the caller actually captured the path as a regular
    file, and the upgrade returns no diagnostics otherwise.

    The guidance a consumer still on 1.4 or earlier needs did not disappear with the
    unconditional notice — `adopt.md` carries the one manual deletion step, which is
    where a consumer performing that jump is already reading.
    """
    if _entry(snapshots, _LEDGER_TARGET).get("kind") != "regular":
        return []
    return [
        {
            ...
            "path": _LEDGER_TARGET,
            "message": (
                f"{_LEDGER_TARGET} was generated by the `ledger` subcommand, which version "
                "1.5 removed. Nothing regenerates or owns it now: delete it, or keep it "
