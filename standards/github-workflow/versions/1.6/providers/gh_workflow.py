...
def _block_body(organization: str) -> str:
    ...
    return (
        ...
        "`github-workflow` skill (`SKILL.md`, one read, ~70 lines) before triage, an "
        ...
        "- A related finding you can address this session gets no issue: fix it in place "
        "when this repository owns it, file an issue in the owning repository when an "
        "upstream dependency in the organization owns it, and ask the operator whether to "
        "file or tackle it now only when it warrants a full separate session.\n"
        "\n"
...
def _orphaned_ledger_diagnostic(snapshots: Mapping[str, object]) -> dict[str, object]:
    ...
    `ledger`. Reporting it unconditionally is the point: only payloads from 1.5 onward
    emit this, so the notice ends with the upgrade that caused it.
    """
