...
Pinning binds before the issue exists, which is why the matrix stays in this document: `gh-workflow check` reports missing pinned fields only once there is an issue to check. Initiatives deliberately omit execution-oriented fields because an Initiative itself should normally not be directly implemented. Organization schema changes — adding, renaming, or retiring a field or a value — are human work. Agents audit and report drift with `gh-workflow audit`; they never mutate the organization schema.
