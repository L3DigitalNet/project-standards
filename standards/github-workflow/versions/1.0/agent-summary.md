...
- Reconcile delivers the skill, six references, and the `gh-workflow` binary under `.agents/skills/github-workflow/`, renders `.standards/packages/github-workflow/policy.toml`, and adds one managed instruction block per selected harness.
...
- `docs/GH-WORKFLOWS.md` is tool-generated: owned whole-file by `ledger`, outside the payload digests and drift-check, never hand-edited.
