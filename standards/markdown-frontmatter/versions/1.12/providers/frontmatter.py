...
from project_standards.validate_id import ADR_ID_RE, validate_id
from project_standards.validate_references import reference_values
...
# 5.8.0 FR-006/FR-007 / issue #28: legacy namespace path for the workflow_ownership
# escape and the whole-file caller signature that its relinquishment claim targets.
_WORKFLOW_OWNERSHIP_POINTER = "/markdown/frontmatter/workflow_ownership"
_CALLER_SIGNATURE = "legacy-validate-standards-workflow"
...
# `fromJSON(inputs.runner-labels)`. An unset or empty `runner_labels` contributes
# nothing, so the render stays exactly the static `workflow-job-caller` bytes 1.9
# shipped — the property `test_markdown_frontmatter_1_10` asserts. The label
...
def _runner_labels_entry(config: Mapping[str, object]) -> str:
    ...
    if not labels:
        return ""
    # Indented four spaces to sit under `jobs.frontmatter`, the key this fragment
    # contributes; the composed target is the shared validate-standards.yml.
    return f"    with:\n      runner-labels: '{json.dumps(labels, separators=(',', ':'))}'\n"
...
def run_render_workflow(
    ...
    content = resources[resource_id].decode("utf-8")
    if mode == "caller":
        content += _runner_labels_entry(config)
...
def _repository_findings(
    ...
    for path, metadata in documents:
        ...
        for reference in reference_values(dict(metadata)):
            if "#" in reference:
                message = "section anchors are not valid document references"
            elif _reference_resolves(reference, ids, regular_paths) or ADR_ID_RE.match(reference):
                continue
            else:
...
def run_migrate(
    ...
    for key in sorted(frontmatter):
        ...
        if key == "version":
        ...
        elif key == "workflow_ownership":
            # 5.8.0 FR-006/FR-007 / issue #28: recognizing this setting is the whole
            # fix for CP-MIGRATION-UNCLAIMED-SETTING. "consumer-owned" additionally
            # unlocks the caller relinquishment claim emitted in the signature loop.
            config["workflow_ownership"] = frontmatter[key]
            recognized.append(_WORKFLOW_OWNERSHIP_POINTER)
        elif key == "schema":
            ...
            config[key] = list(
                dict.fromkeys((*_DEFAULT_EXCLUSIONS, *cast("Sequence[str]", values)))
            )
    ...
    # 5.8.0 FR-006/FR-007 / issue #28: consumer-owned unlocks caller relinquishment.
    # The escape applies only to the composed validate-standards.yml caller.
    relinquish_caller = config.get("workflow_ownership") == "consumer-owned"
    for signature_id, (target, disposition) in dispositions.items():
        signature = raw_signatures.get(signature_id)
        if not isinstance(signature, Mapping):
            continue
        observed = cast("Mapping[str, object]", signature).get(target)
        if not isinstance(observed, Mapping):
            continue
        state = cast("Mapping[str, object]", observed)
        digest = state.get("digest")
        known = state.get("known") is True
        relinquish = signature_id == _CALLER_SIGNATURE and relinquish_caller
        if known:
            ...
            if relinquish:
                # Recognized caller the consumer opted to own: preserve byte-exact,
                # retire no historical unit, and carry no intent pointer — a known
                # digest clears through recognized-history, and an intent pointer on
                # recognized history is itself a CP-MIGRATION-OWNER-RESOLUTION block.
                claims.append(
                    {
                ...
                claims.append(
                    {
                        ...
                        **(
                            ...
                            if signature_id == _CALLER_SIGNATURE
                            else {}
                        ),
                    }
                )
        elif relinquish and isinstance(digest, str):
            # Customized (unknown-digest) caller: the intent pointer authorizes the
            # migration to preserve it as consumer-owned content instead of failing
            # closed on the unrecognized digest. This is the sole escape route.
            claims.append(
                {
                    "signature_id": signature_id,
                    "target": target,
                    ...
                    "ownership": "consumer-owned",
                    "disposition": "preserve",
                    "intent_pointer": _WORKFLOW_OWNERSHIP_POINTER,
                }
            )
        # Unknown digest without the intent: emit no claim so the engine fails closed
        # on CP-MIGRATION-LEGACY-DIGEST — relinquishment stays strictly opt-in.
    return {
        "schema_version": "1.0",
        "package": {
            "standard_id": "markdown-frontmatter",
...
def run_verify_runner_labels(
    ...
    workflow_ownership = config.get("workflow_ownership")
    if workflow_mode == "self-hosted":
        reason = "runner_labels cannot reach the direct self-hosted frontmatter job"
        hint = (
    ...
    elif workflow_ownership == "consumer-owned":
        reason = "runner_labels cannot reach the consumer-owned frontmatter caller"
        hint = (
            "pass runner-labels from the consumer-owned frontmatter caller, or set "
    ...
    return {
        "findings": [
            {
                "code": "FM-RUNNER-LABELS-UNREACHABLE",
                "severity": "warning",
                "path": ".github/workflows/validate-standards.yml",
                "identity": "key:/jobs/frontmatter",
                "message": reason,
                "hint": hint,
            }
        ]
