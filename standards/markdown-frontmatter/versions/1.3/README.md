...
**Package version:** `1.3`. **Document contract:** `1.1` (declared per document as `schema_version`; selected independently through `standards.markdown-frontmatter.config.contract_version`). See [Versioning](#versioning).
...
- **Package payload `1.3`** selects these immutable package resources, providers, and managed outputs through the consumer catalog.
