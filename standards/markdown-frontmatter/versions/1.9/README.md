...
**Package version:** `1.9`. **Document contract:** `1.1` (declared per document as `schema_version`; selected independently through `standards.markdown-frontmatter.config.contract_version`). See [Versioning](#versioning).
...
- **Package payload `1.9`** selects these immutable package resources, providers, and managed outputs through the consumer catalog.
...
Package `1.9` adds an optional `runner-labels` reusable-workflow input to the self-hosted validation workflow (empty keeps the GitHub-hosted runner) and bumps its pinned `setup-uv` action to `v9.0.0` with `prune-cache: true` preserving the prior pruning default. It does not add fields, remove fields, or change controlled vocabularies. New documents should still set `schema_version: '1.1'`.
