...
Lifecycle: active. Package: `markdown-frontmatter@1.15`.
...
- Non-empty `runner_labels` reach only a managed caller in `caller` mode. Consumer-owned and direct self-hosted configurations produce a non-fatal reconciliation warning with the appropriate caller remedy; empty labels remain silent.
...
The aggregate validator covers schema, ID, and enabled reference checks. `format-frontmatter` checks or fixes canonical source style. The repo-local [agent skill](skills/markdown-frontmatter/SKILL.md) guides authoring and ID generation in adopting repositories; it installs as byte-identical managed copies under `.agents/skills/markdown-frontmatter/` for Codex and `.claude/skills/markdown-frontmatter/` for Claude Code, and neither copy may be edited or deleted to deduplicate them. The package also owns one marked `markdown-frontmatter` block in the selected harnesses' `AGENTS.md` and `CLAUDE.md`; that block is body content, never frontmatter.
