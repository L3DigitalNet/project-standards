...
The placeholder token in this standard's examples and templates is the uppercase `XXXXXX`, which is deliberately outside `[0-9a-z]` and therefore fails ID validation. A copied placeholder is reported rather than accepted, and `validate-id --fix` mints the real token. A lowercase placeholder would be a valid token and could ship unnoticed.
...
Adding optional fields or enum values is a compatible schema change. Removing a field or enum value, making a field required, or tightening a pattern is a breaking schema change. Release classification follows the [Versioning Standard](../../../../meta/versioning.md).
