...
**Package version:** `1.13`. **Document contract:** `1.1` (declared per document as `schema_version`; selected independently through `standards.markdown-frontmatter.config.contract_version`). See [Versioning](#versioning).
...
These paths are excluded by default. Keep them outside the managed-document corpus instead of adding document metadata to them. A repository may also exclude its root `README.md` if it prefers not to render a frontmatter table on its public landing page.
...
- **Package payload `1.13`** selects these immutable package resources, providers, and managed outputs through the consumer catalog.
