# Adopt Markdown Frontmatter 1.13
...
The common control-plane lifecycle—initialization, catalog selection, preview, apply, upgrade, disable, and removal—is defined in the [project-standards V5 CLI guide](../../../../docs/usage.md). This guide covers only the choices and verification specific to `markdown-frontmatter@1.13`.
...
Follow the V5 CLI guide for preview and apply mechanics. Applying this package installs the repo-local skill under both `.agents/skills/markdown-frontmatter/` and `.claude/skills/markdown-frontmatter/` — Claude Code discovers project skills only under the latter, Codex only under the former — records the package summary under `.standards/packages/markdown-frontmatter/`, and composes the Frontmatter job into `.github/workflows/validate-standards.yml`. In `self-hosted` mode it also manages `.github/workflows/validate-markdown-frontmatter.yml`, which the composed job calls from the same commit. A second reconciliation must report no changes.

## Package options

| Option | Default | Purpose |
| --- | --- | --- |
| `contract_version` | `"1.1"` | Select the frontmatter document contract independently of package version `1.13`. |
...
The package selector and document-contract selector are different controls. `version = "1.13"` selects the immutable package payload; `contract_version = "1.1"` selects the schema behavior exposed by that payload.
...
- [ ] `.standards/config.toml` enables `markdown-frontmatter@1.13` with accurate include and exclude patterns.
