# Adopt Markdown Frontmatter 1.2
...
The package selector and document-contract selector are different controls. `version = "1.2"` selects the immutable package payload; `contract_version = "1.1"` selects the schema behavior exposed by that payload.
...
- [ ] `.standards/config.toml` enables `markdown-frontmatter@1.2` with accurate include and exclude patterns.
