# Adopt Markdown Frontmatter 1.7
...
The common control-plane lifecycle—initialization, catalog selection, preview, apply, upgrade, disable, and removal—is defined in the [project-standards V5 CLI guide](../../../../docs/usage.md). This guide covers only the choices and verification specific to `markdown-frontmatter@1.7`.
...
| --- | --- | --- |
| `contract_version` | `"1.1"` | Select the frontmatter document contract independently of package version `1.7`. |
...
The package selector and document-contract selector are different controls. `version = "1.7"` selects the immutable package payload; `contract_version = "1.1"` selects the schema behavior exposed by that payload.
...
- [ ] `.standards/config.toml` enables `markdown-frontmatter@1.7` with accurate include and exclude patterns.
