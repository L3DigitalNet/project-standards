...
**Package version:** `1.8`. **Document contract:** `1.1` (declared per document as `schema_version`; selected independently through `standards.markdown-frontmatter.config.contract_version`). See [Versioning](#versioning).
...
- **Package payload `1.8`** selects these immutable package resources, providers, and managed outputs through the consumer catalog.
...
Package `1.8` packages the SHA-pinned self-hosted validation workflow. It does not add fields, remove fields, or change controlled vocabularies. New documents should still set `schema_version: '1.1'`.
