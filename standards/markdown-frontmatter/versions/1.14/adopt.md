# Adopt Markdown Frontmatter 1.14
...
The common control-plane lifecycle—initialization, catalog selection, preview, apply, upgrade, disable, and removal—is defined in the [project-standards V5 CLI guide](../../../../docs/usage.md). This guide covers only the choices and verification specific to `markdown-frontmatter@1.14`.
...
Use the [V5 CLI guide](../../../../docs/usage.md) to initialize the repository when needed. Then enable the exact package:
...
Follow the V5 CLI guide for preview and apply mechanics. Applying this package installs the repo-local skill under both `.agents/skills/markdown-frontmatter/` and `.claude/skills/markdown-frontmatter/` — Claude Code discovers project skills only under the latter, Codex only under the former — records the package summary under `.standards/packages/markdown-frontmatter/`, adds the managed `markdown-frontmatter` instruction block to the selected harnesses' `AGENTS.md` and `CLAUDE.md`, and composes the Frontmatter job into `.github/workflows/validate-standards.yml`. In `self-hosted` mode it also manages `.github/workflows/validate-markdown-frontmatter.yml`, which the composed job calls from the same commit. A second reconciliation must report no changes.

## Package options

| Option | Default | Purpose |
| --- | --- | --- |
| `contract_version` | `"1.1"` | Select the frontmatter document contract independently of package version `1.14`. |
...
| `runner_labels` | `[]` | Runner labels the composed `frontmatter` job passes to the reusable workflow, for example `["self-hosted", "linux", "x64"]`. Empty omits the input and keeps the GitHub-hosted runner. Labels are limited to letters, digits, `.`, `_`, and `-`. |
| `harnesses` | `["claude-code", "codex"]` | Selected coding-agent harnesses. Controls only which skill artifacts install where a target is harness-specific; it does not gate the workflow, schema, or corpus options above. |

`runner_labels` applies to a managed caller in `caller` mode. When it is non-empty the composed `frontmatter` job gains a `with:` block carrying `runner-labels` as a JSON-array string, because a reusable-workflow input cannot be typed as a sequence. When it is empty — the default — no `with:` block is composed and the job is byte-for-byte the one package `1.9` produced, so a repository that sets nothing sees no reconciliation change.

Reconciliation reports a non-fatal warning when non-empty labels cannot reach that input. For `workflow_ownership = "consumer-owned"`, pass `runner-labels` from the consumer-owned `frontmatter` job or return the caller to managed ownership. For `workflow_mode = "self-hosted"`, return to the managed caller path or keep the workflow consumer-owned and pin its `runs-on` selection directly. Empty labels and a managed caller produce no reachability warning.

`harnesses` also decides which agent-instruction files receive the managed `markdown-frontmatter` block: `AGENTS.md` when the option contains `codex`, `CLAUDE.md` when it contains `claude-code`. Each block names the skill path that harness actually reads, and the two files are otherwise untouched — the package owns only the marked block. Deselecting a harness removes that harness's block on the next reconcile.

`harnesses` decides installation of the Codex-only `agents/openai.yaml` skill sidecar: it installs at `.agents/skills/markdown-frontmatter/agents/openai.yaml` only `when_any` `harnesses` contains `codex`. Claude Code has no analogous sidecar convention and has never read it, so a `.claude/skills/markdown-frontmatter/agents/openai.yaml` copy is never declared (issue #175). `SKILL.md` and `scripts/new-doc-id` are unaffected: both keep installing to `.agents/skills/` and `.claude/skills/` unconditionally, because Claude Code does read `.claude/skills/markdown-frontmatter/SKILL.md`. The default selects both harnesses, so a repository that sets nothing keeps the Codex copy and only loses the Claude-side `openai.yaml` copy on reconcile.
...
The package selector and document-contract selector are different controls. `version = "1.14"` selects the immutable package payload; `contract_version = "1.1"` selects the schema behavior exposed by that payload.
...
- [ ] `.standards/config.toml` enables `markdown-frontmatter@1.14` with accurate include and exclude patterns.
...
- [ ] `.agents/skills/markdown-frontmatter/` and `.claude/skills/markdown-frontmatter/` both match the selected payload and each other.
- [ ] Each selected harness's instruction file carries the managed `markdown-frontmatter` block, and no frontmatter was added to those files.
