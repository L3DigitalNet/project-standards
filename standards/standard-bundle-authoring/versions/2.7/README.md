...
This standard defines the machine-checkable contract for authoring versioned standard packages. A package separates stable family identity, complete immutable payloads, and catalog-major channel policy. The authoritative system requirements are [SPEC-BA02](../../../../docs/specs/2026-07-10-standard-bundle-authoring-v2-spec.md); this document is the normative authoring guide for payload version `2.7`.
...
## Consumer runtime and artifact modes

Every payload's consumer-side Python requires Python 3.14 or newer. This includes Python hooks and providers executed from a consumer repository; the authoring contract has no syntax-backport or lower-runtime lane.

A whole artifact's declared artifact mode is the consumer contract; source-tree executable bits are not the consumer contract and may differ from the declared mode. Reconciliation materializes the declared mode, so validation of consumer behavior uses that declaration rather than inferring a mode from the payload source tree.

## Resources and outputs

Every resource is declared with an ID, role, safe relative path, media type, and raw-byte digest. Other regular payload files are declared as an artifact source or static-contribution source and carry the digest fields defined by that declaration. A payload path keeps exactly one inventory entry: it may appear in more than one declaration — the shape a packaged skill needs to install byte-identical copies at both `.agents/skills/` and `.claude/skills/` — only when every declaration for that path pins the same digest, and disagreeing digests for one path still fail. `payload.toml` itself may never be named as its own resource or artifact-source declaration. Symlinks, undeclared files, absolute paths, traversal, and case-fold collisions are invalid.
...
### Governing option pointers

A contribution may declare `governing_options`: the closed list of package options whose values feed its rendered unit. Declare exactly the options that can change the rendered value — a bare name for a top-level option, an absolute multi-segment pointer for a nested one. Declare an empty list for a fixed unit that no option can change; consumer-conflict findings then state that no declared option governs the unit. Omit the field only when governance is genuinely undeclared, and never claim an option that cannot produce the unit's value — the contract validates that every pointer resolves against the declared option schema, but only the author can keep the claim truthful. Gating `when_any` predicates are not governance declarations; they decide whether a unit materializes, not which option renders its value.
...
An automatic direct package-to-package migration may declare a `configuration_transform` array of canonical JSON pointers when its effects include `config:*`. The field is optional and absent by default. When present, it is nonempty, sorted, unique, and limited to disjoint shared scalar leaves. Each pointer traverses shared directly declared object properties with identical requiredness for the traversed token. Its leaf is directly declared and scalar, uses no reference or combinator, retains the same validation keywords except default, annotations, and a target enum superset, and grants no authority over siblings or aggregate ancestor constraints. The migration must target the containing payload and reuse its migrate provider. The control plane validates provider identity, explicit consumer-value preservation, changed-pointer bounds, source and target schemas, and idempotence before planning the consumer config action.

Legacy signatures recognize closed historical shapes through `known_content_digests`. Raw signatures enumerate every byte-version each supported release actually shipped, not only the current resource bytes. A whole-file YAML or TOML signature may instead set `format` and enumerate the canonical semantic digest of each supported historical form; formatting-only variants then share a digest. Semantic recognition is still a closed allowlist: it does not authorize arbitrary customization, and every meaningful action, reference, input, job, key, or value change needs its own reviewed historical form. Unknown bounded blocks and every ownership-acquiring, locking, or destructive transition block automatic migration. A single-target whole-file signature may additionally declare `consumer_owned_intent_pointer`; this authorizes only the FR-037 consumer-owned preserve path and never adds observed bytes to package history. Raw migration input must explicitly supply the literal `consumer-owned` value through that pointer. Marker presence alone never proves ownership.

A whole-file signature whose target stays managed only through bounded units may instead declare `unknown_content_disposition = "preserve"` (mutually exclusive with `consumer_owned_intent_pointer`): the migration preserves unrecognized bytes at that target with a `CP-MIGRATION-BOUNDED-TAKEOVER` warning when the provider claims `preserve` and no whole-file declaration materializes there, and steady-state reconciliation takes over only the bounded package-owned units inside the preserved file. All other unknown or modified legacy bytes remain consumer-owned and block automatic migration. See the [`migration.toml`](templates/migration.toml) and [`legacy-signature.toml`](templates/legacy-signature.toml) templates.
...
`standard-bundle-authoring@2.7` demonstrates this contract as an `internal` payload. It declares a closed empty option schema, canonical documentation, and all author templates. It intentionally declares no adoption guide, consumer artifact, semantic contribution, referenced extension, migration, legacy signature, or executable provider.
