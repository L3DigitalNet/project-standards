...
- Every payload's consumer-side Python requires Python 3.14 or newer; there is no lower-runtime compatibility lane.
- A declared artifact mode is the consumer contract; source-tree executable bits are not the consumer contract.
...
- Every regular payload file is declared and digested; symlinks, undeclared files, and `payload.toml` self-declaration are invalid. A path may repeat across declarations only when every declaration pins the same digest — one inventory entry results; disagreeing digests for one path still fail.
...
- Extensions remain consumer-owned. Migrations use exact package or registered legacy endpoints and enumerate all effects and recognized signatures. A direct automatic package upgrade may opt into a bounded `configuration_transform` allowlist whose pointers follow shared direct-object properties with identical traversed-token requiredness to direct scalar leaves with matching validation profiles except defaults, annotations, and target enum supersets.
- Released payload bytes are immutable; corrections use a new package version.

This `2.7` payload is internal and ships authoring templates only. It has no consumer outputs or executable providers.
