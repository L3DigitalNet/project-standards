---
schema_version: '1.1'
id: 'reference-hbuqgd-standard-bundle-authoring'
...
The "standard for standards": the contract every standard bundle under `standards/{id}/` must declare so the repository can be discovered and composed mechanically — by a future standards graph, and eventually by an MCP server — without hardcoding each standard.

## Purpose & status

This standard closes a gap: the repository defines standards as bundles under `standards/{id}/`, but nothing said what a bundle must declare, and the seven current bundles diverge (five ship machine bundles and `registry.json` entries, `project-spec` is CLI-enforced with no copy-adopt bundle, `python-coding` is an unregistered draft). This document is the single, machine-checkable authoring contract that closes it. It realizes [adr-0001](../../docs/adr/adr-0001-standard-bundle-authoring-contract.md) and is the `SPEC-MT01` Step 02 deliverable.
...
First-release scope is the **written contract plus one worked example** (this standard's own `standard.toml`) and a blank [template](templates/standard.toml). The machine schema, typed model, fixtures, and graph-validation gate that enforce the contract are deferred to `SPEC-MT01` Steps 03–04 — the contract is settled in prose before it is mechanized.
...
| --- | --- | --- |
| `README.md` | **Required** | The canonical standard: the human- and agent-readable contract, with canonical frontmatter. |
...
| _non-adoptable marker_ | Required _if not_ | A standard with `adoption = "none"`, or an unreleased draft, carries an explicit non-adoptable note **instead of** `adopt.md`. |
...
adopt = "adopt.md"            # required for adoptable standards; omit when adoption = "none"
...
The mode is descriptive metadata, not schema-binding until Step 03. `cli` names how `project-spec` is enforced today; broaden it (e.g. to `package-tooling`) only via a spec revision if a second CLI-enforced standard appears ([SPEC-BA01 OQ-001](../../docs/superpowers/specs/2026-07-07-standard-bundle-authoring-standard.md)).
...
| --- | --- | --- |
| `operation` | The generic operation: `validate`, `fix`, `drift-check`, `id-next`, `extract`, … | Yes |
...
- `template` → `templates/standard.toml`, and any bundle-specific IDs under `resources/`.
...
The `standard.toml` manifest describes the standard; the **artifact plane** (`adopt.toml`) describes what a copy-adopt materializes. They stay separate ([adr-0003](../../docs/adr/adr-0003-separate-standard-and-artifact-manifests.md)). Each `standard.toml` therefore either:

- **references its `adopt.toml`** artifact manifest (adoptable standards), or
- **explicitly declares non-adoptability** (`adoption = "none"`, no `adopt` resource).

Artifact ownership, shared artifacts (`_shared`), and destination-collision semantics remain delegated to the artifact plane. This standard does not re-invent the adopt engine; Step 04 graph validation reasons about artifact ownership through the `adopt.toml` link, not by re-deriving it.
...
Until the Step 03 schema and graph validator exist, "conforms to the contract" is decided by this checklist. A `standard.toml` conforms when **every required field is present and well-formed**:
...
- [ ] `[resources]` — `readme` present; `adopt` present **iff** the standard is adoptable.
...
Step 03 must preserve this checklist's field set or record a deliberate supersession.
