...
The current consumer package is [`python-tooling@1.9`](versions/1.9/adopt.md). Use it for the uv/uv_build `src/` baseline, Ruff, BasedPyright strict mode, pytest/coverage, pip-audit, CI, VS Code, and bounded agent instructions.
...
Unsupported option combinations, conflicting TOML ownership, unsafe paths, or managed-unit drift fail closed. See the [version-specific guide](versions/1.9/adopt.md) for the complete option example, outputs, migration, disable semantics, and recovery guidance.
