...
def _build_system(config: Mapping[str, object]) -> str:
    ...
    if backend == "none":
        return ""
    choices = {
        # Issue #182: the `uv_build` bound spans the whole pre-1.0 series rather
        # than one minor. uv only takes the in-process fast path for its own
        # backend when the running uv satisfies this requirement, and warns
        # `... does not contain the current uv version X` otherwise — so a
        # single-minor window (1.15 shipped `<0.12`) makes every managed adoption
        # noisy the day uv's next minor lands. This payload's bytes are immutable,
        # so it cannot chase uv's release train; the bound has to outlive it.
        #
        # Dropping the upper bound instead is rejected: uv warns about that too
        # (`is missing an upper bound on the uv_build version`), because an
        # unbounded sdist breaks on the first breaking backend release. Deriving
        # the bound from the uv installed at reconcile time is also rejected —
        # rendered bytes must be a machine-independent fixed point, or every
        # consumer with a different uv would report drift against the lock.
        "uv_build": (["uv_build>=0.11,<1.0"], "uv_build"),
...
def _dependencies(config: Mapping[str, object]) -> str:
    ...
    # Checked here rather than through `_string_list`, which every sibling list
    # option uses: that helper rejects `<` and `>`, and those are ordinary
    # characters in a PEP 508 requirement (`pytest-asyncio>=1.0,<2`). Without an
    # element check of some kind, a non-string entry reaches `_dependency_name`'s
    # `re.split` and escapes as a raw TypeError, outside the bounded ValueError
    # contract every other option honors. The option schema already pins
    # `items.type = "string"`, so this only catches a caller that invokes the
    # provider with an unresolved config.
    if any(not isinstance(item, str) or not item for item in cast("Sequence[object]", additional)):
        raise ValueError("config.additional_dev_dependencies entries must be nonempty strings")
...
# Issue #181: `E501` is ignored by DEFAULT, not unconditionally. `ruff format`
# rewraps code to `line-length` but never reflows a comment, docstring, or string,
# so leaving E501 selected would fail every consumer's gate on prose the formatter
# itself refuses to fix. Ruff resolves `ignore` after `extend-select`, so a
# consumer could not re-enable the rule from `ruff.extend_select` — the exclusion
# was unreachable from consumer configuration, which is what made an option the
# only possible answer rather than a convenience.
_LINT_IGNORE = ["E501"]
...
def _lint_ignore(ruff: Mapping[str, object]) -> list[str]:
    """Return the owned `lint.ignore` list for the declared line-length intent.

    Opting in renders an empty `ignore`, which is the only spelling that enforces
    the limit: the rule is already selected through the `E` prefix in
    `_LINT_SELECT`, so the exclusion is the whole mechanism. The default renders
    the predecessor's `["E501"]`, which is what keeps every 1.14 consumer's
    `[tool.ruff.lint]` unit byte-identical across the upgrade.
    """
    enforce = ruff.get("enforce_line_length", False)
    if not isinstance(enforce, bool):
        raise ValueError("config.ruff.enforce_line_length must be a boolean")
    return [] if enforce else list(_LINT_IGNORE)
...
# CROSS-FILE CONTRACT: this alphabet is the same one `config.schema.json` spells
# as the `runner_labels` items `pattern`, and both must move together. The schema
# is what a consumer's configuration is validated against; this check is what
# keeps the rendered YAML exact, because no label admitted here can carry a quote,
# a backslash, or whitespace that plain YAML scalars would have to escape.
_RUNNER_LABEL = re.compile(r"[A-Za-z0-9][A-Za-z0-9._-]*")


def _runner_labels(config: Mapping[str, object]) -> list[str]:
    """Return the validated runner-label selection; an absent option selects none.

    Absent rather than empty is tolerated so a configuration resolved against a
    predecessor's schema still renders here instead of failing as a malformed
    array; the two cases mean the same thing to every caller below.
    """
    if config.get("runner_labels") is None:
        return []
    labels = _string_list(config, "runner_labels", option="config.runner_labels")
    if any(_RUNNER_LABEL.fullmatch(label) is None for label in labels):
        raise ValueError("config.runner_labels entries must be safe runner labels")
    return labels


def _runs_on_lines(config: Mapping[str, object]) -> list[str]:
    """Spell the managed job's runner selection.

    Issue #180: this package's workflow is self-contained — it declares its own
    job and fires on `push` and `pull_request` — so there is no `workflow_call`
    boundary to route the selection through. The labels template `runs-on`
    directly, unlike the three sibling packages of issue #132 whose managed
    callers must smuggle the same option across a reusable-workflow input as a
    JSON-array string. Do not add a caller indirection here to match them: it
    would buy no reachability and would rewrite every consumer's managed bytes.

    An empty selection — the default — emits the literal predecessor line, so a
    1.14 consumer who configures nothing re-renders byte-identical bytes and owes
    no migration edge.

    Rejected alternative: a YAML flow sequence (`runs-on: ["self-hosted", ...]`,
    the shape the sibling callers' JSON string decodes to). Prettier owns `*.yml`
    in any repository that also runs Markdown Tooling, and it explodes a flow
    sequence across lines once the line exceeds `printWidth` — so the render would
    be a fixed point only until a consumer declared one label too many, and then
    `prettier --write` would turn the managed file into CP-MODIFIED-MANAGED drift.
    That is issue #115's defect in a new place. A block sequence is Prettier's own
    output at every label count and width, verified against Prettier 3.
    """
    labels = _runner_labels(config)
    if not labels:
        return ["    runs-on: ubuntu-latest"]
    return ["    runs-on:", *(f"      - {label}" for label in labels)]
...
_DEFAULT_CONFIG: dict[str, object] = {
    ...
    # Kept in lockstep with config.schema.json: `_whole_content` and
    # `run_render_semantic` return the immutable static resource only on an exact
    # equality match against this mapping, so a schema default missing here would
    # send every default adoption down the rendered branch and stop proving that
    # the rendered bytes and the shipped resource agree.
    "runner_labels": [],
...
# Naming build_backend = "none" as an escape here would be wrong from 1.18 on:
# that option omits [build-system], never [project], and uv refuses to lock or
# run against a pyproject.toml with no [project] table regardless of backend
# (issue #204).
_PROJECT_METADATA_HINT = (
    ...
    'a deliberately non-installable repository declares it too and sets build_backend = "none" '
    "to omit only [build-system]"
)
...
def run_validate(
    ...
    """Refuse an adoption the consumer's own pyproject.toml cannot support.
    ...
    the table. Issue #204: from 1.18 on, both are reported for a deliberately
    non-installable repository too. `build_backend = "none"` suppresses the
    rendered `[build-system]` table, not `uv lock` - that step and every `uv run`
    in the rendered gate stay in the documented flow, and uv refuses a
    `pyproject.toml` with no `[project]` table whatever backend is selected.
    """
    config = _config(request)
    snapshots = _table(request.get("snapshots"), name="snapshots")
...
def _metadata_findings(
    ...
    """Refuse any adoption whose pyproject.toml uv cannot resolve.

    The gate is uv-resolvability, not installability. `uv lock` and every
    `uv run` in the rendered gate reject a `pyproject.toml` with no `[project]`
    table outright ("No 'project' table found"), so a repository without one
    cannot run this package's own verification commands whatever backend it
    selected. Through 1.17 this short-circuited to no findings when
    `build_backend = "none"` was selected, which only deferred the same failure
    from `reconcile --check` to the first `uv lock` (issue #204). A
    non-installable repository still declares `[project]`; what it omits is
    `[build-system]`, which uv reads as `source = { virtual = "." }`.
    """
...
def _metadata_finding(observation: str) -> dict[str, object]:
    return {
        ...
        "message": (
            f"uv requires a consumer-owned [project] table to lock and run, and {observation}"
        ),
