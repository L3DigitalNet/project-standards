# Python Tooling 1.18 Agent Summary
...
- Use `build_backend = "none"` only for deliberately non-installable repositories; it omits `[build-system]` without removing development tooling. It does not waive the `[project]` table — from 1.18 on, `PT-PROJECT-METADATA` blocks every backend selection when `[project]` is absent, because uv cannot lock or run without it.
- Write an undeclared key such as `pythonpath` or `asyncio_mode` directly into `[tool.pytest.ini_options]`; only `minversion`, `testpaths`, `addopts`, and `markers` are package-owned there. Install the key's plugin through `additional_dev_dependencies` first, or the owned `--strict-config` turns it into a pytest config error.
