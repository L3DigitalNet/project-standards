...
`build_backend = "none"` removes `[build-system]`, not `[project]`. Every adoption still requires a consumer-owned `[project]` table, because the required `uv lock` step and every `uv run` command in the rendered gate refuse a `pyproject.toml` without one; uv classifies a `[project]` table with no `[build-system]` as `source = { virtual = "." }`, which locks and runs without building anything. Package `1.18` therefore evaluates `PT-PROJECT-METADATA` for every adoption (issue [#204](https://github.com/L3DigitalNet/project-standards/issues/204)); through 1.17 the check short-circuited to no findings whenever `build_backend = "none"` was selected, and 1.17's own adoption guide contradicted itself about whether the table was needed.

This tightens behavior for an existing `build_backend = "none"` consumer that has no `[project]` table. Such a repository was already failing at `uv lock`, so 1.18 does not break a working adoption — it surfaces that existing failure at `reconcile --check` instead of at the first gate run. Declare the PEP 621 metadata shown in the [adoption guide](adopt.md) and leave `[build-system]` absent.
...
The rendered CI workflow SHA-pins `astral-sh/setup-uv`, which publishes no moving major or minor tag from v8.0.0 on. Package `1.17` advanced that pin to `v10.0.1` (issue #201) and `1.18` holds it there. Both cache keys stay explicit rather than inherited because each has survived a default flip: v9 stopped pruning before saving the cache, and v10 makes `enable-cache: auto` disable the cache outright for `release`, tag pushes, `pull_request_target`, and `workflow_run`. Naming `enable-cache: true` holds the behavior the gate was measured under, and none of those four events triggers this workflow in any case.

## Migration

The automatic V4 migration recognizes the legacy `python_tooling.version`, Python Tooling option values, and byte-identical files shipped by the V1 copy-adopt bundle. It preserves the consumer contract selector independently from the selected 1.18 package payload; both supported contract values render the same toolchain because the selector remains metadata-only. Known whole-file agent and VS Code content is retired into bounded contributions; shared EditorConfig and extension files are preserved while their package-owned units enter the central lock.

Fresh 1.18 adoption defaults `ci.performance` to false.
...
Payload 1.18 becomes immutable after publication. Behavioral or option changes require a new package version, new payload digest, catalog entry, migration edge where necessary, and source plus extracted-wheel compatibility evidence.
