...
def _source_roots(config: Mapping[str, object]) -> tuple[list[str], list[str]]:
    ...
    # Declared roots keep their declared order after the layout-derived roots so
    # the checker include and coverage source stay in documented lockstep.
    for root in cast("Sequence[str]", additional):
