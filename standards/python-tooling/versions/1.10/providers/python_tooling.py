...
def _ruff_table(config: Mapping[str, object]) -> str:
    ...
    if extend_include:
        root += f"extend-include = {json.dumps(extend_include)}\n"
    lint = (
        "\n"
        "[tool.ruff.lint]\n"
    ...
    if extend_select:
        lint += f"extend-select = {json.dumps(extend_select)}\n"
    if extend_ignore:
        lint += f"extend-ignore = {json.dumps(extend_ignore)}\n"
    return (
        root + lint + "\n"
