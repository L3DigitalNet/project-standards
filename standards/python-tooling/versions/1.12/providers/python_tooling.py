...
def _ruff_values(config: Mapping[str, object]) -> dict[tuple[str, ...], object]:
    ...
    them, and keeping `per-file-ignores` a table leaves a future version free to
    derive its glob from the declared collection roots instead of freezing the
    literal into a payload scope.
    ...
    is inert in Ruff, and this is the whole rendered-surface delta of the cut.
    """
    ...
    return {
        ...
        ("lint", "per-file-ignores"): _PER_FILE_IGNORES,
        ("format",): _FORMAT,
    }
