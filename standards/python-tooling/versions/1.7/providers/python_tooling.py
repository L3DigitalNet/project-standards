...
def _source_roots(config: Mapping[str, object]) -> tuple[list[str], list[str]]:
    ...
    if layout == "src":
        include, sources = ["src", "tests"], ["src"]
    elif layout == "flat":
        include, sources = [".", "tests"], ["."]
    else:
        raise ValueError("config.source_layout is unsupported")
    # Declared roots keep their declared order after the layout-derived roots.
    # Every declared root joins the checker include (and Ruff src); only
    # coverage-scoped roots join coverage.run.source, so a strictly-typed but
    # unmeasured tooling root cannot drag down the coverage gate.
