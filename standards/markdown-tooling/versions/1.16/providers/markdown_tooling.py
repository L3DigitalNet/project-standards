...
def _pathspec_words(selection: Sequence[str]) -> str:
    return " ".join(_shell_word(_pathspec(value)) for value in selection)


def _tracked_files(selection: Sequence[str]) -> str:
    """Render the plain `git ls-files` half of the local markdownlint recipe."""
    return f"git ls-files -z -- {_pathspec_words(selection)}"


def _tracked_regular_files(selection: Sequence[str]) -> str:
    """Render a `git ls-files` half that drops symlinks from the corpus.

    Issue #209: Prettier refuses any *explicitly named* path that is a symbolic
    link, emitting `Explicitly specified pattern "..." is a symbolic link` and a
    non-zero status, while still reporting every real file clean. A repository
    that tracks symlinks matching the declared globs therefore fails its own
    documented gate on a fully clean tree — this repository's generated payload
    projection tracks thousands, so the gate exited 123 with nothing wrong.
    Those paths were never actually checked, so removing them narrows nothing:
    the set of files Prettier reads is identical.

    `git ls-files -s` prints `<mode> <object> <stage>\\t<path>`, so mode `120000`
    identifies a symlink from the index alone — no filesystem stat, and no
    dependence on the link resolving. The sed program drops those records and
    then strips the metadata prefix up to the first tab, which is exactly where
    Git ends it. Filtering here rather than excluding the generated directory
    wholesale keeps the exclusion tied to the actual defect: a consumer that
    tracks symlinked Markdown anywhere gets the same repair without naming its
    layout, and no regular file is ever dropped.

    `sed -z` and its `\\t` inside a bracket expression are GNU forms, matching
    the `sed -z`/`xargs -0` already required by the markdownlint recipe. Unlike a
    `grep -v` filter this cannot exit non-zero, so the pipeline stays safe under
    `set -o pipefail` even when every matched path is a symlink.
    """
    drop_symlinks = r"sed -zn '/^120000 /!s/^[^\t]*\t//p'"
    return f"git ls-files -s -z -- {_pathspec_words(selection)} | {drop_symlinks}"
...
def _local_format_commands(config: Mapping[str, object]) -> list[str]:
    ...
    Symlinks are filtered out of the Git-routed corpus (`_tracked_regular_files`,
    issue #209) because naming one explicitly is a Prettier error rather than a
    skip. The fallback form does not need the filter: Prettier expands its own
    globs, and a symlink reached by expansion is skipped silently.
    ...
    return [
        f"{_tracked_regular_files(_format_local_selection(config))}"
...
def _runner_label_reachability_warning(
    request: Mapping[str, object],
    config: Mapping[str, object],
    *,
    tool: str,
) -> dict[str, str] | None:
    # Absent and empty must behave alike here, exactly as they do in
    # `_runner_labels_lines`. The option schema declares no default and does not
    # require the key, so a consumer config that never mentions `runner_labels`
    # is legal; feeding `None` straight to `_sequence` raised ValueError and
    # turned every `verify` and drift-check for such a consumer into a provider
    # error instead of findings, while the render path accepted the same config.
    raw = config.get("runner_labels")
    if raw is None:
        return None
    labels = _sequence(raw, name="config.runner_labels")
    if not labels:
        return None

    workflow_name = "lint-markdown" if tool == "lint" else "format"
    workflow_path = f".github/workflows/{workflow_name}.yml"
    ownership_option = f"{tool}_workflow_ownership"
    version = request.get("version")
    if config.get("workflow_mode") == "self-hosted":
        reason = (
            f"Markdown Tooling {version} runner_labels are unreachable from the "
            f"direct self-hosted {workflow_name} workflow"
        )
        hint = (
            f'set workflow_mode = "caller" with {ownership_option} = "managed", or keep a '
            "consumer-owned workflow and pin its runs-on selection directly"
        )
    elif config.get(ownership_option) == "consumer-owned":
        reason = (
            f"Markdown Tooling {version} runner_labels are unreachable because "
            f"{workflow_path} is consumer-owned"
        )
        hint = (
            f"pass runner-labels from the consumer-owned {workflow_name} caller, or set "
            f'{ownership_option} = "managed"'
        )
    else:
        return None

    return {
        "code": "MT-RUNNER-LABELS-UNREACHABLE",
        "severity": "warning",
        "path": workflow_path,
        "identity": "$file",
        "message": reason,
...
def _verify(
    ...
    *,
    ...
    reachability = _runner_label_reachability_warning(request, config, tool=tool)
    if reachability is not None:
        findings.append(reachability)
