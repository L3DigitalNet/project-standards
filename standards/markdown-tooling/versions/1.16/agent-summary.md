# Markdown Tooling 1.16: Agent Summary
...
Reconciliation emits one non-fatal warning per enabled caller when non-empty labels are inert. A consumer-owned caller must pass `runner-labels` itself or return to managed ownership. Direct `self-hosted` mode must move to a managed caller path or keep the workflow consumer-owned and pin `runs-on` directly. Empty labels, managed caller mode, and disabled tools stay silent; managed-byte drift remains an error.
