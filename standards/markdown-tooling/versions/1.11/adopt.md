...
- `contract_version`: the independent Markdown Tooling contract selector; package 1.11 supports `1.1`.
...
A `.markdownlint.json` that already equals the rule set this package currently ships is likewise accepted as known legacy content. Package 1.11 adds the shipped bytes' own digest to the `legacy-markdownlint-config` signature, so a consumer whose local deviation upstream has since adopted as the default no longer has to regress the file to a superseded rule value to get past migration.
...
Neither local invocation honors `.git/info/exclude`. The globs above only respect tracked `.gitignore`/`.prettierignore` patterns, plus the `--ignore-path .gitignore` the format caller passes to Prettier; a path excluded solely through the repository-local `.git/info/exclude` file is invisible to that mechanism and still enters the scan. Cover local scratch or evidence content excluded only there by adding it as an explicit negative glob to the local invocation, or as a typed `exclusions` record when it belongs in the reconciled scope — `lint_generated_exclusions` and `gitignore`-style options do not reach it.
...
Before accepting any `prettier --write` or `markdownlint --fix` output, diff it and confirm no literal character inside prose or a code span changed meaning. If a diff shows an unpaired underscore/asterisk rewrite or a change inside backticks, back the run out and instead backtick the bare identifier in the source (for example `` `America/New_York` ``) so no unpaired underscore remains for the formatter to reinterpret.
