...
- `contract_version`: the independent Markdown Tooling contract selector; package 1.13 supports `1.1`.
...
Package 1.13 bounds the documented local Prettier check to the declared corpus and renders it into the two managed instruction blocks, so updating to it rewrites the `markdown-tooling` block in `AGENTS.md` and `CLAUDE.md`. No option, managed config, or workflow byte changes; the corpus is still exactly `markdown_globs` plus `config_globs`.
...
npx markdownlint-cli2 "**/*.md" "!.pytest_cache/**" "!.ruff_cache/**" "!.venv/**" "!node_modules/**"
```

Run a local tool only when its matching `lint` or `format` option is `true`:

- For markdownlint, pass every selected `markdown_globs` value, then the generated-directory exclusions unless `lint_generated_exclusions` is `false`, then each exclusion whose `applies_to` value is `lint` or `both`, every one prefixed with `!`. The command above is that rendering for the defaults. Order matters: `markdownlint-cli2` resolves globs in sequence, so the negations have to trail the positive globs.
- For Prettier, select the Git-tracked files matching every selected `markdown_globs` and `config_globs` value, as the command above does, and supply each exclusion whose `applies_to` value is `format` or `both` through an additional `--ignore-path` file. `:(glob)` pathspec magic is required: under Git's default magic, `**/*.md` skips root-level files and `nested/**/*.md` matches nothing, while `:(glob)` gives `**/` the "zero or more leading directories" reading Prettier uses. Without Git, use `npx prettier --check --no-error-on-unmatched-pattern -- '**/*.md' '**/*.json' '**/*.jsonc' '**/*.yml' '**/*.yaml'` instead; the flag is mandatory because Prettier exits `2` on any pattern that matches nothing, and most repositories have no JSONC file. The managed instruction blocks render both forms from the selected globs, so the block is the current authority for a specific repository.

Never run either tool over a bare `.`. That form reaches every language Prettier supports rather than the declared corpus, and it reaches Git-excluded scratch, where an intentionally invalid test artifact turns a formatting check into a hard error.

`xargs` maps any child exit status between `1` and `125` onto `123`, so the Git-routed form reports clean as `0` and everything else as `123` rather than reproducing Prettier's `1` for findings and `2` for errors. Read the output to tell them apart, or use the fallback form, which preserves Prettier's status.
...
Neither tool reads `.git/info/exclude`, and the markdownlint invocation does not read `.gitignore` either. `markdownlint-cli2` reads Git ignore files only when a `.markdownlint-cli2.*` runner config sets `gitignore: true`; this package ships the rule set `.markdownlint.json`, not a runner config, and the option has no CLI or action equivalent, so its bare globs select ignored trees — cover those with explicit negative globs or a typed `exclusions` record. Prettier reads `.gitignore` and `.prettierignore`, but only the copies in the working directory: a nested `.gitignore` is never consulted. Routing the Prettier check through `git ls-files` closes both gaps at once, because Git honors every ignore file at every level plus `.git/info/exclude`, and lists nothing belonging to an independent nested repository. Do not try to reach `.git/info/exclude` with `--ignore-path`; Prettier anchors an ignore file's patterns to that file's own directory, so every pattern would resolve against `.git/info/`.
