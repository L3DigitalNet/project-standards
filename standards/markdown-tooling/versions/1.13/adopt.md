...
- For Prettier, select the Git-tracked files matching every selected `markdown_globs` and `config_globs` value, as the command above does, and supply each exclusion whose `applies_to` value is `format` or `both` through an additional `--ignore-path` file. Without Git, use `npx prettier --check --no-error-on-unmatched-pattern -- '**/*.md' '**/*.json' '**/*.jsonc' '**/*.yml' '**/*.yaml'` instead.
