...
- `contract_version`: the independent Markdown Tooling contract selector; package 1.4 supports `1.1`.
...
| V4 artifact is modified | A modified `.editorconfig` or `.vscode/extensions.json` is preserved automatically with a `CP-MIGRATION-BOUNDED-TAKEOVER` warning; a modified config or caller workflow blocks until its known content is restored. |
