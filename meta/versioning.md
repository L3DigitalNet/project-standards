...
- The **Python Tooling contract version** is the closed `contract_version` option inside the selected Python Tooling payload. It preserves supported toolchain-policy semantics independently from package release `1.9`.
- The **Markdown Tooling contract version** is the closed `contract_version` option inside the selected Markdown Tooling payload. The package options and managed lint/format workflows enforce its behavior independently from package release `1.9`.
- The **Agent Handoff contract version** is the closed `contract_version` option inside the selected Agent Handoff payload. The provider-backed validators and central lock enforce its layout/integration policy independently from package release `1.5`.
