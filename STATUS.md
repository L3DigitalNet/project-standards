...
- **project-spec tooling Spec #1 implemented (2026-07-05, `2a6c4c0`):** new `src/project_standards/specs/` registry/document/config core plus read-only `project-standards spec validate|lint|extract|next`, frozen JSON/exit-code coverage, reusable `validate-specs.yml`, bundled template wheel smoke test, and maintainer pytest replacement for `check_specs.py`.
...
- [2026-07-05] project-spec tooling Spec #1 implemented and verified green (651 tests, 98% coverage); `new`/`upgrade` remain deferred to Spec #2.
