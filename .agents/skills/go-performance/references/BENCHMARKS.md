...
- Use `b.Loop()` on Go 1.24+; use `for i := 0; i < b.N; i++` only when maintaining older Go versions
- Assign results to a variable (or `_`) to prevent the compiler from optimizing away the call
...
The `-benchmem` flag reports allocations per operation. The `-count` flag runs each benchmark N times for statistical significance.
...
- If benchstat shows `~` (tilde), the difference is not statistically significant
