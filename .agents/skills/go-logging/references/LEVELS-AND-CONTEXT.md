...
**When to use**: Internal state transitions, cache behavior, detailed request/response data during development.
...
**When to use**: Startup/shutdown, configuration changes, significant business events, periodic health summaries.
...
**When to use**: Retries that eventually succeeded, deprecated code paths, approaching resource limits, fallback behavior.
...
**When to use**: Failed operations that affect users, lost connections, data integrity issues, external service failures that weren't recovered.
...
Use `HandlerOptions.Level` with a `slog.LevelVar` to control the minimum level at runtime.
