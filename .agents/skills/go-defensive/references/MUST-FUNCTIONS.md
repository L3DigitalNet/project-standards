...
These are safe because they run at package init time — if they fail, the program cannot function correctly.
...
- **Package-level `var`**: Compiling regexps, parsing templates, loading required config
- **`init()` or early `main()`**: Setting up resources that must exist for the program to run
- **Test helpers**: `t.Fatal` is preferred in tests, but Must can be acceptable for test fixtures
...
- **Name**: `Must` prefix + the fallible function name (e.g., `MustParse`, `MustNew`, `MustCompile`)
