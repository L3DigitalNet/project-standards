...
> **Advisory**: Use `~` when you want to accept named types built on a primitive.
...
Use `~` when callers are likely to define named types over the base type. Omit `~` only when you need to restrict to the exact built-in type.
...
> **Normative**: Use the minimal constraint that supports the operations you perform.
...
Over-constraining limits reuse and forces callers to work around restrictions that the implementation never needed.
...
Supply type arguments explicitly only when there are no function arguments to infer from, the inferred type is wrong (e.g., untyped constant promotes to the wrong type), or readability benefits from making the type visible.
...
> **Normative**: From Google Style Guide — prefer interfaces when types share a useful unifying interface.
...
Generics justify complexity when they eliminate duplication across **multiple call sites**. If only one type is ever used, start concrete.
