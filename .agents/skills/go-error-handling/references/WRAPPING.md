...
- Create fresh, independent errors (especially at system boundaries like RPC/IPC)
...
- **`slog.Warn`**: For issues that may need attention but aren't immediately actionable.
- **`slog.Debug`**: For development tracing — only emitted when the handler's level is set to `LevelDebug`.
