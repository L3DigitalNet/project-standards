...
Prefer `%q` in output intended for humans where the value could be empty or contain control characters.
...
`GoString()` output should be valid Go syntax or close to it — it's meant for debugging, not user-facing display.
