...
_FRONTMATTER_RE: re.Pattern[str] = re.compile(r"\A---\r?\n(.*?)\r?\n---(?:\r?\n|$)", re.DOTALL)
...
def find_bundled_schema(name: str) -> Path:
    ...
    """
    filename: str = f"{name}.schema.json"
...
def resolve_schema_path(schema_value: str | None) -> Path:
    ...
    """
    name: str = schema_value or _DEFAULT_SCHEMA_NAME
...
def _coerce_dates(obj: Any) -> Any:
    ...
    if isinstance(obj, dict):
        mapping: dict[str, Any] = cast(dict[str, Any], obj)
        return {k: _coerce_dates(v) for k, v in mapping.items()}
    if isinstance(obj, list):
        sequence: list[Any] = cast(list[Any], obj)
...
def parse_frontmatter(text: str) -> dict[str, Any] | None:
    ...
    match: re.Match[str] | None = _FRONTMATTER_RE.match(text)
    if not match:
        return None
...
def validate_file(
    ...
    *,
    ...
    for error in sorted(validator.iter_errors(meta), key=lambda e: list(e.path)):  # pyright: ignore[reportUnknownMemberType]
        field: str = ".".join(str(p) for p in error.path) or "(root)"
...
def collect_paths(
    ...
    if explicit or glob_pattern:
        paths.update(p for p: Path in explicit if p.is_file())
        if glob_pattern:
            paths.update(p for p: Path in Path().glob(glob_pattern) if p.is_file())
    elif include_patterns:
        for pattern: str in include_patterns:
            paths.update(p for p: Path in Path().glob(pattern) if p.is_file())
    else:
        paths.update(p for p: Path in Path().glob("**/*.md") if p.is_file())

    excluded: set[Path] = set()
    for pattern: str in exclude_patterns:
...
def load_config(path: Path) -> FrontmatterConfig:
    ...
    if path.exists():
        ...
        if isinstance(raw, dict):
            ...
            markdown: Any | None = raw_dict.get("markdown")
            if isinstance(markdown, dict):
                markdown_dict: dict[str, Any] = cast("dict[str, Any]", markdown)
                frontmatter: Any | None = markdown_dict.get("frontmatter")
                if isinstance(frontmatter, dict):
                    fm: dict[str, Any] = cast("dict[str, Any]", frontmatter)
                    schema_val: Any | None = fm.get("schema")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config: FrontmatterConfig = load_config(args.config)

    schema_path: Any | Path = args.schema if args.schema is not None else resolve_schema_path(config.schema)
    try:
        schema: dict[str, Any] = json.loads(schema_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc: OSError | json.JSONDecodeError:
    ...
    except SchemaError as exc: SchemaError:
        print(f"error: invalid schema {schema_path}: {exc.message}", file=sys.stderr)
        return 2
    validator = Draft202012Validator(schema)

    require_frontmatter: bool = config.required and not args.no_require_frontmatter
    paths: list[Path] = collect_paths(list(args.files), args.glob, config.include, config.exclude)
