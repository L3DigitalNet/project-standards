...
- **Develop on `testing`, never on `main`, and admit every change — including leg-worktree cherry-picks — through the GitHub Workflow admission rule below**, not a direct push. Two exceptions: T0, and Agent Handoff documents (`docs/handoff/**`, `docs/STATUS.md`, `docs/TODO.md`), which are transient agent-side state and always commit directly. See `CLAUDE.md`'s Non-Negotiables for the release-branch merge and its PR exemption.
