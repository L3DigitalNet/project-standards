...
- Install or invoke the exact v5 release you intend to pin. For 5.27.0:

  ```bash
  uv tool install --force "git+https://github.com/L3DigitalNet/project-standards@v5.27.0"
  project-standards --version || project-standards --version
  ```

  Confirm that the command reports `project-standards 5.27.0` before continuing. The first `--version` probe immediately after a forced install can fail transiently while the freshly installed environment finishes import wiring; retry once before treating a failure as real.
