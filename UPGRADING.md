...
- Install or invoke the exact v5 release you intend to pin. For 5.29.0:
  ...
  Confirm that the command reports `project-standards 5.29.0` before continuing. The first `--version` probe immediately after a forced install can fail transiently while the freshly installed environment finishes import wiring; retry once before treating a failure as real.
...
**setup-uv v10 flips one default, and none of the three payloads is exposed to it.** Under `enable-cache: auto`, v10 now disables the cache for `release`, tag pushes, `pull_request_target`, and `workflow_run` events. All three rendered workflows set `enable-cache` explicitly — `true` for Python Tooling, a repository-scoped expression for the other two — so the caching behavior each gate was measured under is unchanged. If you own your workflow (`workflow_ownership = "consumer-owned"`) and wrote `enable-cache: auto` yourself, check whether any of those four events triggers it before you advance your own pin. No configuration option changed in any of the three packages.
...
### What the 5.28.0 defaults rewrite on refresh: Python Tooling and Markdown Tooling adoption fixes

**Python Tooling 1.17 → 1.18 corrects three adoption-friction defects**, each found by a fresh adoption that passed preview and then failed at `uv lock` or the pytest gate. No configuration option or schema changes; refreshing rewrites the adoption guide, the provider, and one finding's behavior.

`build_backend = "none"` no longer documents that a repository needs no `[project]` table — it does, because `uv lock` and every `uv run` in the rendered gate require one regardless of build backend (project-standards issue [#204](https://github.com/L3DigitalNet/project-standards/issues/204)). `adopt.md` is corrected, and `PT-PROJECT-METADATA` is now reported for that case on every adoption instead of being short-circuited. **This is a behavior tightening, not only a doc fix**: a repository adopted under the old text was already failing at `uv lock`, so the new finding surfaces a gate that was already red rather than introducing a new one. Add the minimal `[project]` table uv needs (`name`, `version`, `requires-python`; no `[build-system]`, so uv treats the project as virtual) if you have not already worked around this.

`source_layout = "explicit"` adoption guidance now states the import-path prerequisite for a subproject package (project-standards issue [#205](https://github.com/L3DigitalNet/project-standards/issues/205)): declaring `additional_source_roots` reaches the Ruff, basedpyright, and coverage scopes, but does not put the package on the root environment's import path. `pythonpath` under `[tool.pytest.ini_options]` is consumer-owned and one route; a uv workspace with the subproject as a member plus `additional_dev_dependencies` is the documented alternative.

An undeclared `[tool.pytest.ini_options]` leaf key, such as `asyncio_mode`, stays consumer-owned (project-standards issue [#206](https://github.com/L3DigitalNet/project-standards/issues/206)). The package owns exactly `minversion`, `testpaths`, `addopts`, and `markers` in that table and adds no pass-through option; install the plugin through `additional_dev_dependencies` before setting the key, or pytest refuses it under `--strict-config` with `ERROR: Unknown config option: asyncio_mode`. The provider also rejects a non-string `additional_dev_dependencies` entry with a bounded `ValueError` instead of a raw `TypeError`.

**Markdown Tooling 1.15 → 1.16 fixes the documented Prettier gate command and advances one action pin.** The rendered gate command in `AGENTS.md` exited 123 on a completely clean tree because `git ls-files` includes the generated payload projection's symlinks, and Prettier refuses a symlink passed as an explicitly specified pattern (project-standards issue [#209](https://github.com/L3DigitalNet/project-standards/issues/209)). The corrected command filters symlinks out of the corpus before invoking Prettier; the set of regular files checked is unchanged, and a deliberately misformatted tracked file still fails the gate. Separately, the pinned `DavidAnson/markdownlint-cli2-action` advances from `v24` (`6bf21b07…`) to `v24.2.0` (`21c1be1b93ad9ed58fa840aacc3f279cde2a72ff`; project-standards issue [#211](https://github.com/L3DigitalNet/project-standards/issues/211)), and the verify path now tolerates an absent `runner_labels` as the render path already did. Refreshing rewrites the managed gate block in `AGENTS.md` and `.github/workflows/lint-markdown.yml`'s pin on the first reconcile after the refresh; no configuration option changes in either fix.

### What the 5.28.0 defaults rewrite on refresh: GitHub Workflow 1.9 adds four admission classes and a check that is yours to run
