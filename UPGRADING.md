...
- Install or invoke the exact v5 release you intend to pin. For 5.3.0:

  ```bash
  uv tool install --force "git+https://github.com/L3DigitalNet/project-standards@v5.3.0"
  project-standards --version
  ```

  Confirm that the command reports `project-standards 5.3.0` before continuing.
...
Pin reusable workflows and the installed CLI to the same v5 release line. Use `@v5` for compatible updates or `@v5.3.0`/a commit SHA for an immutable pin. Never mix a v5 workflow with a v4 `standards-ref`.
