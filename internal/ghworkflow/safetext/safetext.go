// Package safetext holds the one encoder for untrusted GitHub text (titles, bodies,
// field values, finding messages, and raw API error bodies).
//
...
//
// This package is a leaf on purpose — it imports nothing from the tool — because every
// layer that touches untrusted text must be able to call it: render (where work items
// enter the model), ghapi (where a non-2xx response body becomes error text), cli (where
// the envelope is written), and audit (where live organization schema text is printed).
// Living in render, as it did through 1.9, put it below the API and CLI layers in the
// import graph and left those two surfaces unsanitized; render.SanitizeText remains as a
// one-line alias so the rendering call sites read unchanged.
//
// Sanitizing at more than one layer is deliberate and not redundant: the function is
// idempotent, and the envelope writer cannot know which of its findings came from a
// sanitized source, so it treats all of them as untrusted.
//
// This is not Markdown escaping. render.EscapeText owns the Prettier/markdownlint
// fidelity rule for table cells; the two compose, and neither substitutes for the other.
package safetext
