...
def test_dogfood_config_selects_current_cli_documentation_default() -> None:
    config = tomllib.loads(Path(".standards/config.toml").read_text(encoding="utf-8"))
    catalog = tomllib.loads(Path(".standards/catalog.toml").read_text(encoding="utf-8"))
    ...
    default = catalog["standards"]["cli-documentation"]["default"]
    assert lock["standards"]["cli-documentation"]["resolved"] == default
