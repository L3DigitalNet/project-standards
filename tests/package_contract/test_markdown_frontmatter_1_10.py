...
advance catalogs/5.toml. `runner_labels` reaches caller mode only, so 1.10
supersedes none of the self-hosted bytes this repository renders; those follow
the catalog default and are proven in the reconstruction suite.

1.10 is now a retained predecessor. The rows below that concern the current
authority read the version from catalogs/5.toml instead of naming it, so a later
cut moves them rather than breaking them.
"""
...
def test_markdown_frontmatter_1_10__catalog_role__selects_the_successor_as_default() -> None:
    """1.10 keeps an advertised, non-default row behind the cuts that followed it.

    The default moved on past 1.10; what a consumer holding an exact pin still needs is
    that the row survives every later cut. `retained` is the whole guarantee — a version
    that vanishes from the catalog breaks that pin.
    """
...
def test_markdown_frontmatter_1_10__mutable_navigation__names_the_current_authority() -> None:
    """Family-level readers must resolve the same current payload as the index.

    The version is read from the catalog rather than named, because this is not a claim
    about 1.10: the family root always points at the default, so hardcoding the number
    turns every later cut into a failure in a predecessor's proof. What 1.10 owns is the
    other half — once superseded, its own directory may no longer be advertised there.
    """
    ...
    for name in ("README.md", "adopt.md", "agent-summary.md"):
        ...
        assert f"versions/{default}/{name}" in content
