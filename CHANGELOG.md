...
- **Nine standard-package versions advanced for additive summary resources.** ADR `1.0` → `1.1`, Agent Handoff `1.0` → `1.1`, CLI Documentation `1.0` → `1.1`, Markdown Frontmatter `1.1` → `1.2`, Markdown Tooling `1.1` → `1.2`, Project Specification `1.0` → `1.1`, Python Coding `0.4` → `0.5`, Python Tooling `1.0` → `1.1`, and Standard Bundle Authoring `1.0` → `1.1`. Consumer contract defaults and adoption behavior are unchanged; summaries are discoverable package resources, not consumer-installed artifacts.
