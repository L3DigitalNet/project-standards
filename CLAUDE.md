...
- Develop on `testing`, never on `main`: every change to `testing`, including leg-worktree cherry-picks, arrives only through the GitHub Workflow admission rule — a draft PR declaring `Final: #N`, `Supporting: #N`, or `Standalone` under `## Governing work`, then `ready --pr N`, then `merge --pr N`, with two exceptions: a T0 commit, and Agent Handoff documents (`docs/handoff/**`, `docs/STATUS.md`, `docs/TODO.md`), which are transient agent-side state and always commit directly to `testing`, never through a PR — and at release time the validated `testing` is merged into `main` and pushed with the release; `main` otherwise receives only that fast-forward plus the `release: prepare vX.Y.Z` commit, and `testing` is fast-forwarded from `main` after tagging. The release commit is exempt from the PR rule by construction: `scripts/release_prep.py` pins `RELEASE_BRANCH = "main"` and refuses any other branch, so it is authored directly on `main` with `PROJECT_STANDARDS_RELEASE_COMMIT=1`; the tracked hook `scripts/githooks/main-branch-guard` (installed into `.git/hooks` by `scripts/bootstrap-worktree.sh` or `make githooks`) exists to admit that commit, and a deliberate exception needs `PROJECT_STANDARDS_MAIN_COMMIT_OVERRIDE=1`.
