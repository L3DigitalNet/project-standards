...
# The release-consistency codes a correct mid-train tree reports before the
# release-prep bump: `.standards/` and the catalog projection are regenerated by
# release prep, and the release-current prose still names the published version.
# The fourth expected finding, the pre-bump `PC-RELEASE-LEVEL`, is NOT listed
# here and must never be added: that code has two producers in
# `release.classify_catalog_diff`, and the breaking-default-promotion one is a
# genuinely forbidden transition. `--staged` takes it from `CatalogDiff.pre_bump_lag`,
# which names the individual finding, so keep this set to codes with a single
# producer. Keep both in step with docs/usage.md's `--staged` entry.
_STAGED_EXPECTED_CONSISTENCY_CODES = frozenset(
    {
...
def _format_findings(
    ...
    *,
    expected: frozenset[PackageFinding] = frozenset(),
) -> str:
    """Render findings one per line, labelling members of `expected` as non-fatal.

    `expected` holds whole findings rather than codes because `PC-RELEASE-LEVEL`
    is emitted both for the harmless pre-bump lag and for a forbidden breaking
    promotion; matching on the code would mislabel the second. It is empty on
    every path except `check-release --staged`, so the default rendering — and
    therefore the byte-for-byte output of every other command — is unchanged.
    """
    if not findings:
    ...
    for finding in findings:
        ...
        label = "EXPECTED-PRE-BUMP" if finding in expected else "ERROR"
        lines.append(
...
def _emit_findings(
    ...
    *,
    ...
    if json_mode:
        ...
        print(_format_findings(findings, expected=expected), file=stream)
...
def _run_check_release(argv: list[str]) -> int:
    ...
    try:
        ...
        expected_consistency_codes = (
            _STAGED_EXPECTED_CONSISTENCY_CODES if staged else frozenset[str]()
        )
        root = _safe_root(cast("Path", args.root))
        ...
        if consistency_findings and any(
            finding.code not in expected_consistency_codes for finding in consistency_findings
        ):
        ...
        # The classifier names its own lag findings; the CLI never re-derives them
        # from a code or a message, because `PC-RELEASE-LEVEL` also carries the
        # forbidden breaking-promotion case, which must still fail under `--staged`.
        expected = (
            frozenset([*carried, *result.pre_bump_lag]) if staged else frozenset[PackageFinding]()
        )
        blocking = tuple(finding for finding in findings if finding not in expected)
        ...
        if json_mode:
            ...
            if staged:
                ...
                document["expected_pre_bump"] = sorted(
                    {finding.code for finding in findings if finding in expected}
                )
            ...
            if findings:
                print(
                    _format_findings(findings, expected=expected),
                    file=sys.stderr,
                )
