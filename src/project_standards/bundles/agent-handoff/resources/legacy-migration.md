...
Use this guide to migrate one repository from any older handoff layout to Agent Handoff v1. The migration is agent-guided because historical consumers differ in structure, version, local edits, and fact placement. No deterministic converter can safely classify every repository.
...
```bash
project-standards adopt agent-handoff --dest . \
  --harness claude-code \
  --harness codex \
  --dry-run --json
```
...
Run the same adoption command without `--dry-run`, then validate:
