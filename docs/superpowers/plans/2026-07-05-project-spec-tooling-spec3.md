...
- [ ] **Step 5b: (Codex CR-002 — already resolved out-of-band)** The repo's `Validate Specs` workflow used to fail at any `main` merge because `.project-standards.yml` has no `spec:` block and `spec validate` exit-2s on its "never a vacuous green run" guard — a pre-existing gap since Spec #1, independent of `upgrade`. **Fixed in a standalone commit** (`.github/workflows/validate-specs.yml`): the this-repo validate/lint steps are now gated on a top-level `spec:` key existing, so CI is green until `project-spec` is registered and auto-enables when a `spec:` block appears. No action needed in this task beyond confirming the workflow file is unchanged by the upgrade work; `spec validate --config` will still exit-2 locally (expected — no `spec:` block yet), which is why the `upgrade` dogfood in Step 2 (validating the upgraded *output*) is the relevant proof, not a repo-wide `spec validate`.
