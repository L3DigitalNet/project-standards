...
def main(argv: list[str] | None = None) -> int:
    ...
    p_validate = sub.add_parser("validate", help="validate Markdown frontmatter (delegates)")
    p_validate.add_argument("rest", nargs=argparse.REMAINDER)

    args = parser.parse_args(argv)

    if args.command == "validate":
        return validate_frontmatter.main(args.rest)
    # `list` and `adopt` both touch bundle/registry data, so both sit inside the
    # error boundary — broken or drifted metadata must produce a clean exit code,
    # never a traceback (Component 3 / acceptance).
    try:
...
**Files:**
- Modify: `pyproject.toml:14-15` (`[project.scripts]`)
...
- [ ] **Step 1: Add the script (leave `version = "2.0.0"`)**

In `pyproject.toml`, replace the `[project.scripts]` block (do **not** change `version`):
...
- [ ] **Step 2: Sync and smoke-test both entry points**

Run: `uv sync && uv run project-standards list && uv run validate-frontmatter --help >/dev/null && echo OK`
Expected: standards listed, then `OK`. Confirm the lock is unchanged: `git diff --quiet uv.lock && echo LOCK_CLEAN`
...
**Files:**
- Modify: `CHANGELOG.md`
- Modify: `docs/handoff/deployed.md`, `docs/handoff/architecture.md`, `docs/handoff/specs-plans.md`, `docs/handoff/state.md`
