...
These are *dogfoodable* artifacts — their canonical source is the repo's actual file, so copy byte-for-byte:
...
Run: `cmp .editorconfig src/project_standards/bundles/_shared/editorconfig && cmp .vscode/extensions.json src/project_standards/bundles/_shared/vscode-extensions.json && echo OK`
Expected: `OK`
...
Run: `python3 -c "import tomllib,yaml; tomllib.load(open('src/project_standards/bundles/markdown-tooling/adopt.toml','rb')); yaml.safe_load(open('src/project_standards/bundles/markdown-tooling/lint-markdown.caller.yml').read().replace('{{ref}}','v2')); print('OK')"`
Expected: `OK`
...
Run: `python3 -c "import tomllib; tomllib.load(open('src/project_standards/bundles/adr/adopt.toml','rb')); print('OK')"`
Expected: `OK`
...
Run: `uv run pytest tests/test_adopt_manifest.py -v`
Expected: FAIL with `ModuleNotFoundError: ... adopt.manifest`
...
Run: `uv run pytest tests/test_adopt_engine.py -v`
...
Run: `uv run pytest tests/test_adopt_safety.py -v`
...
Run: `uv run pytest tests/test_adopt_safety.py -k "atomic or source_side or format_report" -v`
...
Run: `uv run pytest tests/test_adopt_cli.py -v`
...
Run: `git diff -- pyproject.toml; grep -A6 '\[project.scripts\]' pyproject.toml`
Expected: the block already contains `validate-frontmatter` plus in-progress entries (e.g. `sync-vscode-colors`, `sync-standards-include`). **Do not remove any of them.**
...
Run: `uv sync && uv run project-standards list && uv run project-standards validate --config .project-standards.yml --quiet && echo OK`
Expected: standards listed, validator runs, then `OK`. Confirm the lock is unchanged: `git diff --quiet uv.lock && echo LOCK_CLEAN`
...
Run: `uv run pytest tests/test_adopt_dogfood.py -v`
...
Run: `uv run pytest tests/test_adopt_dogfood.py -k "starter or agent_stub" -v`
...
Run: `uv run pytest tests/test_adopt_packaging.py -v`
Expected: PASS. **If it FAILS** because `uv_build` omits the bundle data files, add explicit inclusion to `pyproject.toml` under `[tool.uv.build-backend]` per current uv docs (verify the exact key against https://docs.astral.sh/uv/concepts/build-backend/ at implementation time), then re-run. Do not skip this test.
...
- `project-standards` CLI with an `adopt <standard>...` subcommand that materializes a
  chosen standard's canonical artifacts into a consumer repo (plus `list` and a
  `validate` subcommand). The `validate-frontmatter` console script is retained as a
  back-compat alias.
- Per-standard `adopt.toml` manifests and bundled templates under
  `src/project_standards/bundles/`.

### Changed

- Copy-adopt scaffolds relocated out of README/`adopt.md` prose into packaged bundles
  (documentation reorganization; non-breaking — consumers pin tags/workflow filenames).
- Python Tooling `.editorconfig` JSON/Markdown indentation reconciled to the shared
  superset floor (clarifying change to a copy-adopt standard; not consumer-breaking).
```
...
Run: `uv run ruff format --check . && uv run ruff check . && uv run basedpyright && uv run coverage run -m pytest && uv run coverage report && uv run pip-audit`
