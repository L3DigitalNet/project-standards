...
- [Linting / Formatting Stack — Working Document](#linting--formatting-stack--working-document)
  - [Decisions made (2026-06-04 session)](#decisions-made-2026-06-04-session)
    - [DEC-1 — Target **MADR 4.0** (resolves D5 / O5)](#dec-1--target-madr-40-resolves-d5--o5)
    - [DEC-2 — ADR Manager extension is an **optional convenience**, not a conformance target (resolves O6)](#dec-2--adr-manager-extension-is-an-optional-convenience-not-a-conformance-target-resolves-o6)
    - [DEC-3 — Stack-B linter is **markdownlint-cli2** (Node, via `npx`), not PyMarkdown (resolves O1)](#dec-3--stack-b-linter-is-markdownlint-cli2-node-via-npx-not-pymarkdown-resolves-o1)
  - [0. The core distinction (read this first)](#0-the-core-distinction-read-this-first)
  - [1. Stack A — Frontmatter validation (current, working)](#1-stack-a--frontmatter-validation-current-working)
    - [Runtime dependencies (what must be installed to validate)](#runtime-dependencies-what-must-be-installed-to-validate)
    - [Files that make up Stack A](#files-that-make-up-stack-a)
    - [How it runs](#how-it-runs)
    - [What the schema actually enforces (the "must pass" rules)](#what-the-schema-actually-enforces-the-must-pass-rules)
    - [Conventions the validator does NOT enforce (authoring rules only)](#conventions-the-validator-does-not-enforce-authoring-rules-only)
    - [Stack A's own dev/test gate (separate from runtime)](#stack-as-own-devtest-gate-separate-from-runtime)
  - [2. Stack B — Markdown lint/format (config present, NOT enforced)](#2-stack-b--markdown-lintformat-config-present-not-enforced)
    - [What exists](#what-exists)
    - [What's missing (the gap)](#whats-missing-the-gap)
    - [Why this matters for the decision](#why-this-matters-for-the-decision)
  - [3. The ADR / MADR dimension](#3-the-adr--madr-dimension)
    - [3.1 What the project already ships (already ~MADR 4.0 — see §3.5)](#31-what-the-project-already-ships-already-madr-40--see-35)
    - [3.2 The installed "ADR Manager" — what it actually is](#32-the-installed-adr-manager--what-it-actually-is)
    - [3.3 Conflicts to resolve (the heart of the decision)](#33-conflicts-to-resolve-the-heart-of-the-decision)
    - [3.4 MADR version cheat-sheet (for accuracy)](#34-madr-version-cheat-sheet-for-accuracy)
    - [3.5 MADR 4.0 conformance deltas (verified against `adr/madr@4.0.0`)](#35-madr-40-conformance-deltas-verified-against-adrmadr400)
  - [4. Decisions to make (with the options on the table)](#4-decisions-to-make-with-the-options-on-the-table)
    - [D1 — ✅ RESOLVED: engine = `markdownlint-cli2` (Node, via `npx`) — see **DEC-3** for the full trail](#d1---resolved-engine--markdownlint-cli2-node-via-npx--see-dec-3-for-the-full-trail)
    - [D2 — Frontmatter style rules: enforce? and how? (O2 — researched 2026-06-04)](#d2--frontmatter-style-rules-enforce-and-how-o2--researched-2026-06-04)
  ...
  - [5. Dependency / requirements summary](#5-dependency--requirements-summary)
    - [Already required (installed, working)](#already-required-installed-working)
    - [Potentially required (pending decisions)](#potentially-required-pending-decisions)
    - [Reference material gathered](#reference-material-gathered)
  - [6. Question ledger](#6-question-ledger)
    - [Sub-decisions — RESOLVED](#sub-decisions--resolved)
    - [Implementation backlog (decisions made; edits NOT yet applied — still gather/decide mode)](#implementation-backlog-decisions-made-edits-not-yet-applied--still-gatherdecide-mode)
...
- ✅ **DONE (2026-06-05) — DEC-8:** `README.md` now has a "### 3. Optional — Markdown body linting" subsection (the `lint-markdown.yml` `uses:` block + seed-your-`.markdownlint.json` guidance) alongside the frontmatter workflow, plus a `markdown.adr.require_sections` note in the config section. The two workflows are documented as independently adoptable.
