...
| markdown-frontmatter | `validate-markdown-frontmatter.caller.yml` (`kind=workflow-caller`) | dogfoodable |
...
| markdown-tooling | `.markdownlint.json`, `.prettierrc.json` (`kind=file`), `lint-markdown.caller.yml` (`kind=workflow-caller`); refs shared `.editorconfig`, `.vscode/extensions.json` | dogfoodable |
