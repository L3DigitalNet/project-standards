...
- **`PATH` does not exist (or exists as a regular file with `--force`):** create missing parent directories with `Path.mkdir(parents=True, exist_ok=True)`; if a parent path component exists but is not a directory, that `mkdir` fails and is reported as exit 2 (no traceback). Then write **atomically**: write to a temp file in the destination's directory and `os.replace(tmp, PATH)` (atomic same-filesystem rename; overwrites in place under `--force`). On any write error the temp file is removed and nothing replaces `PATH` (I8).
...
| Any flag-matrix conflict (Component 1) | usage error | 2 |
