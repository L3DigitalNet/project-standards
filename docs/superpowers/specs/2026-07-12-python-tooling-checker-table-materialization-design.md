...
**Date:** 2026-07-12 **Status:** owner-approved; contract audit converged after rounds 1–6; implementation reconciliation records Pyright 1.1.411 packaging drift, the corrected oracle network boundary, and complete root-materialization predecessor reconstruction **Author:** Claude (Fable 5) with Chris Purcell / L3DigitalNet
...
| `tests/fixtures/package_compatibility/legacy/release-root/**`, `tests/package_compatibility/release_candidate.py`, and `tests/package_compatibility/test_release_candidate.py` | Complete root-authority predecessor overlay, Pyright carry-through, post-atomic reconstruction, and dev-group pre-alignment proofs coordinated with coverage-plan Tasks 9 and 11 |
...
11. Extend the release-candidate fixture and test in coordination with coverage-plan Tasks 9 and 11: derive the overlay from legacy signatures plus every non-`.standards/` target having a non-`create-only` declaration and the three pinned authority/predecessor files; freeze each declared file or absence from the named pre-atomic Git tree; prove excluded all-`create-only` targets already exist and remain byte-identical; carry `pyright` through `additional_dev_dependencies` in both intents; restore the complete overlay in post-atomic reconstruction; follow the normative sequence — restore predecessor, set release version, build and extract the installed distribution, inject intent only, resolve options and render through that installed provider, guarded rewrite, preview/apply, refresh and check the lock — asserting the provider comes from the extracted installed tree; pre-align `/dependency-groups/dev` through the guarded helper — provider-derived value from schema-resolved total options, exact pre-write value and digest preconditions bound after `set_release_version`, bounded single-unit rewrite, before/after and next-lock digest assertions, and a mutation-occurred assertion; add the negative tests proving unexpected dependency drift, source-digest drift, and an already-aligned shortcut refuse without mutation; build the post-atomic source from the completed migrated tree, reconstruct it, and prove byte-identical predecessor trees plus equivalent release evidence; and prove preview applicability, migrated dev-group and lock ownership, fixed-point convergence, locked offline sync, and both oracle selections after the transition.
