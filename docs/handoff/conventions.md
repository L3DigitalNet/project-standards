...
| # | Title | Applies when |
| --- | --- | --- |
| 1 | Dogfood the standards | Editing local managed Markdown selected in `.standards/config.toml` |
...
**Applies when:** editing local managed Markdown selected in `.standards/config.toml` — currently `CHANGELOG.md`, `UPGRADING.md`, `docs/usage.md`, `docs/workflows/**/*.md`, `meta/**/*.md`, and `docs/adr/**/*.md`.
...
**Rule:** these are harness configuration, not managed documents — never add frontmatter. They are excluded from validation in `.standards/config.toml`.
