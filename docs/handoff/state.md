...
- **Release version DECIDED: `3.0.0` (MAJOR).** Two `meta/versioning.md` "previously-passing rule" triggers: `validate-id` now runs in the consumer CI workflow (old-style ids newly fail) + `parse_frontmatter` rejects duplicate top-level keys (Task 0.5). CHANGELOG heading aligned to v3; **consumer pins INTENTIONALLY left mixed on `testing`** (`adopt.md` says `@v3`, `adopt` still delivers `@v2` via `major_ref()`) — reconciled atomically at E3 (full bump list in `TODO.md`). **Do NOT "fix" the pin drift before E3.**
...
4. **E3 (cut `3.0.0`) on user go:** atomic `v2→v3` bump in one signed commit — `pyproject` + `uv.lock` + ALL consumer pins + tag `v3.0.0` + freeze `v2`/create moving `v3` + `deployed.md` + GitHub release. **Full checklist in `TODO.md`** (the caller stub auto-bumps via `major_ref()`).
