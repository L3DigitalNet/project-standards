...
| SPEC-MT01 Step 03 — `standard.toml` manifest (design) | `docs/superpowers/specs/2026-07-07-standard-manifest-schema-model-design.md` | **IMPLEMENTED 2026-07-08** (`89464ea`…`95977f8`) — Codex spec-review converged r2. Delivers the `standard.toml` Pydantic model, `load_standard_manifest` loader, generated + drift-tested `standard.schema.json`, and the valid/invalid fixture corpus. CR-004 machine-layer diff empty (no `registry.json`/bundles/`spec.include` touched) |
| SPEC-MT01 Step 03 — `standard.toml` manifest (plan) | `docs/superpowers/plans/2026-07-07-standard-manifest-schema-model.md` | **IMPLEMENTED 2026-07-08** (`89464ea`…`95977f8`) — Codex plan-review converged r2; executed subagent-driven across 11 tasks. One critical null-byte resource-path bug caught and fixed during review (loader now rejects null bytes before filesystem calls). Full gate green (949 tests, 98% coverage) |
