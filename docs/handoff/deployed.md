...
| `v2` (freezes at the `v3` push) | last `2.x` release — `v2.0.0` (commit `3ece2c9`); does **not** advance to 3.0.0 (breaking), so `@v2` trackers stay on 2.0.0 | published on `main` — still the live moving 2.x tag until `v3.0.0` publishes, then frozen |
| `v3.0.0` | `format-frontmatter` + opt-in `validate-references` + `project-standards fix`; `validate` runs all three validators; `.pre-commit-hooks.yaml`. **BREAKING** — `validate-id` now runs in the reusable CI workflow (old-style kebab ids fail) and `parse_frontmatter` rejects duplicate top-level keys; see `CHANGELOG.md` "Migration from v2" + [`UPGRADING.md`](../../UPGRADING.md). | **staged on `staging`** — publishes at the `main` push + `v3.0.0` tag |
| `v3` (moving) | will track the newest 3.x release (`v3.0.0`) | **staged** — created at the `main` push |
