...
```bash
uv tool install "git+https://github.com/L3DigitalNet/project-standards@v5.18.0"
```
...
sha256sum dist/project_standards-5.18.0-py3-none-any.whl
uv tool install ./dist/project_standards-5.18.0-py3-none-any.whl
```
...
The version command reports `project-standards 5.18.0`. A candidate built from a checkout reports the same number as the release it is being prepared for, so two candidates built from different commits are told apart only by the wheel digest above. The `mcp` subcommand accepts exactly one launch-time option:
