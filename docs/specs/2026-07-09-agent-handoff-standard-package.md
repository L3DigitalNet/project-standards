...
| 1.2 | 2026-08-27 | Claude (currency correction; not scope-affecting, so no owner re-approval) | Editorial currency only, no requirement, scope, or acceptance change: replace the hardcoded `agent-handoff@1.7` / pending-`1.8` claim in the normative-precedence note with a version-neutral pointer to the Catalog 5 default and `docs/handoff/deployed.md`, and repoint the dead T32 plan link at the retained open-issue resolution closeout evidence now that T32 is closed. |
...
**Normative precedence and current implementation:** This specification preserves the approved V1 baseline and its stable requirement IDs. For Catalog 5 package anatomy, configuration, version selection, adoption, reconciliation, and lifecycle mechanics, [SPEC-CP01](2026-07-10-consumer-standards-control-plane-spec.md), [SPEC-BA02](2026-07-10-standard-bundle-authoring-v2-spec.md), [ADR 0023](../adr/adr-0023-unified-consumer-standards-control-plane.md), and [ADR 0024](../adr/adr-0024-catalog-scoped-package-version-channels.md) take precedence over conflicting V1 mechanics below. This specification names no current payload version: the released successor is whichever immutable `agent-handoff` payload Catalog 5 marks `role = "default"` in `catalogs/5.toml`, with published pins recorded in [`docs/handoff/deployed.md`](../handoff/deployed.md). The adaptive launcher correction specified by revision 0.9 shipped in payload `1.8` and has since been carried forward by later successors. The repository-local ownership, knowledge layout, safety, validation, and migration principles remain normative unless one of those authorities explicitly supersedes them. The legacy-engine retirement gates below — two protected-consumer validations, authoritative-branch proof, and owner-approved retirement-record closure — were closed by task T32 of the open-issue resolution program on 2026-08-04; that program completed on 2026-08-12 and its master plan was retired under repository policy, so its retained record is the [open-issue resolution closeout evidence](../research/2026-08-01-open-issue-resolution-closeout-evidence.md).
