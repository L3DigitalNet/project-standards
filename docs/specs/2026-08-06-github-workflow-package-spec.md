...
| FR-021 | The mutation subcommands shall perform validated repository-scoped work-state changes: `set` shall update Issue Field values only after validating them against `org-schema.yaml`, refusing invalid values with the valid list; `new` shall create a typed issue with the canonical body headings and any supplied initial field values, printing the creation receipt on success; `close` shall atomically pair the terminal `Workflow` value with the matching GitHub close reason (design-input invariants 10 and 11) and `reopen` shall restore a nonterminal `Workflow` value. Organization-schema access stays read-only. | Field mutations are the most error-prone API surface; tool validation converts vocabulary and sync discipline into mechanical refusal (design D11). | Fixture tests cover value validation, refusal output, scaffold content, receipt emission, and terminal-sync pairing; no subcommand can mutate organization schema. | Must |
...
| EC-006 | Tool binary missing or invoked on an unsupported platform. | Tool or skill reports the precondition failure distinctly; the agent surfaces the gap instead of improvising `gh` mutations; reconcile repair restores a damaged binary. |
