...
- A compiled Go tool, `gh-workflow` (linux/amd64), with `audit` and `ledger` subcommands, shipped as a managed artifact and invoked by the skill under the operator's `gh` authentication.
...
| G-005 | Open work state is visible to the operator without querying GitHub or spending model tokens. | `docs/GH-WORKFLOWS.md` is current after mutating tasks and regenerates in one tool run. | FR-019, FR-020, DR-003 |
...
| FR-015 | The package shall deliver `gh-workflow` — one binary exposing `audit` and `ledger` subcommands — as a managed artifact at `.agents/skills/github-workflow/bin/gh-workflow` with mode `0755`, compiled for linux/amd64 from repository Go source. | Owner-directed Go tooling with zero consumer toolchain requirement; one artifact to pin and rebuild (design D7/D10). | Artifact present with pinned digest and executable mode after reconcile; binary is statically linked (`CGO_ENABLED=0`); both subcommands present. | Must |
...
| IR-004 | `gh-workflow` CLI | The tool shall run non-interactively with subcommands `audit` and `ledger`, support human-readable and JSON findings modes for `audit`, and require no arguments for either subcommand's default behavior, resolving `org-schema.yaml`, `policy.toml`, and the ledger path from their delivered locations. | Exact flag surface recorded in the skill references (OQ-002). | Both subcommands run from a consumer checkout with an authenticated `gh`. |
...
| `bin/gh-workflow` | Deterministic read-only org-schema audit (`audit`); ledger generation (`ledger`) | CLI; operator `gh` auth; YAML/TOML inputs; writes `docs/GH-WORKFLOWS.md` | Static Go binary, linux/amd64, reproducibly built (NFR-005); the only skill component that touches the network, and only in sessions. |
...
| Go tool | Audit finding classification, precondition failures, JSON/human output; ledger header, TOC anchors, layout, atomic write, gate-clean output | Offline fixture-driven tests via the repository Go gate (`make go-check`); reproducible-build check (NFR-005) | Yes |
...
| --- | --- | --- |
| FR-001–FR-020 | To be completed by the implementer per Appendix B.3 | Not Started |
...
3. `gh-workflow` (audit + ledger subcommands) implemented under the repository Go lane, reproducible build wired, binary committed (FR-015, FR-016, FR-019, NFR-005); OQ-002 resolved and recorded.
