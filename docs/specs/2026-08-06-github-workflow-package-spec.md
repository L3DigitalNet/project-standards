...
| NFR-005 | Reproducibility | The tool binary shall be reproducibly buildable from repository Go source via one documented exact invocation: `GOOS=linux GOARCH=amd64 GOAMD64=v1 CGO_ENABLED=0 go build -trimpath -buildvcs=false` with deterministic linker flags and the toolchain pinned by `go.mod`. | An independent rebuild from a clean worktree at the same commit yields the committed bytes; verified in the repository gate or release evidence. | Must |
...
5. Agent keeps `Workflow` transitions legal; terminal transitions go through `gh-workflow close` / `reopen`, which pair `Workflow` values with GitHub close reasons atomically.
