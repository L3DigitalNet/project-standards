...
| NFR-003 | `scripts/verify.sh` (dog: instruction blocks land only in selected harnesses; consumer prose survives block insertion); at 1.4 the rendered body is 8 content lines — one heading, one directive paragraph, six standing invariants — against the ~12-line ceiling. From 1.5: a package test asserts the rendered body is at most 2,400 B, as a ceiling rather than an equality, because the organization login's length varies the byte count; the 1.6 invariant-15 replacement lengthens one bullet without adding one, so the same ceiling assertion is the proof the block did not overflow | Passing (1.4); **Pending re-verification (1.5)**; **Pending re-verification (1.6)** |
