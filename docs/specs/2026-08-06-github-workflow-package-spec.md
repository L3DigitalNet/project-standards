...
| 1.13 | 2026-08-29 | Claude with owner directive (2026-08-29) | `github-workflow` 1.6 **replaces standing invariant 15** — “durable follow-up work discovered while implementing becomes an Issue before the session ends” — with the owner's finding-disposition rule, per owner directive 2026-08-29 (D-016). A related finding the agent can address in the session gets no issue: it is fixed in place when the repository being worked in owns it, and filed as an issue in the owning repository when an upstream dependency hosted in the organization owns it; only a finding large enough to warrant a full separate session sends the agent to ask the operator whether to file an issue or tackle it now. Amends the Glossary “Standing invariants” row, FR-007, D-003, §10.1 step 6, and the §17.3 FR-007 and NFR-003 rows; adds D-016 and DEV-004. The block still carries six standing-invariant bullets and the 2,400 B rendered-body ceiling (NFR-003, FR-007) is unchanged. No other requirement, decision, or open question changes; OQ-004, D-012, D-014, and the 1.5 `ledger` removal are untouched. Bullet count corrected to six on verification. |
...
| Standing invariants | The block-carried subset of the operating model's invariants that bind even when the skill was never loaded. Through package version 1.4: never infer readiness (design-input invariant 5); no `Execution mode` self-promotion plus the human-applied org-schema rule (invariant 6); a nontrivial PR links its governing Issue (invariant 8); terminal-state synchronization (invariants 10 and 11); durable follow-up work becomes an Issue (invariant 15). **Amended at package version 1.5** (owner decision 2026-08-26; DEV-002): invariant 5 becomes “the agent authors the acceptance criteria, sets `Workflow`, and admits work to `Ready` itself; an issue whose acceptance criteria the agent could not write is `Needs definition`, and open state alone never implies `Ready`”, and invariant 6 splits into “`Unattended agent` is the owner's grant, never a mode an agent asserts” plus the unchanged human-applied org-schema rule. Invariants 8, 10, 11, and 15 are unchanged; 1.5 additionally places the routing table in the block (FR-007, D-003, D-014). **Replaced at package version 1.6** (owner directive 2026-08-29; D-016; DEV-004): invariant 15 is no longer “durable follow-up work becomes an Issue”; it becomes the finding-disposition rule — a related finding the agent can address in the session gets no issue and is fixed in place when the repository being worked in owns it, a finding owned by an upstream dependency hosted in the organization is filed as an issue in that dependency's repository, and only a finding large enough to warrant a full separate session sends the agent to ask the operator whether to file an issue or tackle it now. Invariants 5, 6, 8, 10, and 11 are unchanged at 1.6, and the block still carries six invariant bullets. | Numbering follows the design input's invariant list (its section 34). The 1.5 wording of invariants 5 and 6 diverges from that preserved document and is recorded as DEV-002 rather than back-patched into it; the 1.6 wording of invariant 15 diverges from it likewise and is recorded as DEV-004. |
...
6. From package version 1.6, the agent disposes of a discovered finding rather than filing one automatically (D-016): a related finding it can address in this session gets no issue — it is fixed in place when the repository being worked in owns it, and filed as an issue in the owning repository when an upstream dependency hosted in the organization owns it — and only a finding large enough to warrant a full separate session sends the agent to ask the operator whether to file an issue or tackle it now. Through package version 1.5 this step read: discovered durable follow-up work becomes an Issue before the session ends.
...
Statuses below were all **Passing** at package version 1.4. Revision 1.9 re-opens the rows it amended: a row reading **Pending re-verification (1.5)** was proven for the 1.0–1.4 contract and must be re-proven against the amended text before the 1.5 cut is accepted, a row reading **Pending (1.5)** is new and unproven, and a row reading **Retired at 1.5** was Passing through 1.4 and has no successor verification because the requirement no longer exists. Revision 1.13 re-opens the two rows it amended the same way: **Pending re-verification (1.6)** marks a row whose amended text must be re-proven against the 1.6 payload before that cut is accepted.
...
| FR-007 | `scripts/verify.sh` (dog: instruction blocks land only in selected harnesses; consumer prose survives block insertion); from 1.5 also a dogfood assertion that the rendered block carries the routing table intact after `reconcile` and the markdown-tooling gate, plus the NFR-003 byte-ceiling assertion; from 1.6 also assertions that both rendered blocks carry the finding-disposition bullet and that no delivered artifact still requires an issue for every discovered durable follow-up | Passing (1.4); **Pending re-verification (1.5)**; **Pending re-verification (1.6)** |
...
| NFR-003 | `scripts/verify.sh` (dog: instruction blocks land only in selected harnesses; consumer prose survives block insertion); at 1.4 the rendered body is 7 content lines — one heading, one directive paragraph, six standing invariants — against the ~12-line ceiling. From 1.5: a package test asserts the rendered body is at most 2,400 B, as a ceiling rather than an equality, because the organization login's length varies the byte count; the 1.6 invariant-15 replacement lengthens one bullet without adding one, so the same ceiling assertion is the proof the block did not overflow | Passing (1.4); **Pending re-verification (1.5)**; **Pending re-verification (1.6)** |
...
| NFR-006 | `scripts/verify.sh` (pkg: `SKILL.md` at most 70 lines / 12,000 B and `field-vocabulary.md` at most 30 lines / 3,200 B against the payload sources; `metadata.lines` equals the actual line count) | **Pending (1.5)** |
...
| DEV-004 | Glossary “Standing invariants”; FR-007; D-003; §10.1 step 6 | The 1.6 wording of design-input invariant 15 diverges from the preserved design input, which still requires that durable follow-up work discovered while implementing become an Issue before the session ends. The package now carries the finding-disposition rule at that invariant's position (D-016). Invariant numbering, and the archived document's invariant text, are unchanged. | The owner's 2026-08-29 directive changes the operating model's disposition rule, not the package's mechanics, and this document packages that model without redefining it. Per this document's own preservation rule the archived design input is not back-patched, so the divergence is recorded here, following the DEV-002 pattern. | **Open** — the owner may close it as DEV-002 was closed, by adding a dated amendment note to `docs/specs/archive/2026-08-06-github-repo-administration-preliminary-design.md` section 34 that records the 1.6 divergence in invariant 15 and points to this row and to the Glossary “Standing invariants” wording, leaving the preserved invariant text itself unchanged. |
