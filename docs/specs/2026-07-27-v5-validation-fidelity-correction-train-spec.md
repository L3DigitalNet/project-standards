...
| FR-013 | Markdown Frontmatter's adoption guide and skill documentation shall never link a **mutable** GitHub ref (`blob/main` or `blob/v5`) — the defect class #53 actually reports. A document that stays inside this repository's own tree (e.g., `standards/markdown-frontmatter/versions/1.6/skills/.../SKILL.md` linking its own family's `README.md`) shall use a repository-relative link, which resolves correctly at every location that document is read from within the repository. A document reference that only resolves once the file is **installed into a consumer repository** (e.g., `adopt.md`'s reference to this repository's own `docs/usage.md` CLI guide, which does not travel with the installed payload) shall instead use a release-tag-pinned absolute URL (`blob/v5.10.0/...` or an exact commit SHA), because a relative link to a path the consumer's repository does not contain cannot resolve once installed. | #53 — 1.5's `adopt.md` and `SKILL.md` hardcode mutable `blob/v5`/`blob/main` URLs that move a reader off a pinned release tag; the same pattern recurs across 1.2-1.5. Rev 0.1's blanket "always use a repository-relative link" requirement was itself defective: applied literally to `adopt.md`'s CLI-guide reference, a relative link breaks the moment that file is installed into a consumer repository, since the consumer repository has no `docs/usage.md` at that relative path. | The new successor payload's `adopt.md` and `SKILL.md` contain no occurrence of `blob/(main\|v5)`; every reference that only resolves at an installed location uses a release-tag-pinned absolute URL or exact commit SHA instead of a mutable ref or a broken relative path; every reference that stays inside the repository's own tree remains repository-relative; a mutable-ref guard test scans every version of every family (not just markdown-frontmatter) for `blob/(main\|v5)`, fails immediately on any new occurrence outside a fixed, shrink-only allowlist of already-released files predating the guard, and exempts `legacy-reference`-role resources (frozen historical bytes kept for whole-file recognition, not authored prose); released 1.2-1.5 bytes remain unchanged and are documented as a known historical limitation via that allowlist. | Must |
...
- [ ] #66's create-only removal guard (FR-017) is committed and passing.
...
This table names the actual landed test files as of the audited integration diff, not the fixture names rev 0.1 anticipated before implementation. Several acceptance clauses this revision adds or corrects (§ Deviations Log) describe a target the audited diff does not yet fully reach; those rows are marked **Planned — parallel fix** rather than **Landed**, and name the existing test whose expectation the parallel fix must also update where one exists.

| Requirement | Landed Verification | Status |
| --- | --- | --- |
| FR-001 | `tests/test_spec_validate.py::test_author_numbered_subsection_under_canonical_parent_passes`, `::test_numbered_subsection_under_unknown_parent_is_still_rejected`, `::test_subsection_acceptance_does_not_mask_top_level_order` | Landed (present-parent acceptance and non-canonical-parent rejection); the absent-but-canonical-parent case (an orphan subsection under a top-level number the document never writes) is not yet covered by any test in this diff and is not yet rejected by `_check_sections`, which consults the registry, not the document, for parent presence — **Planned — parallel fix** |
| FR-002 | `tests/test_spec_validate.py::test_compound_id_under_configured_reference_prefix_is_not_undeclared`, `::test_compound_id_under_unconfigured_prefix_still_reports_inner_prefix`, `::test_compound_id_under_appendix_a_declared_prefix_is_not_undeclared`, `::test_plain_id_after_a_lowercase_hyphenated_word_is_still_checked`, `::test_compound_head_longer_than_a_prefix_does_not_suppress_the_inner_id` | Landed (depth-one compound suppression under a configured external prefix, and the non-suppression guards); `test_compound_id_under_appendix_a_declared_prefix_is_not_undeclared` currently asserts the **opposite** of this revision's corrected requirement (it expects an Appendix-A-declared prefix to suppress the finding) — **Planned — parallel fix**, which must also invert that test's expectation |
| FR-003 | `tests/test_spec_lint.py::test_inline_code_and_autolinks_are_not_placeholders`, `::test_real_placeholders_are_still_flagged`, `::test_fenced_placeholders_and_guidance_are_not_linted` | Landed (masking, bare-angle-group residual, and fenced-block workaround all verified exactly as this revision describes); CommonMark backtick-run length-pairing and the nonempty-domain autolink tightening are not yet distinctly tested — **Planned — parallel fix** for those two refinements only |
...
| FR-005 | `UPGRADING.md`'s `CP-MIGRATION-PLATFORM-VERSION` row already states the `"v4"`-recommended, section-cited wording this revision requires; no test in this diff asserts the runtime hint text in `control_plane/migration.py` matches it yet (the hint still reads "both tags name the same legacy wire format" without naming a recommended literal or citing the section) | **Planned — parallel fix** |
| FR-006 | `tests/package_contract/test_python_tooling_1_10.py::test_python_tooling_1_10__help_flag__prints_usage_and_runs_nothing`, `::test_python_tooling_1_10__unknown_argument__fails_without_running_the_gate` (covers the `--`-separator case), `::test_python_tooling_1_10__no_arguments__runs_every_command_in_order`, `::test_python_tooling_1_10__failing_command__stops_and_propagates_exit_code` | Landed (help alone, unknown argument alone including post-`--` tokens, no-argument, failure propagation); the co-occurring `--help` + unrecognized-argument case is not yet tested, and `main()` currently checks `--help`/`-h` before any unknown-argument check, so it does not yet reject that combination — **Planned — parallel fix** |
...
| FR-012 | `tests/agent_handoff/test_policy.py::test_conventions_entry_findings_name_each_oversized_section`, `::test_conventions_entry_size_excludes_fenced_examples` | Landed (per-section, line-located findings with fence-content exclusion from the measured size); `test_conventions_entry_findings_name_each_oversized_section` currently asserts messages of the form `"section 1. First entry has 1300 chars; max 1200"`, which names the consumer's section — the **opposite** of this revision's redaction requirement — **Planned — parallel fix**, which must also rename/rewrite that test to drop the section name, exclude blank separator lines from the measured size, and add the engine/payload equivalence fixture for the five previously divergent cases |
| FR-013 | `tests/package_contract/test_release_doc_link_hygiene.py::test_versioned_payload_has_no_mutable_release_ref`, `::test_allowlist_has_no_stale_entries`, `::test_mutable_ref_scan_would_have_caught_issue_53` | Landed (the mutable-`blob/(main\|v5)`-ref guard, scoped exactly to that defect class, with a shrink-only historical allowlist); `standards/markdown-frontmatter/versions/1.6/adopt.md` still links its CLI usage guide with a repository-relative path (`../../../../docs/usage.md`), which does not resolve once the file is installed into a consumer repository — **Planned — parallel fix** to move that specific reference to a release-tag-pinned absolute URL |
...
| FR-017 | Mirrors `tests/control_plane/test_adapters_whole_file.py`'s existing create-only preservation shape, applied to a zero-byte rendered/observed target | **Planned — parallel fix**; no test exists in this diff yet |
| NFR-001, NFR-006 | Baseline/candidate outcome matrix and `packages check-release --baseline v5.9.0` (or equivalent) | Planned |
| NFR-003 | Per-issue regression-test/closure-evidence audit, `tests/issue_regressions/ledger.toml` | Landed for #50-#61/#63-#65/#68/#69 parts 1-2; Planned for #66 pending FR-017 |
