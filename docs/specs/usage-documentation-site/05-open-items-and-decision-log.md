...
| D-003 | Recorded 2026-08-27: the seven implementation specifications (`00`–`06`) use the Standard profile and `README.md` stays Light. Standard rationale, as it stood in each Appendix D: “This specification uses the Standard profile because the change spans one repository, several standards, packaged CLI behavior, validation machinery, and dogfooding requirements, but it does not introduce durable runtime data or external production services.” Upgrade trigger: “Upgrade to Full only if the implementation introduces a durable service, external integration, release orchestration system, or substantial runtime data model beyond standard repository files.” Index rule: “This index should stay Light profile unless it starts carrying substantive implementation requirements. If that happens, move those requirements into `00-master-spec.md` or a focused child spec instead of upgrading the index.” | Project Specification 1.9 lint treats Appendix D as a canonical boilerplate surface (`SL-BOILERPLATE`), so the restored surface admits no per-document prose; the profile decision needed a durable home, and D-001 puts accepted decisions in §8.3. | Leave the rationale in Appendix D and accept the strict-lint warning; rejected, hosted CI fails on WARN. Drop the rationale; rejected, the profile choice and its upgrade trigger would become unrecorded. | TBD local ADR |
...
- docs/specs/usage-documentation-site/05-open-items-and-decision-log.md
