...
| --- | --- | --- | --- | --- |
| FR-001 | The system shall, in the distributor repository, add the new standard to `standards/README.md`. | The standards index is the entry point for governed standards. | Index row links bundle and adopt guide. | Must |
| FR-002 | The system shall, in the distributor repository, update registry and CLI parity lists for `usage-documentation-site`. | Adopt/list commands enforce parity. | `project-standards list` succeeds. | Must |
| FR-003 | The system shall, in the distributor repository, update `meta/versioning.md` for the new contract marker. | Consumers need release-contract semantics. | Versioning table includes `usage_documentation_site.version`. | Must |
| FR-004 | The system shall, in the distributor repository, adopt the standard in `project-standards` under `docs/usage/`. | Dogfood is mandatory demonstration and interoperability proof. | Dogfood files exist and strict build passes. | Must |
| FR-005 | The system shall, through the dogfood site, document the user-facing usage of the `project-standards` tools. | The repository must model the standard it ships. | Site contains user-facing pages for the public command surface. | Must |
| FR-006 | The system shall, in the distributor repository, validate every governed standard together after adoption. | Interoperability is the proof point. | Repository validation, spec validation, Markdown checks, and MkDocs strict build pass. | Must |
