...
- **Docs dogfood** — `validate-frontmatter` stays green (design docs under `docs/specs/archive/` remain outside the frontmatter `include` set); this repo's own `format.yml` + `lint-markdown.yml` stay green on the edited docs.
