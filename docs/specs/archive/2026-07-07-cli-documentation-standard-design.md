...
**Date:** 2026-07-07 **Status:** approved — codex spec-review converged round 3 ("needs minor correction"; minor items applied) **Author:** session 2026-07-07
...
| CLI surface to dogfood | `pyproject.toml` registers **seven** console scripts: `project-standards` (primary; top-level `validate`, `fix`, `adopt`, `list` + the `spec` group's `validate`/`lint`/`extract`/`next`/`new`/`upgrade` — 10 leaf commands plus the `spec` group overview) plus six standalone single-purpose commands (`validate-frontmatter`, `validate-id`, `sync-vscode-colors`, `sync-standards-include`, `format-frontmatter`, `validate-references`), all public (downstream CI invokes `validate-frontmatter` et al. directly) (codex SA-NEW-001). Exit codes 0/1/2(/3 where applicable). **No `--version` on any entry point today** — added by this work (codex SA-002). Profile selection recorded as **Packaged** (§8). |
...
- **Profile selection (recorded here per §1):** `project-standards` selects **Packaged**. Rationale: 10 leaf commands plus the `spec` group overview, single-page reference maintainable; the deep profile's generated-pages trade-off does not pay for itself at this size (codex SA-001).
- **Public installed-command set (codex SA-NEW-001):** all seven `[project.scripts]` keys are public — the six standalone commands are consumer-facing (downstream CI invokes `validate-frontmatter` directly; none are classified internal). Documentation obligation under §1's grouped-page provision: `docs/usage.md` documents `project-standards` in full (**all 10 leaf commands plus a `spec` group-overview entry**) plus a **Standalone commands** section giving each of the six its own complete entry (`NAME`, `SYNOPSIS`, `OPTIONS`, `EXIT STATUS`), cross-referenced to unified equivalents where one exists (e.g. `validate-frontmatter` ↔ `project-standards validate`).
...
- **Option/exit-code parity pass** (codex SA-NEW-003 — inventory alone proves headings, not content): during authoring, every documented command's `OPTIONS` and `EXIT STATUS` sections are checked against its normalized `--help` output (`NO_COLOR`, fixed width) and source-defined exit behavior; the plan records this as an explicit per-command verification checklist, and any intentionally manual assertions are enumerated in the implementation notes. Automated per-option snapshot tests are **not** required for 1.0 (deliberate scope bound — the dogfood's drift guards are the inventory test + smoke tests + this recorded authoring pass).
...
- The option/exit-code parity pass (§8) is recorded in the plan's verification evidence, with any manual assertions enumerated (codex SA-NEW-003).
