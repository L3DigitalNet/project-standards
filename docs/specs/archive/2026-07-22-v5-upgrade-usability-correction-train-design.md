...
| FR-006 | `CP-CONSUMER-CONFLICT` findings for semantic units must carry the expected package value and observed consumer value with full fidelity in JSON output and bounded truncation in human output; byte-valued and whole-file conflicts must carry expected and actual content digests instead of raw bytes. |
