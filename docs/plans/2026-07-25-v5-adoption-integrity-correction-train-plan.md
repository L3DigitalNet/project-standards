...
> **This file is definition, not state.** The owner approved the original implementation contract for T1-T17 on 2026-07-26, selected SPEC-VAIC rev 0.4 Option 1, and authorized autonomous convergence through the rev 0.5-rev 0.12 review corrections and appended T19. T19 may not enter Python-specific GREEN until this amendment and its independent reviews converge. Live progress belongs under `.project-pipeline/2026-07-25-v5-adoption-integrity-correction-train/`. T18 remains blocked on separate authorization for the exact qualified candidate.
...
- publishes immutable successor payloads for Python Tooling, Markdown Tooling, Agent Handoff, and CLI Documentation plus the contract-only internal Standard Bundle Authoring 2.6 successor;
...
3. The owner approved SPEC-VAIC rev 0.4 Option 1 and authorized autonomous convergence of the bounded T19 amendment, including rev 0.5-rev 0.12 review corrections, on 2026-07-26. Python-specific T19 GREEN requires passing amended spec/plan validation and converged independent reviews; all prior T1-T17 authorization remains unchanged.
...
- Release-document/catalog consistency checks and the required Standard Bundle Authoring 2.6 contract successor.
...
- Every qualifying family-indexed Python Tooling predecessor that directly declares `ci.performance` and whose schema-resolved empty config yields true has a direct opted-in edge to 1.9; enabled CI materializes the prior effective value, disabled CI produces an empty diff, and explicit true/false remains unchanged. A version with no directly declared option has no relevant option and no edge; one that resolves it false receives no edge; and a declared option whose resolved empty config is absent, non-boolean, or underivable stops under AW-005. Every no-edge version must render no performance gate from empty config or return to AW-005 for owner disposition.
...
- Markdown Tooling 1.9's lint-selected corpus must remain byte-for-byte identical to the 5.8.0 lint-selected corpus for every characterized exclusion form; only the format-selected corpus may narrow to that lint corpus. Any lint-corpus deviation in either direction or format-corpus widening requires owner disposition under SPEC-VAIC AW-004.
...
| FR-003 | #36 | T14, T15, T17 | TC-T14-001, TC-T14-002, TC-T15-001, TC-T17-002 |
...
| NFR-009 | Release classification | T1, T14, T15, T17, T19 | TC-T1-004, TC-T14-002, TC-T15-001, TC-T17-002, TC-T19-003 |
...
| C-009 | T19 permits only explicit direct automatic-edge opt-in whose changed pointer paths pass bounded declaration eligibility and whose target-valid consumer config yields a fully valid source view through shared-direct-object recursion plus whole-key atomic omission, one provider and one plan transform, declared pointer diffs, introduced-leaf dual-schema validity, idempotence, ordinary plan/apply preconditions, and no package-ID branch. | TC-T19-001, TC-T19-002, TC-T19-003, TC-T17-002 plus T19 scope logs |
...
| Standard Bundle Authoring successor | `standards/standard-bundle-authoring/versions/2.6/`, family manifest, SPEC-BA02, templates/docs/tests | Add only the optional transform authoring contract; keep the existing 2.0-2.5 internal catalog rows immutable and add 2.6 as another internal row only at T15. |
...
| Release consistency | release/package validation source and tests; root `README.md`, `UPGRADING.md`, and `AGENTS.md`; `standards/README.md`; the T14.0-enumerated mutable family-level `standards/{family}/*.md` corpus | Every consumer-family version reference equals its candidate-catalog default or carries an asserted historical classification; for a family with internal rows and no default, the current reference equals the numerically greatest canonical catalog `(major, minor)` row carrying role `internal`, explicitly excluding staged `unadvertised` rows and failing with `PC-RELEASE-INTERNAL-CURRENT-MISSING` when that set is empty; candidate commit, catalog, defaults, internal-current references, links, commands, and prose agree before tag. |
...
**Successor staging rule:** T8, T9/T11, T12, and T13 create complete consumer payloads, including their adoption guides, with `availability = "consumer"` and add each version to its family `standard.toml`, but do not add those versions to canonical `catalogs/5.toml`. T8 owns Python Tooling 1.9's fresh default and package features; T19 solely adds its generic-transform provider/declarations, the contract-only Standard Bundle Authoring 2.6 family version, and the required family/projection digest consequences after the engine contract exists. Neither staged successor enters canonical `catalogs/5.toml` during T19. Each owning task that creates or changes staged-successor bytes regenerates the family-manifest aggregate digest, the corresponding packaged payload projection, and `standards/catalog.md`; the new catalog row remains `unadvertised`, and later byte changes alter that row only when its rendered resource/provider/output counts change. Package, integrity, graph, schema, provider-unit, documentation, rendered-catalog, and payload-projection gates cover the family-indexed staged payloads directly, while resolution cannot select an uncatalogued version and this repository's `version = "latest"` selectors remain on the released defaults. After T19 and the final owning P2 task finish, those successor bytes/digests are final. T15 solely owns canonical catalog activation: it changes the four prior consumer defaults to retained, adds the four already-validated consumer successors as defaults, adds the unchanged T19-authored Standard Bundle Authoring 2.6 digest with role `internal` alongside the byte-identical existing 2.0-2.5 internal rows, regenerates catalog projections and `standards/catalog.md` roles without changing payload/family bytes, and reruns every package/provider/migration/integration gate. The T19 delta analysis and T15 exact-candidate check must both classify that additive internal row as MINOR. Released predecessor bytes never change.
...
2. **Before GREEN:** inspect `git diff --name-status "$TASK_BASE"` and `git diff --stat "$TASK_BASE"`. The RED diff may contain only focused tests, fixtures, and ephemeral evidence needed to prove the task's specified behavior. Production changes or unrelated cleanup at this checkpoint are a hard stop, except for a base-commit input listed in §6.2 whose objective mutation test is already RED before any further production edit.
...
4. **Phase boundary:** after the final task in each phase, compare the aggregate phase diff and commits with SPEC-VAIC scope, constraints, and non-goals before starting the next phase. T19 has its own prerequisite boundary: it must pass this aggregate review and seal its bytes before T14 begins.
...
| --- | --- | --- |
| T19 | Owner-authorized SPEC-VAIC rev 0.4-rev 0.12 convergence prepared SPEC-BA02 rev 0.15 and the Standard Bundle Authoring 2.6 input payload before resumed T19 RED. | T19.1 tests the prepared declaration and eligibility predicate through executable contract cases that fail against a deliberately mutated copy before passing against the base bytes; package, graph, schema, and projection gates then bind the unchanged prepared input. |
...
| T14 | Add candidate-bound release consistency | P3 | T8-T13, T19 | FR-003, FR-019, NFR-009, IR-004 |
| T15 | Activate successors and prove migrations | P3 | T2-T14, T19 | FR-003, FR-012, FR-015, FR-019, FR-021, NFR-001, NFR-004, NFR-005, NFR-008, NFR-009, IR-005, DR-002 |
...
| T19 | Add generic direct package-config transforms | P2/P3 prerequisite | T5, T8 | FR-017, FR-019, FR-021, NFR-002, NFR-004, NFR-008, NFR-009, IR-004, IR-005 |
...
- **acceptance:** Prettier-clean `.vscode/settings.json` and `.claude/settings.json` fixtures plus the parametrized JSON/JSONC corpus stay clean after every supported semantic insertion/replacement; parsing succeeds under the appropriate parser; existing keys/comments/values outside owned units are byte-preserved where the adapter contract promises preservation, including JSONC comments, trailing commas, and compact values in `.claude/settings.json` (TC-T3-001, TC-T3-002).
- **sub-tasks:**
...
- **acceptance:** pinned effective-file-set cases cover `dir`, `dir/`, `dir/**`, nested files, supported negation, and platform separators across provider-rendered caller inputs, reusable-workflow input/materialization, shell/cwd behavior, and each tool's matching dialect; the causal record identifies the actual divergence; configured exclusions remain authority; the correction narrows the format-selected corpus to the lint-selected corpus while proving the 1.9 lint-selected corpus is identical to the 5.8.0 lint-selected corpus for every characterized form; any lint-corpus deviation in either direction or format-corpus widening routes through AW-004 before GREEN; the complete consumer 1.9 payload is family-indexed, package-visible, and uncatalogued while root `latest` remains on 1.8; shared reusable workflows stay byte-identical unless the owner explicitly dispositions AW-004 before GREEN; an exact 1.8 fixture remains unmigrated and compares its `@v5` workflow outcomes across baseline/candidate; Markdown Tooling 1.8 bytes remain unchanged (TC-T9-001, TC-T9-002).
- **sub-tasks:**
  ...
  - **T9.3 GREEN** — create complete uncatalogued consumer Markdown Tooling 1.9 with family registration, regenerate its packaged payload projection and `standards/catalog.md` `unadvertised` row, and implement only the characterized payload/provider materialization needed to narrow the format-selected corpus to the unchanged 5.8.0 lint-selected corpus. If the lint-selected corpus changes in either direction, format scope widens, or shared workflow bytes must change, stop for AW-004 owner disposition before GREEN and extend the released-selection matrix; if no divergence reproduces, carry 1.8 rendering forward unchanged and add the guard. T11 completes and seals this successor; T15 owns only catalog/default activation.
...
- **goal:** Prevent a release commit whose release-facing documentation and default-package claims do not match its exact metadata and candidate catalog. · **phase:** P3 · **depends_on:** [T8, T9, T10, T11, T12, T13, T19] · **requirements:** [FR-003, FR-019, NFR-009, IR-004] · **priority:** must
- **files:** release/package consistency validation source/tests/workflow as needed; root `README.md` as a gate input plus the narrow current-authority correction from Standard Bundle Authoring 2.4 to the existing canonical 2.5 row; `UPGRADING.md` and `AGENTS.md` as gate inputs/fixtures only; catalog-derived inputs/fixtures for `standards/README.md` and the T14.0-enumerated Markdown documents directly under each package family; repository-wide internal-version reference search; editable corrections in `standards/project-spec/README.md` for the stale prose assertion and one enable command plus `standards/project-spec/adopt.md` for the second enable command
- **acceptance:** fixtures prove the gate fails independently on stale project version, release-current pins, package/default rows, family-level version references, versioned links, enable commands, internal package references, unclassified historical examples, or broken current-package links; the editable family corpus is exactly the tracked `standards/{family}/*.md` set enumerated at T14.0 and is mechanically disjoint from immutable `standards/*/versions/**`; each consumer-family reference in that corpus must equal the supplied candidate-catalog default or carry an asserted explicit historical classification. An internal family with no default uses the numerically greatest canonical catalog `(major, minor)` row carrying role `internal` as the current reference, ignores staged `unadvertised` rows, and fails with `PC-RELEASE-INTERNAL-CURRENT-MISSING` rather than skipping when no canonical internal row exists; expected values are derived from the catalog rather than literals; deliberately historical references/permalinks remain classified and preserved; catalog/index regeneration is byte-idempotent; a candidate fixture proves the Standard Bundle Authoring references in root `README.md`, `standards/README.md`, and `AGENTS.md` must equal the derived canonical `internal` row, and an older still-catalogued internal reference plus any other unclassified superseded internal-version reference in the characterized mutable-current corpus fails the repository-wide sweep. Immutable payload directories, archived specifications, changelog history, dated handoff/session history, dated revision/deviation tables in live specs/plans, live governing spec/plan body prose explicitly classified as historical characterization, inventory, or catalog-range context, and explicit historical permalinks are classified history; catalog- and family-manifest-derived enumerations are authoritative inventories validated against their source rather than current-reference assertions. The live T14 commit corrects root `README.md` from 2.4 to the existing canonical internal 2.5 row, and T15 performs the indivisible 2.5-to-2.6 catalog-plus-prose move after adding the 2.6 canonical row; `standards/project-spec/README.md` corrects its prose assertion and enable command from 1.2 to 1.4, while `standards/project-spec/adopt.md` corrects its enable command from 1.2 to 1.4; an exact release-commit fixture passes. This task does not bump the repository version or edit activation-owned successor-family documents; its root and Project Specification corrections restore existing current truth, while T15 assembles the five successor activations (TC-T14-001, TC-T14-002).
- **sub-tasks:**
  - **T14.0 CHARACTERIZE** — enumerate the actual tracked mutable `standards/{family}/*.md` corpus and assert it is disjoint from immutable `standards/*/versions/**`; inventory internal-version references and classify the repository-wide scan before RED. Mutable release-current prose is enforced; immutable payloads, archived specifications, changelog history, dated handoff/session history, dated revision/deviation tables, and live governing spec/plan body prose that names versions as historical characterization, inventory, or catalog-range context are recorded as structural history; explicit historical permalinks are preserved; catalog- or family-manifest-derived all-version enumerations are inventory validated against their source. Record the exact live documents and references admitted by each class, plus path/line evidence for the root 2.4 mismatch, `standards/project-spec/README.md`'s 1.2 prose and enable command, and `standards/project-spec/adopt.md`'s 1.2 enable command; stop rather than expanding the scan when a new path class lacks a declared current-versus-historical rule.
  - **T14.1 RED** — add fixture commits/documents with each independent stale field, including separate current-reference cases in direct family `README.md`, `adopt.md`, `agent-summary.md`, and another enumerated direct family document plus an explicitly historical reference whose classification is removed; add release-current/historical classification, live-spec revision-history, live governing body-prose characterization/inventory/catalog-range context, and catalog-derived inventory classifications, non-idempotent projection, root/standards/AGENTS SBA references, numeric internal ordering with 2.6 versus 2.10, an empty canonical-internal-set diagnostic, an older still-catalogued internal reference, repository-wide mutable-current superseded-internal-version sweep, the root 2.4 current-authority mismatch, and all three stale Project Specification current-reference regressions.
  - **T14.2 Verify RED** — prove the current checker misses the historical #36 mismatch, a fixture whose SBA prose disagrees with its supplied canonical internal catalog row, the root 2.4 current-authority assertion, and the Project Specification 1.2 prose assertion plus two enable commands.
  - **T14.3 GREEN** — derive release-facing facts from supplied candidate metadata/catalog; scan the enumerated direct family Markdown corpus and require each version, versioned link, and enable command for that family either to equal its candidate-catalog default or to carry an asserted explicit historical classification. For an internal family with no default, derive its current version by numeric `(major, minor)` ordering over canonical rows whose role is exactly `internal`, explicitly excluding staged `unadvertised` rows and emitting `PC-RELEASE-INTERNAL-CURRENT-MISSING` for an empty set. Add the pre-tag gate, preserve the characterized immutable, structural-history, inventory, and classified-history corpus such as Python Tooling's build-backend timeline, validate the three current SBA references and reject an older still-catalogued or other unclassified superseded internal-version reference in mutable-current prose, correct root `README.md` from 2.4 to the existing canonical 2.5 row, and correct the named Project Specification prose assertion plus two enable commands to 1.4 without assembling release-version bytes or editing successor-family activation prose.
...
- **goal:** Select the five successors together, complete the release commit bytes, and prove package, transform, migration, issue-ledger, and consumer-outcome compatibility before Opus review. · **phase:** P3 · **depends_on:** [T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T19] · **requirements:** [FR-003, FR-012, FR-015, FR-019, FR-021, NFR-001, NFR-004, NFR-005, NFR-008, NFR-009, IR-005, DR-002] · **priority:** must
- **files:** family manifests, `.standards/config.toml`, canonical and generated catalog/projection artifacts including `standards/catalog.md`, `pyproject.toml`, `uv.lock`, `CHANGELOG.md`, root `README.md`, `UPGRADING.md`, `AGENTS.md`, `standards/README.md`, the T14.0-enumerated direct family-level current-reference Markdown corpus for the four consumer successor families and internal Standard Bundle Authoring family, every repository artifact managed by the four activated consumer packages as enumerated before T15 RED—including root `.markdownlint.json`, managed Markdown caller workflows, `.agents/**` Agent Handoff surfaces, `.claude/settings.json`, `.vscode/settings.json`, and managed instruction blocks—status/handoff candidate-preparation state, current-train ledger rows, cross-package/migration/coherence/outcome tests, and catalog-identity fixtures including `tests/package_contract/test_current_catalog_activation.py`
- **acceptance:** defaults select Python Tooling 1.9, Markdown Tooling 1.9, Agent Handoff 1.5, and CLI Documentation 1.4; canonical `catalogs/5.toml` adds the unchanged T19-authored Standard Bundle Authoring 2.6 digest and role `internal` while retaining every existing 2.0-2.5 internal row and payload byte; root `README.md`, `standards/README.md`, `AGENTS.md`, and the T14.0-enumerated Standard Bundle Authoring family current-reference documents move to the same canonical 2.6 selection in this indivisible change, and the repository-wide superseded-internal-version sweep passes with only classified historical references; the standards index and affected direct family-level current references name the four consumer defaults plus the derived internal Standard Bundle Authoring 2.6 current row and carry matching versioned links and enable commands where applicable, while any older direct family-level version reference carries an asserted historical classification; expectations derive from the candidate catalog; `standards/catalog.md` changes the four staged consumer successors from `unadvertised` to `default`, their predecessors from `default` to `retained`, and Standard Bundle Authoring 2.6 from `unadvertised` to `internal` without changing prior internal rows; catalog fixtures prove exact internal resolution remains available for Standard Bundle Authoring 2.0-2.5 and adds 2.6, while consumer `latest` selection is not applicable to this internal-only family; after pre-GREEN analysis supports MINOR, the exact-candidate `packages check-release --baseline v5.8.0` also reports MINOR including that additive internal row, and the release commit carries version 5.9.0, regenerated `uv.lock`, dated changelog, release-current documentation, and classified historical references; `main` is an ancestor and the commit is fast-forwardable without tree change; every predecessor digest is unchanged; fresh/all-predecessor migration matrices include T19's direct Python transforms and detect loss of an effective gate as incompatibility, an unmigrated exact Markdown Tooling 1.8 `@v5` workflow fixture and `latest` outcome matrices converge without pass-to-fail results; catalog-identity fixtures derive package counts, versions, digests, roles, and migration predecessors from family/catalog/migration authorities rather than literals; reconciliation regenerates the enumerated managed root artifacts from sealed successor bytes, the activated root uses explicit `MD060: false`, coherence stays green, and the #27 literal fixture is unchanged; the Python Tooling direct edge materializes only `/ci/performance = true` in this repository's sparse `.standards/config.toml` to preserve its effective gate; every current-train issue has a committed executable ledger row; a preliminary wheel/sdist digest and complete candidate gate supply exact evidence for T16 (TC-T15-001 through TC-T15-004).
- **sub-tasks:**
  - **T15.1 RED** — enumerate every repository artifact managed by the four activated consumer packages before edits, then add exact default-selection, candidate-derived standards index plus complete direct family-level current/historical reference consistency, managed-root artifact regeneration including explicit `MD060: false` and Python `/ci/performance = true`, generated-catalog role transition, predecessor-digest, fresh-adoption, all-predecessor migration, repeated-reconcile, exact-selection/`latest` pass-fail, activated coherence, current-train ledger completeness, release-byte, ancestry/fast-forward, and catalog-identity derivation cases.
  - **T15.2 Verify RED** — confirm all four family-indexed consumer successors and the contract-only Standard Bundle Authoring 2.6 successor, with complete required docs, pass package/integrity/graph/schema/provider-unit/rendered-catalog/payload-projection gates while canonical catalogs omit them and root `latest` still resolves 1.8/1.8/1.4/1.3 with internal Standard Bundle Authoring rows 2.0-2.5 unchanged; prove final activation is one indivisible catalog-only five-package transition.
  - **T15.3 GREEN** — as the sole owner of canonical `catalogs/5.toml` edits, add the four already-final consumer successor identities/digests as defaults, change their four prior defaults to retained, and add the final Standard Bundle Authoring 2.6 digest and role `internal` alongside unchanged 2.0-2.5 internal entries, without changing any payload/family byte; regenerate the resulting packaged catalog projection and `standards/catalog.md` role column, verify the unchanged payload projection, and validate final migrations/providers together. Reconcile the enumerated managed root artifacts from sealed successor bytes, including root `.markdownlint.json`, Markdown callers, Agent Handoff surfaces, editor settings, instruction blocks, and the sparse Python `/ci/performance = true` config transform. In the same change, update the current Standard Bundle Authoring references in root `README.md`, `standards/README.md`, `AGENTS.md`, and its T14.0-enumerated direct family-level current-reference documents to the canonical internal 2.6 selection and pass T14's repository-wide sweep. Update the standards index and the exact T14.0-enumerated direct family-level current-reference corpus for every family whose derived current reference changes at activation—the four consumer successor families plus internal Standard Bundle Authoring—while preserving explicitly classified historical references and the immutable-payload disjointness invariant. Add current-train ledger rows; require exact internal-resolution fixtures for 2.0-2.6 and the exact-candidate release check to classify the five-package catalog transition as MINOR, then assemble the release commit with version 5.9.0, regenerated `uv.lock`, dated changelog, and release-current docs; do not tag, publish, close issues, or alter hosted state.
...
- **goal:** Add the minimum generic, explicitly opted-in direct package-upgrade configuration transform and use it to preserve Python Tooling's effective performance lane from every qualifying family-indexed predecessor into 1.9. · **phase:** P2/P3 prerequisite · **depends_on:** [T5, T8] · **requirements:** [FR-017, FR-019, FR-021, NFR-002, NFR-004, NFR-008, NFR-009, IR-004, IR-005] · **priority:** must
- **files:** `src/project_standards/package_contract/payload.py`; generated `src/project_standards/schemas/standard-payload.schema.json`; `src/project_standards/control_plane/resolution.py`, `planner.py`, `schemas.py`, `config_edit.py`, and bounded nested-inline-leaf support in `adapters/toml.py`; generated `src/project_standards/schemas/reconciliation-plan.schema.json`; `codec.py` only if required to reuse the canonical TOML scalar renderer; the already-prepared narrow SPEC-BA02 rev 0.15 plus immutable Standard Bundle Authoring 2.6 documentation/template successor, family digest, packaged projection, and `standards/catalog.md` unadvertised counterpart, changed during T19.3 only if their executable contract tests require correction; Python Tooling 1.9 `payload.toml`, provider, closed `config-transform-input`/`config-transform-report` schemas, family digest, packaged projection, and generated catalog counterpart; `tests/control_plane/test_schemas.py`, `test_planner.py`, `test_resolution.py`, `test_package_config_edit.py`, and new `test_package_config_upgrade.py`, plus any other existing T5-authored assertion, snapshot, or fixture of the emitted reconciliation-plan schema version; focused payload/provider tests including Python Tooling 1.9 and Standard Bundle Authoring 2.6 contract tests; and `tests/package_compatibility/test_python_tooling_config_upgrade_parity.py`. `cli.py`, `executor.py`, lock schemas, canonical `catalogs/5.toml`, catalog policy, released payloads, dependencies, release files, and other package providers are excluded.
- **interface/data:** add one default-absent configuration-transform JSON-pointer allowlist to an automatic direct package-to-package migration that uses its existing provider binding. The additive field retains payload-manifest schema 1.0; SPEC-VAIC FR-021 authorizes the narrow SPEC-BA02 revision recorded in its revision history, and immutable Standard Bundle Authoring 2.6 documents it without changing another authoring surface. Python Tooling 1.9 declares `migrate-config` as operation `migrate`, kind `python`, phase `plan`, effect `migration-report`, with closed identity/config input and migration-report output schemas; the existing provider runner validates both declared schemas first, and FR-021's identity, pointer, source/target, explicit-value, and idempotence checks then narrow the accepted result further. Package validation walks only declared pointer paths: each ancestor is a directly declared object whose traversed token has identical requiredness, and the shared scalar leaf rejects reference/combinator mediation and validation-profile changes except target scalar-enum supersets plus leaf default/annotation differences. Ancestor aggregate defaults, annotations, siblings, and other constraints grant no authority and are handled by runtime source/target resolution and validation. Planner first proves unchanged raw config target-valid, then builds the bounded source projection: recurse through shared direct object `properties`; omit target-only keys; preserve each shared atomic value as a whole when source-valid; otherwise omit that whole key so source defaults resolve; never filter collections per element; and require complete source validation. Every explicit declared-pointer value and every value outside provider-declared leaves remains unchanged in output. The planner invokes one target provider through the existing runner, binds output to the invocation identity/no-legacy-claims, confines semantic leaf additions to absent declared pointers, treats only missing direct-object containers on a declared path as part of that leaf diff, requires introduced leaves under both schemas, complete output under the target schema, output projection under the source schema, and provider idempotence. Typed value-redacted transform evidence advances reconciliation-plan schema 1.2 to 1.3. Preview and `--check` are read-only; an empty diff records no config action/drift, empty changed pointers, equal pre/post config digests, and no lock desired-config-digest change, while a nonempty diff plans one lexical config action before dependent artifacts and lock. Apply re-plans under the ordinary writer lock. Existing package evidence without an exact authoritative applied version, including inferred-only evidence, and more than one applicable transform fail closed.
- **acceptance:** synthetic generic fixtures prove valid opt-in, missing/wrong provider and invalid edge rejection, exact pointer minimality with legitimate empty diffs that record empty changed pointers and equal pre/post digests while planning no config action/drift and preserving the lock desired-config digest, declared-container leaf semantics, changed-pointer rejection for target-only/reference/combinator/range/type/rename/relocation/collection evolution, bounded shared-direct-object projection, source-valid atomic preservation, whole-key source-invalid omission with source-default resolution, unchanged raw/output target values, no per-element collection filtering, remaining required/cross-constraint failure, source/target validation, same-change successor-only unrelated keys and explicit target-only declared enum preservation, provider identity/failure/invalid-output rejection, provider idempotence using a source projection recomputed from candidate output, typed no-value preview/check/apply and direct programmatic parity, stale-config CAS, unknown/inferred applied-version recovery guidance, multiple-transform rejection, config-first order, post-config-publication fault/resume, and second-pass fixed point. The candidate emits reconciliation-plan schema 1.3 in every current plan assertion/snapshot; decoders recognize 1.1 and 1.2 as prior versions and reject unsupported versions. A raw target-invalid consumer value on an opted-in edge uses the existing ordinary configuration diagnostic and remedy, not a transform finding. A combined opted-edge version bump plus target-valid consumer edit that remains invalid under complete source projection fails before writes with the ordered upgrade-then-adopt remedy, and the two-step sequence converges without changing the explicit declared-pointer value. Fresh adoption, same-version selection, non-opted direct edges, manual edges, and indirect/multi-hop paths each prove no transform invocation. Lexical tests cover nested inline, dotted, and table forms without unrelated-byte normalization. Python Tooling tests classify every family-indexed predecessor from its direct option declaration and resolved empty source config, stop on a declared absent/non-boolean/underivable result, prove every no-edge version renders no performance gate from empty config, require each qualifier and only each qualifier to have a newly declared direct 1.9 edge limited to `/ci/performance`, require package graph and changed-pointer declaration eligibility to admit that exact edge set including 1.6's unrelated historical array evolution, prove a 1.6 object-form source-root survives raw/output config while the provider receives the source default, and compare source-rendered artifacts/gates before and after, stopping under AW-005 on a no-edge performance gate or any unrelated delta: fresh 1.9 stays false and invokes no transform; enabled absent states materialize true; explicit true/false is unchanged; disabled CI produces an empty diff; same-change `build_backend = "none"` and new Ruff/coverage options remain unchanged; unrelated config bytes and target defaults remain absent; explicit true retains pytest exit 5. For every qualifier, the candidate versus characterized predecessor outcome differs only by transform evidence and the possible single config action: migration authorization, non-config actions, rendered artifacts, and gates remain equivalent. One source/extracted-wheel case proves identical preview, final config, artifacts, lock, and fixed point (TC-T19-001 through TC-T19-003).
...
- **sub-tasks:**
  - **T19.0 CHARACTERIZE** — verify T5's reconciliation-plan 1.2 and shared redaction projection as prerequisites; prove current direct version declarations lose providers during resolution and the staged V4 test bypasses unified upgrade; classify every family-indexed Python predecessor by direct option declaration and resolved empty config, derive the qualifying set, and prove each no-edge version renders no performance gate from empty config; confirm package graph and changed-pointer declaration eligibility admit the exact direct incoming edge set; characterize SPEC-BA02/SBA 2.5 as the immutable manifest-authoring authority requiring 2.6; identify the existing `ControlAction`/`PlannedTarget`, executor tuple-order publication and writer-lock re-plan, and ordinary CLI drift path that carry the config action without `executor.py` or `cli.py` changes; and perform a delta classification of Standard Bundle Authoring 2.6 plus its planned internal catalog transition, the additive `configuration_transform` field, reconciliation-plan 1.3, and the direct migration edges. Reproduce nested-inline TOML leaf limitations and current absence of typed config evidence. If the existing seam cannot carry config planning, drift reporting, and writer-lock re-plan without `executor.py`, `cli.py`, or lock-schema edits, stop before RED and return to SPEC-VAIC under AW-005. If any no-edge predecessor renders the performance gate, any qualifying predecessor fails graph or changed-pointer eligibility, or source-rendered behavior differs outside `/ci/performance`, return to SPEC-VAIC under AW-005; any non-MINOR delta classification returns under AW-004 for owner disposition. Do not narrow the set, widen the allowlist, or substitute multi-hop behavior. Python Tooling 1.6's unrelated array evolution is admitted because its source-invalid object form is omitted only as a whole key from the provider source view and remains unchanged in target config.
  - **T19.1 RED** — SPEC-BA02 rev 0.15 and the Standard Bundle Authoring 2.6 payload/docs were prepared under the owner-authorized rev 0.4-rev 0.12 convergence before resumed T19 RED; treat them as declared T19 inputs and add their executable contract tests before further production GREEN. Add the exact generic declaration/provider/lifecycle, changed-pointer eligibility, source-valid atomic preservation, whole-key source-invalid omission, unchanged target config, remaining cross-constraint failure, target-only same-change options, typed evidence, lexical config, and Python qualifying-predecessor/five-state/source-wheel tests in the acceptance matrix; keep legacy migration suites unchanged.
  ...
  - **T19.3 GREEN** — implement only the declaration parser/schema, changed-pointer-path eligibility, target-admissibility plus bounded shared-direct-object/whole-key atomic source projection before artifact resolution, existing-provider invocation, pointer/introduced-leaf-schema/target/idempotence validation, typed value-redacted evidence, bounded lexical config plan action, Python Tooling 1.9 provider/direct declarations, and caused generated counterparts. Treat the already-prepared SPEC-BA02 rev 0.15 and Standard Bundle Authoring 2.6 payload as base inputs and change them only if their RED contract tests prove a correction is required.
  ...
  - **T19.6 Verify Task** — repeat scope review; run Ruff, BasedPyright, package/graph/schema/projection/catalog gates, focused and complete required tests, strict spec validate/lint for SPEC-VAIC and SPEC-BA02, plan validation, and one exact extracted-wheel source/parity gate; record the MINOR delta-classification evidence, commit with IDs, and block T14/T15 until green.
...
- `uv run project-standards spec validate docs/specs/2026-07-10-standard-bundle-authoring-v2-spec.md`
- `uv run project-standards spec lint --strict docs/specs/2026-07-10-standard-bundle-authoring-v2-spec.md`
...
- Replay the exact-selection and `version = "latest"` outcome matrices; prove the 1.9 lint-selected corpus is identical to 5.8.0 for every characterized exclusion form, and prove format scope only narrows to that lint corpus.
...
| TC-T3-001 | FR-002, NFR-001 | T3 | `.vscode/settings.json`, `.claude/settings.json`, and parametrized JSON/JSONC composition-to-Prettier fixed point | property/regression |
...
| TC-T9-001 | FR-015, NFR-001, NFR-008 | T9 | Provider/workflow/shell/tool causal matrix plus unmigrated 1.8 `@v5` outcomes, identical lint corpus, and format-only narrowing | integration |
| TC-T9-002 | FR-015, FR-019, NFR-004 | T9 | Directional format-to-unchanged-lint normalization or no-divergence guard plus Markdown Tooling 1.8 digest | contract |
...
| TC-T14-002 | FR-003, FR-019, NFR-009, IR-004 | T14 | Exact candidate fixture, idempotent projection, historical carve-outs, IR-004 classification, and SBA 2.6 guard | release |
| TC-T15-001 | FR-003, FR-012, FR-019, FR-021, NFR-001, NFR-009, IR-005 | T15 | Catalog-only five-package activation, four consumer defaults, additive Standard Bundle Authoring 2.6 internal row with prior internal resolution retained, synchronized highest-internal current references, generic transforms, MINOR classification, migrations, coherence, and fixed point | integration/release |
...
| TC-T19-001 | FR-017, FR-021, C-009 | T19 | Opt-in/direct-automatic/provider/pointer schema contract; SPEC-BA02 FR-018 and Standard Bundle Authoring 2.6 authoring-parity assertion; changed-pointer-path rejection; unrelated 1.6-style sibling-array evolution admission; bounded shared-direct-object projection, source-valid atomic preservation, whole-key source-invalid omission/source-default resolution, unchanged target values, and no per-element filtering; introduced-leaf source/target validity; ordinary ERR-008 routing for raw target-invalid consumer config; target-only/default-freezing change rejection; non-idempotent and multi-transform rejection; fresh, same-version, non-opted, manual, and indirect/multi-hop bypass | contract/security |
| TC-T19-002 | FR-017, FR-021, NFR-002, IR-004, IR-005 | T19 | Typed value-redacted preview/check/apply/programmatic lifecycle; candidate schema 1.3 emission, 1.1/1.2 prior recognition, and unsupported-version rejection; exact possibly-empty pointer diff whose empty case records equal pre/post digests, no action/drift, and an unchanged lock desired-config digest; inline/dotted/table lexical preservation; config-first order; stale/unknown/inferred authority; provider failure/invalid output; fault/resume; and second-pass fixed point | integration/recovery |
| TC-T19-003 | FR-017, FR-019, FR-021, NFR-004, NFR-008, NFR-009, IR-005 | T19 | Every-family-version performance-gate classification, qualifying-predecessor/five-state/fresh-bypass matrix, same-change successor-only key/enum success, combined target-valid edit rejection with ordered upgrade-then-adopt convergence, source-rendered behavior equivalence, unchanged migration authorization/non-config actions, source/extracted-wheel equivalent config/artifacts/lock/fixed point, and MINOR delta classification including Standard Bundle Authoring 2.6/internal-catalog transition, manifest field, schema 1.3, and direct edges | compatibility/release |
