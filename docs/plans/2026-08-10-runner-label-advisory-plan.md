...
revision: 3
revises_revision: 2
revision_reason: 'authorize only the Project Specification 1.9 provider-input verify enum member and required digest and contract refresh'
pause_reason: ''
...
| --- | --- | --- | --- | --- |
| `request` | normative | Continue autonomously through v5.19.0 and authorize only the Project Specification 1.9 provider-input `verify` operation-enum member, with required digest and focused contract refresh. | 2026-08-10 | §§3–7, 9–12; T3 |
...
| `repo:src/project_standards/control_plane/providers.py::invoke_provider` | current-state evidence | The provider boundary validates the generated `operation = "verify"` input against the declaration's input schema before entrypoint execution. | verified 2026-08-10 | §§4–7, 9–12; T3 |
...
| `repo:standards/project-spec/versions/1.9/schemas/provider-input.schema.json` | current-state evidence | Closed operation enum omits `verify`, so the real provider boundary rejects T3 before the declared verify provider can run. | #62 checkpoint `318946a5`, verified 2026-08-10 | §§3–7, 9–12; T3 |
...
- No configuration key, option default, config-schema shape, severity enum, public finding schema, control-plane conditional, or central package-ID table. T3 may add only `verify` to Project Specification 1.9's provider-input operation enum; no other schema member or shape changes.
...
| T3 owns | Only the #143 extension of the completed #62 Project Specification 1.9 candidate, the provider-input `verify` enum member required to execute it, refreshed digests/projection/tests, and its serialized catalog contribution. |
...
- The successor configuration and public finding schemas retain the same option set, types, defaults, and shape. Project Specification 1.9's provider-input operation enum adds exactly `verify`; every prior member and all other schema bytes remain unchanged. Mechanical successor identity constants and manifest/resource digests may change.
...
Each successor payload owns a verify-phase advisory at its package boundary. A non-empty label selection yields one warning per affected enabled caller, with the provider invocation supplying the package/version identity and the finding naming the caller path and the reason labels are unreachable. Managed caller mode with non-empty labels remains clean; empty/unset labels remain clean for every ownership/mode combination. Existing error findings, rendering, and reconciliation semantics remain unchanged. Project Specification 1.9 admits the real `verify` request by adding only that literal to its existing provider-input operation enum.
...
| Project Specification | 1.8 has no verify provider; #62 composes 1.9 independently, but its input schema rejects `verify`. | The completed #62 1.9 candidate gains #143's reachability verify contract and only the required input-operation enum member. | #62 lint/coverage behavior, conformance proof, every prior operation, all other schemas, and 1.8 bytes. |
...
| D-004 | Add verify declarations to the two payloads that lack them. Markdown Frontmatter reuses its existing schemas unchanged; Project Specification 1.9 adds only `verify` to its closed provider-input operation enum and leaves its findings and every other schema unchanged. | Reconcile executes verify-phase providers for every selected payload; direct provider-boundary validation proves Project Specification cannot execute that request without the enum member. | planner/executor/provider-boundary current-state evidence; direct owner authorization | T2–T3 |
...
| REQ-005 | The #62-produced Project Specification 1.9 candidate shall gain the verify advisory and add only `verify` to its provider-input operation enum without changing conformance lint/coverage, findings-schema, or other schema contracts; Project Specification 1.8 shall remain byte/mode immutable. | issue #143; #62 T3 contract; `request` | Must | T3 | T3, T4 | PV-T3-001, PV-T4-001 |
...
- **consumes:** [verified EG-002 checkpoint, project-spec-1.9-lint-v1, complete unadvertised 1.9 source/projection/family/catalog facts, T1/T2 companion candidates, existing findings schema, provider-input schema requiring the authorized enum-only extension]
...
- **preserves:** [all Project Specification 1.8 bytes/modes, #62 conformance codes/checks/loci/line/severity and human/JSON behavior, every prior provider-input operation, 1.9 findings/config/content/mutation/migration schemas, all non-allowlisted 1.9 templates and resources, Catalog 5/self-host 1.8 selection]
- **invariants:** [no second Project Specification version directory, no edit before EG-002, provider-input schema adds exactly verify and still rejects unknown operations, verify provider remains separate from document validate/lint providers, PV-T3-001 from the #62 plan reruns after the extension]
...
- **files:** [`standards/project-spec/versions/1.9/providers/project_spec.py` (modify after EG-002; owner T3), `standards/project-spec/versions/1.9/schemas/provider-input.schema.json` (add only the verify operation enum member after EG-002; owner T3), `standards/project-spec/versions/1.9/payload.toml` (modify after EG-002; owner T3), `standards/project-spec/versions/1.9/{README.md,adopt.md,agent-summary.md}` (modify only for #143 owner truth; owner T3), `src/project_standards/payloads/project-spec/1.9/**` (validate/refresh corresponding symlink projection after EG-002; owner T3), `standards/project-spec/standard.toml` (refresh 1.9 digest after EG-002; owner T3), `tests/package_contract/test_project_spec_1_9.py` (modify after #62 T3 ownership transfer; owner T3), `standards/catalog.md` (modify through renderer; owner T4)]
- **parallel_safe:** no
- **conflicts_with:** [T1, T2, T4]
- **supersedes:** []
- **superseded_by:** []
- **evidence:** [ephemeral]
- **recovery:** if EG-002 is absent or its candidate differs from the declared contract, block before writes. If any schema change exceeds the single authorized `verify` enum member or an unknown operation becomes valid, revert T3 before further proof. After writes, revert only #143's 1.9 delta or append a correction task; never recopy 1.8 over the #62 candidate, weaken conformance proof, create 1.10, or activate the package.
...
- **sub-tasks:**
  ...
  - **T3.2 RED** — extend the existing 1.9 contract with direct real-boundary verify cases first; expected failures are the input-schema rejection of `verify` and the missing verify declaration/advisory while #62 conformance tests remain green.
  - **T3.3 Verify RED** — run the focused real-boundary and 1.9 selection proof; confirm `verify` fails only the closed operation enum, every prior operation remains valid, an unknown operation remains invalid, and no candidate/import/conformance drift explains the failure.
  - **T3.4 GREEN** — add only `verify` to the provider-input operation enum, add the config-only verify provider/declaration and concise #143 versioned owner truth, then refresh only affected resource/manifest/family digests, projection validation, and generated catalog facts.
  - **T3.5 Verify GREEN / REFACTOR** — prove the real-boundary invocation and reachability matrix; assert the enum delta is exactly one member and unknown operations still fail; rerun complete #62 PV-T3-001 source/candidate contract; reject any findings/config/other-schema, conformance, template, output, or default-selection difference.
...
| A-003 | Project Specification 1.9's config-only verify request is expressed by the authorized single `verify` operation-enum addition; Markdown Frontmatter and every findings schema remain unchanged. | If real-boundary proof requires any broader schema or engine change, pause T3 and return to owner authority rather than widening the interface implicitly. |
...
- Released 1.14, 1.10, and 1.8 payload bytes/modes/digests remain exact; configuration option sets/defaults and managed render behavior do not change. Project Specification 1.9's provider-input enum adds exactly `verify`, retains every prior operation, rejects an unknown operation, and every other 1.9 schema remains byte-exact.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `runner-label-reachability-v1` | T4 aggregate; T1–T3 package implementations | package users, reconcile CLI, T4 integration | Non-empty labels are silent when ownership/mode bypasses caller input. | Warning per affected enabled caller; package/version from invocation; path/identity names caller; cause/remedy distinguishes consumer-owned from self-hosted. Project Specification 1.9 admits the real request through only the `verify` input-operation enum member. | Empty labels, managed caller mode, and disabled Markdown Tooling tool are silent; existing error findings remain errors; unknown operations remain invalid. | Provider-owned semantics; no config/findings/control-plane change; deterministic finding ordering/cardinality; every other 1.9 schema unchanged. | issue #143; provider/executor contracts; authorized enum delta |
...
| PV-T3-001 | REQ-001, REQ-002, REQ-005 | T3 | serialized provider/package compatibility contract | EG-002 candidate digest and #62 PV-T3-001; released 1.8 digest/bytes/modes; real provider-boundary schema validation; exact pre-T3 schema corpus | Run the updated `tests/package_contract/test_project_spec_1_9.py`, a direct real-boundary verify invocation, all #62 T1–T3 focused tests/proofs, T1/T2 regressions, and five package validators. | 1.9 adds exactly the reachability verify contract and `verify` input-operation member; real invocation succeeds; prior operations remain valid; unknown operation fails; silent controls pass; conformance codes/checks/output, every other schema, and exact-selected profiles remain unchanged; 1.8 exact; 1.9 complete/unadvertised. | Remove conformance `checks`; alter warning/strict behavior; emit advisory from lint; omit or broaden the operation enum; admit an unknown operation; pass empty/managed labels; mutate another schema or 1.8; create 1.10 or activate 1.9. | local bootstrapped source and candidate runtime after EG-002 | ephemeral |
