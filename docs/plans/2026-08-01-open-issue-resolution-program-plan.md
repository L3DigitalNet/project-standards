...
revision: 6
revises_revision: 5
revision_reason: 'authorize the exact consolidated v5.19 activation, release-prep, current-reference, publication, and closeout surfaces without changing the final acceptance boundary'
pause_reason: ''
source: 'GitHub open-issue inventory frozen 2026-08-01T09:21:01Z; owner-approved release decisions; owner-authorized format-3 migration and 2026-08-11 v5.19 consolidation and claim corrections'
spec_ref: ''
created: 2026-08-01
updated: 2026-08-11
owners:
...
| --- | --- | --- | --- | --- |
| request | normative | owner-approved issue boundary, prioritization, release policy, format-3 migration, the 2026-08-04 decision to fold adr@1.4 into v5.15.0, and the 2026-08-11 directions to consolidate #62/#55 in v5.19 and authorize the exact release and closeout claims | 2026-08-11 | §§1–13, T1–T43 |
...
| `repo:meta/versioning.md` | decision | accepted release classification, clean-main preparation, exact release verification, publication, and immutability contract | current at `b478dff2` | §§3, 5, 10, T8, T29–T30, T35–T36, T41, T43 |
| `repo:AGENTS.md` | normative | repository policy that completed implementation plans are deleted only after their durable outcomes are harvested | current at `b478dff2` | §§10–13, T29 |
| `repo:ROADMAP.md#5190` | decision | v5.19 is one consolidation train and orders #62 before #55 inside the Project Specification feature phase | current at `b478dff2` | §§3–12, T25–T29, T42–T43 |
| `repo:catalogs/5.toml` | current-state evidence | Catalog 5 retains the four completed successor payloads as unadvertised before the release qualifier promotes them | current at `b478dff2` | §§4–12, T43 |
| `repo:standards/catalog.md` | current-state evidence | generated package inventory identifies agent-handoff 1.12, markdown-frontmatter 1.11, markdown-tooling 1.15, and python-tooling 1.14 as the four unadvertised successors | current at `b478dff2` | §§4–12, T43 |
| `repo:tests/package_contract/test_current_catalog_activation.py` | current-state evidence | release-facing activation oracle and prior-release baseline for the final Catalog 5 promotion | current at `b478dff2` | §§7–12, T43 |
...
| `repo:docs/plans/2026-08-10-github-workflow-1-2-guidance-plan.md#t2-activate-12-and-prove-release-facing-package-selection` | decision | terminal GitHub Workflow 1.2 activation supplies a separate v5.19 checkpoint and defers its repository-wide gate to final release preparation | revision 4 | T43 |
...
| C-007 | T43 may land one clean preparatory activation/current-reference commit before `release_prep.py`; the final release commit must contain that commit and all release-prep changes before the sole candidate build and repository-wide gate. | request; `repo:meta/versioning.md#release-requirements` | T43 |
...
<!-- release-consistency: historical-characterization standard-bundle-authoring -->

At revision 6 base `b478dff2`, every program task except T43 and T29 is terminal or superseded. The #62 and #55 child outcomes and GitHub Workflow 1.2 activation checkpoint are complete but unpublished, and the published tool version remains 5.18.0. Catalog 5 contains exactly four completed successors that remain unadvertised: agent-handoff 1.12, markdown-frontmatter 1.11, markdown-tooling 1.15, and python-tooling 1.14. The release-facing current-reference review has eight bounded findings: four GitHub Workflow 1.1 references and three Project Specification 1.8 references in `README.md`, plus one Standard Bundle Authoring 2.0 current-reference classification in its specification. A reconcile preview over the four promotions reports three tracked updates — the Markdown Frontmatter delivered skill, consumer catalog snapshot, and agent summary — while the lock advances only on apply. No release-prep, reconciliation apply, final candidate, repository-wide gate, hosted run, publication, or closeout mutation has occurred under T43/T29.

### 4.2 Target State

The durable master validates as `plan_format: 3`; the byte-identical repository-local bridge preserves completed history and drives the remaining graph. T43 first lands one clean preparatory activation/current-reference commit that promotes exactly the four successors, updates exactly the eight bounded current references and their activation oracles, and applies the reviewed reconcile plan. It then runs release preparation, lands the final v5.19 release commit, and qualifies and publishes that exact commit once. T29 consumes T43's retained receipts to reconcile tracker, ledger, repository, handoff, and completed-plan truth without rebuilding or rerunning release verification.
...
| Later trains | all v5.19 product checkpoints terminal; four successors remain unadvertised and eight current references remain stale | one preparatory activation/current-reference commit → one final v5.19 release commit and qualification/publication → receipt-only closeout | terminal task history, issue product acceptance, one-release consolidation, and owner gates | exact release diff and external approval latency |
...
| Project Spec features | #62/#55 child outcomes complete and unpublished | both terminal checkpoints land in one successor release | `docs/specs/`, `docs/plans/` | T24–T27, T42–T43 |
...
| D-007 | Treat the four completed unadvertised successors, eight observed current-reference findings, release-current files, and reconcile projection as one bounded T43 preparation surface. Permit a reviewable activation/current-reference commit before the mechanical release-prep commit, but build and qualify only the final release commit. | makes the exact mutation boundary auditable without creating a second candidate or gate, and prevents release preparation from silently absorbing unclaimed generated or documentation changes | one monolithic unreviewed release commit and separate per-successor release gates rejected | request; `meta/versioning.md`; reconcile preview | T43 |
...
- **Authoritative commands:** focused pytest/package tests; exact Git diff and catalog-role inspection for the preparatory activation/current-reference commit; reconcile preview/apply/second-preview convergence; package/graph/schema/projection checks; Markdown tooling; Agent Handoff validation/drift; and one final release verification block on the exact v5.19 release commit containing a fresh candidate, direct-local `scripts/verify.sh --full`, the release package checks, and hosted CI. T42, T26, T27, and T29 do not repeat that block.
...
| T43 | Qualify and publish consolidated v5.19 | active | operational | P8 | T5, T22, T27 | REQ-900 | TC-T43-001 | no / sole final candidate, gate, hosted, and publication owner |
...
- **boundary:** deployment
- **depends_on:** [T5, T22, T27]
- **dependency_reason:** ordering-only: T5 and T22 own the shared `UPGRADING.md` and `docs/mcp-server.md` release-current surfaces consumed here; T27's verified unpublished #55 conversion surface must precede release; T27 transitively follows T42, while T43 preflight separately requires terminal GitHub Workflow T2 and all other v5.19 checkpoints
- **requirements:** [REQ-900]
- **proof:** [TC-T43-001]
- **source_refs:** [request, repo:meta/versioning.md#release-requirements, repo:ROADMAP.md#5190, repo:catalogs/5.toml, repo:standards/catalog.md, repo:tests/package_contract/test_current_catalog_activation.py::test_catalog_activation__staged_successors__become_canonical_rows, repo:docs/plans/2026-08-01-project-spec-conformance-plan.md#12-final-verification, repo:docs/plans/2026-08-10-github-workflow-1-2-guidance-plan.md#12-final-verification]
- **consumes:** [project-spec-62-unpublished-checkpoint-v1, T27 verified #55 checkpoint, terminal GitHub Workflow T2 activation checkpoint, all other terminal v5.19 task checkpoints, exact release-prep source state, the four completed unadvertised successors, the eight observed current-reference findings, the reviewed reconcile preview, action-specific authorization]
- **produces:** [one clean preparatory activation/current-reference commit, one prepared v5.19 release commit on main containing it, one candidate and release verification receipt, signed immutable full-version tag, moving-major tag, byte-verified wheel/sdist publication, #62/#55 release dispositions, EV-004]
- **preserves:** [every published predecessor and tag, every predecessor catalog entry as retained, predecessor payload bytes, unrelated package roles and generated bytes, unrelated current and historical references, unrelated work, one-release consolidation, action-specific authorization]
- **invariants:** [exactly agent-handoff 1.12, markdown-frontmatter 1.11, markdown-tooling 1.15, and python-tooling 1.14 advance from unadvertised to Catalog 5 defaults while their predecessors become retained; exactly the eight observed current-reference findings are corrected; reconcile changes only its reviewed three update targets plus the apply-generated lock; the preparatory commit and release preparation land on clean main before the candidate build; exactly one fresh candidate and one direct-local full gate cover the final release commit; hosted CI covers that same release commit; no task-level full gate is replayed; tags/assets/issues change only after green proof and authorization; a failed or partially published release is never rewritten]
...
<!-- release-consistency: historical-characterization standard-bundle-authoring -->
- **files:** [`docs/research/2026-08-01-project-spec-feature-release-evidence.md` (create as consolidated v5.19 release evidence; owner T43), `pyproject.toml` (release version and release-owned configuration only; owner T43), `uv.lock` (regenerate from the release version; owner T43), `CHANGELOG.md` (5.19.0 release entry; owner T43), `README.md` (four GitHub Workflow and three Project Specification current references plus adjoining release-current prose only; owner T43), `UPGRADING.md` (5.19.0 exact-version references only; owner T5), `docs/mcp-server.md` (5.19.0 release-current references only; owner T22), `ROADMAP.md` (5.19.0 shipped and next-release state only; owner T43), `tests/package_contract/test_current_catalog_activation.py` (5.19.0 activation baseline and exact four-successor expectations only; owner T43), `tests/fixtures/package_contract/valid/full/expected/catalog.toml` (Catalog 5 activation fixture only; owner T43), `docs/specs/2026-07-10-standard-bundle-authoring-v2-spec.md` (one current Standard Bundle Authoring 2.0 reference to 2.6/classified only; owner T43), `catalogs/5.toml` (promote exactly four successors and retain their predecessors; owner T43), `standards/catalog.md` (regenerated four-successor advertisement state only; owner T43), `.standards/catalog.toml` (reconciled Catalog 5 snapshot only; owner T43), `.standards/lock.toml` (reconciled four-successor lock only; owner T43), `.agents/skills/markdown-frontmatter/SKILL.md` (reconcile-generated 1.11 projection only; owner T43), `.standards/packages/markdown-frontmatter/agent-summary.md` (reconcile-generated 1.11 projection only; owner T43)]
- **parallel_safe:** no
- **conflicts_with:** []
- **supersedes:** [T28]
- **superseded_by:** []
- **evidence:** [EV-004]
- **recovery:** stop before apply if the reconcile plan contains any mutation beyond the three reviewed update targets and the apply-generated lock; before release preparation, correct forward or revert only the preparatory activation/current-reference commit if its exact diff fails; after the final release commit, stop before tags on any local or hosted failure; if an external effect has begun, retain exact receipts, complete only the authorized idempotent step or publish a fully qualified successor, and never move an immutable full-version tag or replace an asset silently
...
- **sub-tasks:**
  ...
  - **T43.2 PREFLIGHT** — validate T42/T27, the terminal GitHub Workflow T2 checkpoint, every other v5.19 checkpoint, clean-main eligibility, target 5.19.0, previous-release baseline, the four unadvertised-successor census, the eight current-reference findings, the three-update reconcile preview plus apply-only lock, signing/hosted prerequisites, and EV-004 recovery boundary.
  - **T43.3 APPLY** — on clean main, promote exactly agent-handoff 1.12, markdown-frontmatter 1.11, markdown-tooling 1.15, and python-tooling 1.14 while retaining their predecessors; correct exactly the four GitHub Workflow, three Project Specification, and one Standard Bundle Authoring current references and their release-facing activation fixtures; regenerate the catalog; apply the reviewed reconcile plan; prove the preparatory diff contains only the claimed paths and land it as a clean preparatory commit; then run `release_prep.py 5.19.0`, apply only its owner-reviewed release-current/roadmap edits, and land the final release commit before building.
...
- **proof:** [TC-T29-001]
- **source_refs:** [request, repo:AGENTS.md, repo:meta/versioning.md#release-requirements]
- **consumes:** [T43 EV-004 release and issue receipts, exact repository and external state after publication, terminal child-plan checkpoints, action-specific closeout authorization]
- **produces:** [sanitized EV-005 closeout evidence, synchronized GitHub ledger, repository status/TODO/handoff truth, retired completed child-plan masters, t29 verified operational result]
- **preserves:** [T43 release and gate receipts, immutable history, predecessor bytes, unrelated work, and action-specific authorization]
- **invariants:** [no external effect before authorization; failed proof cannot be reported as success; T29 consumes T43 receipts and never rebuilds the candidate, reruns local or hosted release verification, redownloads assets, or repeats product proof; only child plans proven complete are deleted; the active program master remains until T29 checkpoint/state completion]
- **executor_discretion:** [bounded evidence formatting and repository-conforming handoff wording that does not alter release or issue truth]
- **files:** [`docs/research/2026-08-01-open-issue-resolution-closeout-evidence.md` (create as sanitized program closeout evidence; owner T29), `docs/GH-WORKFLOWS.md` (regenerate from final issue/PR state; owner T29), `docs/STATUS.md` (record published 5.19.0 and completed program state; owner T29), `docs/TODO.md` (remove completed program work and retain only live work; owner T29), `docs/handoff/state.md` (record current post-release state; owner T29), `docs/handoff/deployed.md` (record T43's published refs and byte-verified assets; owner T29), `docs/handoff/sessions/2026-08.md` (append concise release/program closeout entry; owner T1), `docs/handoff/specs-plans.md` (record terminal child plans and program outcome; owner T29), `docs/plans/2026-08-01-project-spec-conformance-plan.md` (delete after terminal checkpoint/evidence harvest; owner T25), `docs/plans/2026-08-01-project-spec-conversion-plan.md` (delete after terminal checkpoint/evidence harvest; owner T27), `docs/plans/2026-08-10-github-workflow-1-2-guidance-plan.md` (delete after terminal checkpoint/evidence harvest; owner T29)]
- **parallel_safe:** no
- **conflicts_with:** []
- **supersedes:** []
- **superseded_by:** []
- **evidence:** [EV-005]
- **recovery:** stop at the last verified receipt; do not repeat publication, issue writes, hosted verification, or another external effect; retain EV-004 and partial closeout evidence; regenerate ledger/handoff truth idempotently; restore a child-plan master if its terminal checkpoint or durable harvest cannot be proved; never rerun T43 qualification to repair closeout prose
...
- **sub-tasks:**
  ...
  - **T29.2 PREFLIGHT** — confirm T43's immutable EV-004 receipts, final issue/release state, the exact ledger/handoff targets, terminal child-plan checkpoints and durable harvest, rollback boundary, and clean attributable scope.
  - **T29.3 APPLY** — create EV-005, regenerate `docs/GH-WORKFLOWS.md`, reconcile status/TODO/deployed/state/session/spec-plan truth, and delete only the three proven-complete child plan masters; leave the active program master until T29 is checkpointed and terminal.
  - **T29.4 VERIFY** — reconcile issue, release, ledger, roadmap, status, TODO, changelog, handoff, child-plan cleanup, and scratch truth against T43's retained receipts; do not rebuild, rerun gates/hosted CI, redownload assets, or repeat product proof.
...
6. T43 lands one clean four-successor activation/eight-reference preparatory commit, then prepares and publishes one consolidated v5.19 release from the final release commit, running the final local/hosted verification once. T29 consumes those receipts to reconcile the ledger/handoff, retire the completed child plans, and close the program without rerunning verification.
...
| R-008 | Release preparation silently broadens the four promotions, eight current-reference corrections, or reconcile projection. | medium | high | freeze the exact preparatory diff, require a converged second preview, and build only after the final release commit | T43 |
| R-009 | Closeout repeats expensive release proof or deletes a still-active child plan. | low | high | consume EV-004 only and require terminal checkpoint/evidence harvest before each child-plan deletion | T29 |
...
- T43 alone promotes exactly agent-handoff 1.12, markdown-frontmatter 1.11, markdown-tooling 1.15, and python-tooling 1.14; corrects exactly the eight observed current references; applies only the reviewed reconcile projection; prepares the exact v5.19 release commit on main; builds one fresh candidate; runs one direct-local `scripts/verify.sh --full` plus the release package block; obtains hosted green on that commit; and proves classification, publication, asset parity, and recovery. T29 consumes those receipts without rerunning them.
...
- GitHub, releases, status, TODO, roadmap, changelog, ledger, handoff, and completed-plan cleanup truth agree.
...
- **Scratch teardown:** remove format-3 `execution/` only after durable harvest; remove preserved legacy scratch only after its useful evidence is superseded and explicitly reviewed; after T29's checkpoint and terminal transition, complete the sanctioned plan closeout and delete this completed master under repository policy.
...
| Consolidated v5.19 release boundary | T43 | T29 and consumers | terminal v5.19 product checkpoints with four unadvertised successors and eight stale current references | one preparatory activation/current-reference commit followed by one exact-main release commit and one local/hosted qualification for #62, #55, GitHub Workflow T2, the four successor promotions, and every other v5.19 checkpoint | unexpected promotion/reference/reconcile diff, missing checkpoint, failed gate, missing authorization, or partial external effect blocks | published predecessors, retained catalog entries, predecessor payload bytes, and full tags are immutable | one preparatory commit, one final release commit, one candidate, one full gate, one hosted run, one publication receipt | request; `ROADMAP.md`; `meta/versioning.md` |
...
| TC-T43-001 | REQ-900 | T43 | consolidated operational release acceptance | `meta/versioning.md`, `ROADMAP.md`, Catalog 5 roles, terminal T42/T27 and GitHub Workflow T2 checkpoints, the frozen eight-finding census, reviewed reconcile preview, exact prepared main commit, and independently observed hosted/release state | execute T43 authorization/preflight; land the exact four-promotion/eight-reference preparatory commit and converged reconcile projection; prepare 5.19.0 on clean main; build one candidate from the final release commit; run direct-local `scripts/verify.sh --full` once plus the release package/current-reference block; obtain hosted green; publish/byte-verify authorized tags/assets; close #62/#55 | the exact release commit contains every approved v5.19 checkpoint; one local/hosted qualification is green; classification/current references/migration/predecessors pass; signed tags and matching assets publish once; #62/#55 close with release references | promote any package beyond the exact four; retain one of the eight stale references or rewrite another reference; apply an unreviewed reconcile target; omit or mismatch T42, T27, or GitHub Workflow T2; run a task-level duplicate full gate; build before the final content commit; use a non-main release commit; fail hosted/package/asset proof; or mutate/close before authorization; T43 must stop | clean main plus authorized GitHub release scope; Git-dependent release checks direct local | EV-004 |
| TC-T29-001 | REQ-901 | T29 | operational acceptance | T43 EV-004 plus independently observed repository, GitHub ledger, handoff, and child-plan checkpoint state | execute the T29 receipt-only operational lifecycle and its action-specific approval gates without running any release verification | all 24 issues are closed or explicitly accepted; docs/releases/ledger agree; scratch is harvested and removed. | missing authorization, wrong target, failed external proof, non-idempotent repeat, any rerun of T43 proof, or deletion of a nonterminal/unharvested child plan prevents completion | local plus authorized GitHub/consumer scope | EV-005 |
...
| EV-004 | T43 | docs/research/2026-08-01-project-spec-feature-release-evidence.md | exact preparatory and final v5.19 release commits, four-promotion/eight-reference diff census, reconcile convergence, terminal checkpoint census, one candidate/full-gate/hosted receipt, classification, tags, asset digests, recovery state, and #62/#55 dispositions | no credentials, signing material, private consumer configuration, or raw CI logs | retain the consolidated v5.19 acceptance after scratch teardown without repeating verification |
| EV-005 | T29 | docs/research/2026-08-01-open-issue-resolution-closeout-evidence.md | frozen-issue reconciliation, release mapping, ledger/handoff convergence, completed child-plan cleanup, accepted dispositions, and final status | no credentials, private issue drafts, or unbounded API responses | retain the program completion basis |
