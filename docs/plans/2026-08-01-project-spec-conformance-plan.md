...
| `repo:standards/catalog.md` | current-state evidence | Generated family-version inventory and advertised/unadvertised role projection | `4c6d2b7e` | §§4–5; T3–T4 |
| `repo:.standards/config.toml` | current-state evidence | Self-host selection and exact Project Specification dogfood corpus | `4c6d2b7e` | §§3–5, 10; T4 |
...
- Implementing the separate #143 advisory. Its verified Project Specification provider checkpoint and cross-package readiness are external preconditions to T4 activation; its edit must preserve and rerun this plan's candidate proof.
...
| Catalog/self-host activation | Catalog and self-host resolve 1.8 | Catalog and self-host resolve 1.9 only with a strict-clean selected corpus | `project-spec-1.9-dogfood-v1` | T4 |
| Documentation | Successor docs disclose unverified prose; selected specs carry divergent phrasing | Define checked surfaces/repair and remediate only successor-reported selected loci | 1.9 docs plus selected `.standards/config.toml` corpus | T3, T4 |
...
| Configuration | yes | No new option; Catalog/self-host resolution advances only with corpus convergence | PV-T4-001 | T4 |
| Security / trust | yes | Untrusted document text is parsed and compared without execution or path expansion | PV-T1-001 | T1 |
| Compatibility / migration | yes | 1.8 bytes/behavior and absent-metadata output remain exact; selected specs preserve meaning through repair | PV-T2-001, PV-T3-001, PV-T4-001 | T2, T3, T4 |
| Operations / deployment | no | Publication is outside this child plan | PV-T3-001 | T3 |
| Documentation | yes | 1.9 owner truth matches behavior and selected specs become strict-clean without semantic drift | PV-T3-001, PV-T4-001 | T3, T4 |
...
| D-004 | #143 modifies the unreleased 1.9 candidate between T3 composition and T4 activation, and must preserve/rerun #62 proof; it is not implemented here. | The current roadmap orders the same-train companion before #62 publication and requires one Project Specification cut. | approved design release boundary, issue #143, and `ROADMAP.md` | T3, T4 |
| D-005 | Compose 1.9 before advertising it; activate Catalog 5 and self-host only with selected-corpus remediation. | The repository is itself a strict consumer, so advertising first would create a knowingly red owner state. | `.standards/config.toml` and selected-corpus inventory | T3, T4 |
...
- **Authoritative commands:** focused pytest files named by each task; the five package/graph/schema/projection/catalog checks; candidate-wheel `project-standards validate`; successor `spec lint --strict`; Markdown checks; intermediate `scripts/verify.sh`; final `scripts/verify.sh --full`.
- **Oracles:** immutable profile templates, approved diagnostic/output contract, current 1.8 bytes and package tests, the selected provider's schema-validated structured output, and before/after requirement ID/acceptance/priority plus semantic review of selected-doc repairs.
- **Negative controls:** for each Light, Standard, and Full profile, mutate each canonical surface independently, alter only permitted surrounding prose, and remove the exact requirement prefix; also return no provider metadata, run selected 1.8 and legacy paths, and deliberately change a predecessor byte in the proof fixture.
- **Test layers:** unit parsing/comparison, CLI contract, selected-provider integration, V2 package contract, selected-corpus migration/semantic review, source/candidate/installed routing, repository static/package gates, and full serial verification.
- **External environments:** none for child-plan acceptance. The full gate runs locally because repository tests inspect Git metadata that rexec does not synchronize.
- **Evidence:** repeatable T1–T3 output is ephemeral. T4 retains the expensive semantic migration receipt as `EV-001`; parent T25 retains release, hosted, artifact, and issue-closure evidence in `EV-009`.
...
- **superseded_by:** []
- **evidence:** [EV-001]
- **recovery:** if inventory, semantic review, reconcile, strict lint, or full verification fails, restore the pre-activation catalog/root/self-host/corpus checkpoint as one unit; leave the unadvertised T3 candidate intact for forward repair and never weaken the linter.
- **acceptance:** PV-T4-001 proves the #143 prerequisite is checkpointed, the selected path set is unchanged from precheck, every successor finding is repaired, every changed requirement retains its ID/meaning/rationale/acceptance/priority and gains truthful revision history, strict lint and validate are clean through source/candidate/installed paths, catalog rendering and activation/reconcile converge, and the full local gate passes.
- **sub-tasks:**
  - **T4.1 PRECHECK** — verify the #143 Project Specification checkpoint/cross-package readiness, freeze the selected path set and hashes, snapshot Catalog/family-root/config/lock state, run 1.9 strict lint without mutation, and classify every finding by surface/row; block before writes if #143 is absent.
  - **T4.2 SNAPSHOT** — retain an ephemeral per-requirement before table keyed by document and ID with requirement, rationale, acceptance, and priority for post-edit semantic comparison.
  - **T4.3 APPLY** — activate 1.9 in Catalog/family navigation, render `standards/catalog.md`, repair only reported selected loci, add truthful revision rows, and reconcile self-host state with the candidate runtime.
  - **T4.4 VERIFY** — rerun successor lint/validate and compare every changed requirement against the before table; independently review meaning, acceptance, priority, and lifecycle preservation.
  - **T4.5 PROVE RECOVERY** — in an isolated fixture/worktree, show a failed semantic or reconcile precondition leaves/restores the pre-activation unit and a second clean reconcile is a no-op.
  - **T4.6 CAPTURE EVIDENCE** — write sanitized EV-001 with selected-path hashes/counts, finding/repair totals, per-document semantic-review disposition, #143 checkpoint, catalog/reconcile convergence, and gate results.
  - **T4.7 Verify Task** — run PV-T4-001, all T1–T4 focused tests, five package checks, candidate/installed dogfood, Markdown gates, `git diff --check`, and `scripts/verify.sh --full` locally; create the required checkpoint.
...
- Compatibility period: T3 leaves 1.8 as the default; T4 makes 1.9 default while 1.8 remains retained/selectable.
...
- Recovery proof: PV-T4-001 plus the isolated pre-activation restoration and second reconcile check.

### 10.3 Late Failure and Correction

An integrated, dogfood, semantic-review, or later #143 same-candidate failure blocks T4/T25 acceptance. Append a correction task against the completed owner when necessary, preserve checkpoint history, rerun PV-T1-001 through PV-T4-001, and do not publish until the child plan and parent preflight are green.
...
| R-003 | The required #143 work changes the same unreleased 1.9 provider between T3 and T4. | high | medium | Preserve #62 tests as the compatibility oracle, rerun PV-T3-001 after the edit, and block T4 until its checkpoint is verified. | external #143 owner / T4 |
...
| R-005 | Mechanical prefix repair changes the subject or meaning of hundreds of selected requirements. | high | high | Snapshot every row, edit by ID, compare rationale/acceptance/priority, and require independent semantic review before activation can complete. | T4 |
| R-006 | #143 is not checkpointed before the shared 1.9 activation boundary. | medium | high | T4 blocks before writes; the separately governed #143 workflow must supply its provider checkpoint and cross-package readiness. | external #143 owner / T4 |
...
| --- | --- | --- | --- | --- | --- |
| EV-001 | T4 | `docs/research/2026-08-01-project-spec-conformance-dogfood-evidence.md` | #143 checkpoint/readiness, selected-path/hash inventory, finding/repair counts, per-document semantic-review disposition, catalog/reconcile convergence, and final gate summary | no credentials, unbounded logs, private consumer data, or internal reasoning | Preserve the expensive hundreds-row migration and activation basis for parent T25 and later #143 verification |

## Appendix D. Deferred Work

| Item | Reason Deferred | Follow-up / Reopen Trigger |
| --- | --- | --- |
| #143 advisory in markdown-tooling, project-spec, and markdown-frontmatter | Separate accepted issue and cross-package behavior; this plan only preserves the shared 1.9 candidate seam | Execute and checkpoint #143 between T3 composition and T4 activation, then rerun PV-T3-001 |
