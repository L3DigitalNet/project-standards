...
revision: 3
revises_revision: 2
revision_reason: 'authorize the reconciliation-required tracked Catalog 5 snapshot in T2'
pause_reason: ''
...
| T2 owns / aggregates | Catalog/default activation, mutable ADR-family and standards navigation, this repository's dogfood option/config/tracked catalog snapshot/lock, final form of the shared catalog document and focused contract, frozen-corpus proof, and final full qualification. |
...
| Must preserve | Every ADR 1.5 and earlier byte/mode/digest; absent/empty amendment-field validity; default provider results; three-section behavior; deterministic existing findings; legacy migration semantics; create-only scaffold ownership; unrelated bytes in source catalog, tracked catalog snapshot, config, lock, and managed content. |
...
- Revision-2 T2 stopped before apply when candidate-wheel reconciliation previewed exactly one out-of-claim action: `update .standards/catalog.toml — refresh catalog 5 from 5.18.0 to 5.18.0`. Its seven already claimed authored edits remain preserved; `.standards/catalog.toml`, `.standards/lock.toml`, and `standards/catalog.md` remain untouched at this recovery boundary.
...
The package is first reviewable as an exact-selectable unadvertised candidate. Only after the final v5.19 ADR corpus is frozen and corrected does T2 make 1.6 default, retain 1.5, repoint mutable family documentation, opt this repository into the new guardrail, and reconcile the tracked consumer-catalog snapshot and lock from the same candidate runtime. The tagged v5.17/v5.18 corpora, every 1.5-compatible payload document, and the final v5.19 corpus prove the new default cannot regress released consumers and the opt-in check accepts known-good amendment populations.
...
- **Late failure:** revision 2 already blocked T2 before apply on the catalog-snapshot claim omission. Revision 3 resumes from that preserved boundary. Any later failure blocks the owning task, appends a correction task with `corrects`/`discovered_from`, completes its checkpoint, and reruns the failed focused proof plus the applicable fast/full final gate.
...
- **preserves:** [all 1.6 payload bytes from T1, all 1.5 and earlier bytes/digests/roles except 1.5 default-to-retained role, unrelated source-catalog/config/tracked-catalog-snapshot/lock/managed bytes, consumer-authored ADR/scaffold bytes, release version/tags/GitHub state]
- **invariants:** [external corpus freeze before writes, 1.6 default and 1.5 retained atomically, `version = "latest"` plus explicit `validate_amendments = true`, reconcile only after candidate runtime contains 1.6, `.standards/catalog.toml` equals the candidate-rendered Catalog 5 snapshot and changes only for activation/provenance, create-only scaffold unchanged, zero amendment findings, second reconcile no-op, final full gate last]
...
- **superseded_by:** []
- **evidence:** [ephemeral]
- **recovery:** resume from the recorded pre-apply blocker only after revision 3 is active and the seven claimed authored edits plus untouched `.standards/catalog.toml`, `.standards/lock.toml`, and `standards/catalog.md` boundary are confirmed; if the corpus set is unsettled or its T4 checkpoint is missing, do not begin; if pre-commit activation, reconciliation, corpus, or full-gate proof fails, restore the source catalog, mutable navigation, config, tracked catalog snapshot, lock, shared catalog, and activation assertions together to the T1 checkpoint or forward-repair under an appended correction task—never modify frozen ADRs merely to silence the new provider
...
- **sub-tasks:**
  - **T2.1 PRECHECK** — on recovery, verify revision 3 owns `.standards/catalog.toml`, retain the recorded sole-action preview receipt, and confirm the seven claimed authored edits remain preserved while `.standards/catalog.toml`, `.standards/lock.toml`, and `standards/catalog.md` remain pre-apply; obtain the release coordinator's final ADR-writing-set and checkpoint inventory; verify the corpus-corrections T4 checkpoint and T1 checkpoint/digest, a clean green base, no concurrent owner of shared activation files, and a current candidate runtime containing 1.6.
  ...
  - **T2.3 APPLY** — add the 1.6 default catalog row, retain 1.5, repoint mutable navigation, set only `validate_amendments = true` in the ADR config, rebuild the candidate runtime, run direct-local candidate-wheel reconciliation with `--apply` so it refreshes the tracked Catalog 5 snapshot and lock from that runtime, regenerate the catalog document, and update the focused activation assertions.
  - **T2.4 VERIFY** — inspect resolved config/tracked catalog snapshot/lock and provider request; require zero frozen-corpus findings, exact catalog/family identities, a candidate-rendered snapshot limited to activation/provenance facts, unchanged scaffold/predecessors/unrelated config and managed bytes, and green focused/package/documentation checks.
...
3. After revision 3 activates, T2 rechecks the preserved pre-apply boundary, proves the frozen corpus passes explicit candidate validation before writes, activates 1.6 and self-host configuration atomically, reconciles the tracked Catalog 5 snapshot and lock, proves idempotency, and runs the final full gate direct-local.
...
- Rollback / forward repair: before the T2 checkpoint, restore all activation/config/tracked-catalog-snapshot/lock/navigation/aggregate-test files together to T1; after a checkpoint, preserve history and append a correction. Never edit ADR 1.5, immutable tags, or consumer ADRs to manufacture green.
- Recovery proof: PV-T1-001 preserves predecessor/default compatibility and PV-T2-001 proves the preserved pre-apply boundary, pre-activation corpus validity, atomic source-catalog/config/tracked-snapshot/lock agreement, idempotent reconciliation, and final qualification.

### 10.3 Late Failure and Correction

A failure in T1 prevents candidate completion and activation. Revision 2 left the valid unadvertised candidate and seven claimed T2 authored edits intact while T2 blocked before reconciliation apply; revision 3 authorizes the sole previewed tracked-catalog target and resumes from that boundary. A final corpus or integration failure creates an append-only correction task owned by the responsible surface, with `corrects` and `discovered_from`; after its checkpoint, rerun the failed provider/package proof and the direct-local full T2 gate. An authority change to relationship semantics returns to issue/ADR ownership through a plan revision rather than silently changing stable codes or option defaults.
...
- Catalog 5 marks 1.6 default and 1.5 retained; mutable family and standards navigation names 1.6; `.standards` config/tracked Catalog 5 snapshot/lock resolves 1.6 and both enabled checks; the snapshot contains only candidate-rendered activation/provenance deltas, and the existing create-only scaffold is byte-identical.
...
- Reconciliation's second preview/apply is a no-op, the final diff contains only plan-authorized implementation paths including the one tracked Catalog 5 snapshot, and no unrelated config/catalog/scaffold byte, release metadata/tag/GitHub/handoff/publication mutation, or unresolved correction remains.
...
| `catalog-5-adr-1.6-activation-v1` | T2 | parent v5.19 release | 1.5 default/root/self-host with matching committed Catalog 5 snapshot and lock. | 1.6 default/root/self-host with both checks true; 1.5 retained; tracked snapshot and lock reconciled from the candidate runtime. | Missing corpus-freeze/T4 checkpoint, reconcile drift, extra path or unrelated snapshot byte, or failed full gate blocks. | Atomic activation; create-only scaffold/unrelated config and managed bytes preserved; no publication authority. | request; corpus-corrections plan; Catalog 5 |
...
| PV-T2-001 | REQ-001, REQ-005, REQ-006, REQ-008, REQ-009 | T2 | configuration | T1 checkpoint/digest; release coordinator's final writing-set and corpus-correction T4 checkpoint; Catalog 5 source and tracked consumer snapshot; candidate-wheel provider; immutable tag bytes | Confirm the preserved pre-apply boundary and recorded sole-action preview; precheck external checkpoint; prove explicit provider green; activate/reconcile twice; field-check the tracked snapshot and exact claimed-path diff; run focused/package/static checks through rexec and candidate-wheel/Markdown checks in their declared locations; direct-local rerun tags/diff and `PYTHONPATH="$PWD/build/wheel-runtime" scripts/verify.sh --full`. | 1.6 default/current/self-hosted; 1.5 retained; source catalog, tracked snapshot, config, and lock agree; both checks effective; frozen/tagged corpora zero; scaffold/unrelated state exact; second reconcile no-op; all gates pass. | Omit freeze; leave 1.5 default; omit option/lock/root/snapshot update; mutate an unrelated snapshot field or add an unclaimed path; inject one-way or superseded edge in isolated fixture; mutate scaffold/unrelated config; stale runtime; break no-op. Each gate blocks before checkpoint. | authoritative local checkout for activation/Git/full gate; bootstrapped rexec worker for synchronized-tree-compatible CPU checks | ephemeral |
