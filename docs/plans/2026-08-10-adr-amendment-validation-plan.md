...
revision: 1
revises_revision: 0
revision_reason: 'initial plan from issue 163 and verified ADR 1.5 provider, corpus, and release contracts'
pause_reason: ''
...
| `request` | normative | Select the evidence-backed long-term design: add a new default-false option instead of silently broadening `require_sections`; deliver an unadvertised candidate followed by activation and final verification. | 2026-08-10 | §§1, 3, 5–13; T1–T2 |
...
| `repo:tests/package_contract/test_adr_1_5.py` | current-state evidence | Predecessor immutability, option/provider compatibility, provider invocation, identity, projection, and activation proof pattern. | `e24f50ef` | §§4–7, 9–12; T1–T2 |
...
| `repo:docs/handoff/conventions.md#18-match-verification-to-the-changed-surface` | operational evidence | Candidate payload/test changes require a current candidate runtime and repository gate; final release-prep content uses the full serial battery. | 2026-08-10 | §§7, 9–12; T1–T2 |
| `repo:docs/handoff/conventions.md#22-rexec-v02-is-a-remote-only-root-configured-execution-path` | operational evidence | CPU-intensive synchronized-tree checks run through rexec v0.2; Git/index/history checks remain direct-local because `.git` is never synchronized. | 2026-08-10 | §§3, 7, 9, 12; T1–T2 |
...
- After the external v5.19 corpus-freeze checkpoint, activate 1.6, repoint mutable family navigation, enable the new option in this repository, reconcile the lock and managed state, and prove the frozen corpus has zero amendment findings.
...
| T2 owns / aggregates | Catalog/default activation, mutable ADR-family and standards navigation, this repository's dogfood option/config/lock, final form of the shared catalog document and focused contract, frozen-corpus proof, and final full qualification. |
...
| Must preserve | Every ADR 1.5 and earlier byte/mode/digest; absent/empty amendment-field validity; default provider results; three-section behavior; deterministic existing findings; legacy migration semantics; create-only scaffold ownership; unrelated catalog/config/lock content. |
...
- T1 leaves Catalog 5, `.standards/config.toml`, `.standards/lock.toml`, family-root current-authority pages, and release metadata unchanged. T2 may mutate those activation surfaces only after its external entry gate.
- CPU-intensive synchronized-tree-compatible checks, including focused pytest, Ruff, BasedPyright, and fast/full repository gates, run through `rexec -- ...` or `rexec --shell '...'` when environment composition is required. Git-tag extraction, `git diff`, `git ls-files`, and Git-tracked Markdown selection run direct-local. Reconciliation applies direct-local because it must mutate the authoritative checkout.
...
The package is first reviewable as an exact-selectable unadvertised candidate. Only after the final v5.19 ADR corpus is frozen and corrected does T2 make 1.6 default, retain 1.5, repoint mutable family documentation, and opt this repository into the new guardrail. The tagged v5.17/v5.18 corpora, every 1.5-compatible payload document, and the final v5.19 corpus prove the new default cannot regress released consumers and the opt-in check accepts known-good amendment populations.
...
| Repository dogfood | Latest resolves 1.5 with sections enabled. | Latest resolves 1.6 with both checks enabled. | `.standards/config.toml`; `.standards/lock.toml` | T2 |
...
| D-007 | Audit immutable v5.17.0 and v5.18.0 corpora direct-local and keep CPU-heavy repository verification on rexec. | The tags bracket absence and first use of amendment fields; `.git` cannot exist on the worker. | issue #163; rexec v0.2 convention | T1–T2 |
...
| REQ-009 | Remote-compatible focused/package/static and fast/full gates shall run through rexec v0.2; Git-dependent release-corpus, diff, and tracked-Markdown checks shall run direct-local; T1 shall run the fast gate and T2 the final full gate after their last content changes. | request; repository conventions | Must | T2 | T1, T2 | PV-T1-001, PV-T2-001 |
...
- **Predecessor oracle:** ADR 1.5's aggregate digest, complete file/mode tree, option/provider behavior, examples/templates, migration schema, artifact policy, and current focused test establish the immutable compatibility baseline.
- **Released-corpus oracle:** direct-local Git-tag extraction of `v5.17.0` and `v5.18.0` supplies immutable before/after amendment-vocabulary corpora. The candidate is invoked once with its resolved default and once with amendment validation enabled; both populations must have zero new/amendment findings. Git operations stay local; provider-focused and whole-repository CPU work stays on rexec.
- **Current-corpus oracle:** before T1 checkpoint, remeasure the post-#159/#160/#161 documents and prove zero amendment findings; before T2, consume the release coordinator's frozen checkpoint inventory and rerun over the complete post-correction v5.19 corpus.
- **Negative controls:** remove each reciprocal side independently; reference an absent ID; change an `amends` target to `status: superseded`; couple amendment validation to `require_sections`; enable it by default; restore the generic template hint; mutate one 1.5 byte; stale one 1.6 self-version/digest/link; advertise 1.6 during T1; or leave 1.5 default during T2. Each focused oracle must fail for the intended reason before restoration.
...
- **Documentation/static layer:** run scoped new-file and Git-tracked Prettier/markdownlint checks, `git diff --check`, remote Ruff and BasedPyright, and candidate-wheel `project-standards validate` after T2 dogfood activation.
- **Repository layer:** T1 runs one fast `scripts/verify.sh` through rexec after its last candidate content change. T2 reruns bootstrap after activation/reconciliation and runs one final `scripts/verify.sh --full` through rexec after the last content change. Neither gate substitutes for task-level behavior proof.
- **Evidence:** repeatable output is ephemeral under the generated execution directory. Each checkpoint records tag names, corpus counts/edge counts, aggregate digest, exact commands, exit codes, and concise result summaries; no secret or private downstream content is retained.
- **Late failure:** block the owning task, append a correction task with `corrects`/`discovered_from`, complete its checkpoint, and rerun the failed focused proof plus the applicable fast/full final gate.
...
- **sub-tasks:**
  ...
  - **T1.9 Verify Task** — rerun remote bootstrap after final payload bytes, PV-T1-001, scoped and Git-tracked Markdown checks in their permitted locations, `git diff --check`, authorized name-status/diff inspection, and one rexec fast repository gate; validate this plan and commit with required trailers.
...
- **proof:** [PV-T2-001]
- **source_refs:** [request, issue:L3DigitalNet/project-standards#163, repo:docs/plans/2026-08-10-v519-adr-corpus-corrections-plan.md#phase-p4-corpus-wide-link-and-path-reconciliation, repo:catalogs/5.toml, repo:.standards/config.toml, repo:.standards/lock.toml, repo:standards/adr/README.md, repo:standards/README.md]
- **consumes:** [complete unadvertised adr-1.6-candidate, adr-amendment-validation-v1, adr-1.6-released-corpus-compatibility-v1, release-wide-v5.19-adr-corpus-freeze-v1, corpus-correction T4 checkpoint, current Catalog 5 and self-host state]
- **produces:** [catalog-5-adr-1.6-activation-v1, self-hosted-adr-amendment-guard-v1, adr-1.6-final-qualification-checkpoint-v1]
- **preserves:** [all 1.6 payload bytes from T1, all 1.5 and earlier bytes/digests/roles except 1.5 default-to-retained role, unrelated catalog/config/lock/managed content, consumer-authored ADR/scaffold bytes, release version/tags/GitHub state]
- **invariants:** [external corpus freeze before writes, 1.6 default and 1.5 retained atomically, `version = "latest"` plus explicit `validate_amendments = true`, reconcile only after candidate runtime contains 1.6, create-only scaffold unchanged, zero amendment findings, second reconcile no-op, final full gate last]
...
- **files:** [`catalogs/5.toml` (modify ADR roles/1.6 entry; owner T2), `standards/adr/README.md` (modify current authority; owner T2), `standards/adr/adopt.md` (modify current adoption; owner T2), `standards/adr/agent-summary.md` (modify current summary; owner T2), `standards/README.md` (modify ADR table row; owner T2), `.standards/config.toml` (modify ADR option only; owner T2), `.standards/lock.toml` (reconcile generated selection/config/inventory; owner T2), `standards/catalog.md` (regenerate activated facts; owner T2), `tests/package_contract/test_adr_1_6.py` (modify activation assertions; owner T2)]
- **parallel_safe:** no
- **conflicts_with:** [T1]
- **supersedes:** []
- **superseded_by:** []
- **evidence:** [ephemeral]
- **recovery:** if the corpus set is unsettled or its T4 checkpoint is missing, do not begin; if pre-commit activation, reconciliation, corpus, or full-gate proof fails, restore the catalog, mutable navigation, config, lock, shared catalog, and activation assertions together to the T1 checkpoint or forward-repair under an appended correction task—never modify frozen ADRs merely to silence the new provider
...
- **sub-tasks:**
  - **T2.1 PRECHECK** — obtain the release coordinator's final ADR-writing-set and checkpoint inventory; verify the corpus-corrections T4 checkpoint and T1 checkpoint/digest, a clean green base, no concurrent owner of shared activation files, and a current candidate runtime containing 1.6.
  ...
  - **T2.3 APPLY** — add the 1.6 default catalog row, retain 1.5, repoint mutable navigation, set only `validate_amendments = true` in the ADR config, rebuild the candidate runtime, run direct-local candidate-wheel reconciliation with `--apply`, regenerate the catalog document, and update the focused activation assertions.
  - **T2.4 VERIFY** — inspect resolved config/lock and provider request; require zero frozen-corpus findings, exact catalog/family identities, unchanged scaffold/predecessors/unrelated config, and green focused/package/documentation checks.
  - **T2.5 PROVE IDEMPOTENCY** — preview and reapply reconciliation using the same candidate runtime; require no plan, target, lock, or managed-byte change and rerun the direct-local tagged-corpus compatibility procedure.
  - **T2.6 Verify Task** — rerun remote bootstrap after final catalog/config/payload state; execute PV-T2-001, all five package validators, candidate-wheel `project-standards validate`, remote Ruff/BasedPyright, permitted direct-local Git-tracked Markdown/diff checks, and one final rexec `scripts/verify.sh --full`; inspect the complete authorized diff, validate this plan/checkpoint, and commit with required trailers.
...
3. T2 proves the frozen corpus passes explicit candidate validation before writes, activates 1.6 and self-host configuration atomically, reconciles, proves idempotency, and runs the final full gate.
4. The parent v5.19 release workflow may consume T2's validated checkpoint for release metadata, hosted CI, publication, and synchronized issue closure; this plan grants no such action itself.
...
- Idempotency: schema defaults are deterministic; package projection/catalog rendering are generated fixed points; reconciliation is run twice and the second preview/apply must produce no change.
...
- Rollback / forward repair: before the T2 checkpoint, restore all activation/config/lock/navigation/aggregate-test files together to T1; after a checkpoint, preserve history and append a correction. Never edit ADR 1.5, immutable tags, or consumer ADRs to manufacture green.
- Recovery proof: PV-T1-001 preserves predecessor/default compatibility and PV-T2-001 proves pre-activation corpus validity, atomic selection/config agreement, idempotent reconciliation, and final qualification.

### 10.3 Late Failure and Correction

A failure in T1 prevents candidate completion and activation. A failure after T1 but before T2 leaves the valid unadvertised candidate intact while T2 remains blocked. A final corpus or integration failure creates an append-only correction task owned by the responsible surface, with `corrects` and `discovered_from`; after its checkpoint, rerun the failed provider/package proof and the full T2 gate. An authority change to relationship semantics returns to issue/ADR ownership through a plan revision rather than silently changing stable codes or option defaults.
...
| R-004 | Candidate identity/digests/projection drift or an accidental catalog row advertises 1.6 early. | low | high | Exact predecessor/delta and non-activation controls plus five package validators and generated catalog check. | package owner / T1 |
...
| R-007 | Activation mutates the create-only scaffold or unrelated managed state. | low | high | Snapshot digests/name-status before reconcile, assert exact authorized targets, rerun no-op reconciliation, and fail on any extra path. | activation owner / T2 |
...
- Catalog 5 marks 1.6 default and 1.5 retained; mutable family and standards navigation names 1.6; `.standards` config/lock resolves 1.6 and both enabled checks; the existing create-only scaffold is byte-identical.
- The five package validators, candidate-wheel project validation, focused/provider/predecessor tests, scoped and Git-tracked Markdown checks, Ruff, BasedPyright, `git diff --check`, T1 fast gate, and T2 final full gate all pass in the declared rexec/direct-local locations.
- Reconciliation's second preview/apply is a no-op, the final diff contains only plan-authorized implementation paths, and no release metadata/tag/GitHub/handoff/publication mutation or unresolved correction remains.
...
- **Decisions / deviations harvested:** retain D-001–D-007 and record any approved deviation in the owning issue/plan before execution state is removed; do not rewrite the issue's behavior to match an implementation shortcut.
- **Risks closed / accepted:** close R-001–R-007 from focused/package/corpus/full-gate proof or file a bounded follow-up owned outside release publication.
...
| `catalog-5-adr-1.6-activation-v1` | T2 | parent v5.19 release | 1.5 default/root/self-host. | 1.6 default/root/self-host with both checks true; 1.5 retained. | Missing corpus-freeze/T4 checkpoint, reconcile drift, extra path, or failed full gate blocks. | Atomic activation; create-only scaffold/unrelated config preserved; no publication authority. | request; corpus-corrections plan; Catalog 5 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PV-T1-001 | REQ-001, REQ-002, REQ-003, REQ-004, REQ-005, REQ-006, REQ-007, REQ-009 | T1 | direct provider, V2 package contract, tagged/current corpus compatibility, fast qualification | issue #163 acceptance; ADR 1.5 relationship rules; immutable 1.5 tree/digest; existing provider outputs; v5.17/v5.18 tag bytes | Run `tests/package_contract/test_adr_1_6.py` and 1.5 regression through rexec; direct-local extract/invoke both tags; run five package validators, scoped/Git-tracked Markdown in the declared locations, remote Ruff/BasedPyright, and one rexec fast gate. | Exact stable findings and silent controls pass; 1.6 is complete/unadvertised; 1.5 and tagged/current compatible corpora remain green; all gates pass. | Remove either reciprocal side; use absent ID; supersede target; couple/default-enable option; restore generic hint; mutate 1.5; stale schema/digest/projection; advertise 1.6. Each focused oracle fails for the intended reason. | bootstrapped rexec worker for CPU work; authoritative local Git checkout for tag/diff checks | ephemeral |
| PV-T2-001 | REQ-001, REQ-005, REQ-006, REQ-008, REQ-009 | T2 | configuration | T1 checkpoint/digest; release coordinator's final writing-set and corpus-correction T4 checkpoint; Catalog 5; candidate-wheel provider; immutable tag bytes | Precheck external checkpoint; prove explicit provider green; activate/reconcile twice; run focused/package/candidate-wheel/Markdown/static checks; direct-local rerun tags/diff; run one rexec full gate. | 1.6 default/current/self-hosted; 1.5 retained; both checks effective; frozen/tagged corpora zero; scaffold/unrelated state exact; second reconcile no-op; all gates pass. | Omit freeze; leave 1.5 default; omit option/lock/root update; inject one-way or superseded edge in isolated fixture; mutate scaffold/unrelated config; stale runtime; break no-op. Each gate blocks before checkpoint. | authoritative local checkout for activation/Git; bootstrapped rexec worker for CPU checks/full gate | ephemeral |
