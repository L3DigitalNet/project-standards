...
revision: 4
revises_revision: 3
revision_reason: 'authorize the ADR 1.5 predecessor activation assertion correction in T2'
pause_reason: ''
...
| T2 owns / aggregates | Catalog/default activation, mutable ADR-family and standards navigation, this repository's dogfood option/config/tracked catalog snapshot/lock, final form of the shared catalog document and ADR 1.5/1.6 activation assertions, frozen-corpus proof, and final full qualification. |
...
| Must preserve | Every ADR 1.5 and earlier byte/mode/digest and every ADR 1.5 provider/predecessor behavior assertion; absent/empty amendment-field validity; default provider results; three-section behavior; deterministic existing findings; legacy migration semantics; create-only scaffold ownership; unrelated bytes in source catalog, tracked catalog snapshot, config, lock, tests, and managed content. |
...
- **Documentation/static layer:** run scoped new-file and Git-tracked Prettier/markdownlint checks, `git diff --check`, rexec Ruff/BasedPyright/package validation, and candidate-wheel `project-standards validate` after T2 dogfood activation. Git/index-selected Markdown checks remain direct-local. The final name-status/diff census requires the ten fixed-point paths plus `tests/package_contract/test_adr_1_5.py`, permits only the reconciled `.standards/catalog.toml` snapshot and the predecessor test's two activation assertions within their respective deltas, and rejects every other path or unrelated byte.
...
- **files:** [`catalogs/5.toml` (modify ADR roles/1.6 entry; owner T2), `standards/adr/README.md` (modify current authority; owner T2), `standards/adr/adopt.md` (modify current adoption; owner T2), `standards/adr/agent-summary.md` (modify current summary; owner T2), `standards/README.md` (modify ADR table row; owner T2), `.standards/config.toml` (modify ADR option only; owner T2), `.standards/catalog.toml` (reconcile only the candidate-rendered Catalog 5 snapshot/provenance required by ADR 1.6 activation; owner T2), `.standards/lock.toml` (reconcile generated selection/config/inventory; owner T2), `standards/catalog.md` (regenerate activated facts; owner T2), `tests/package_contract/test_adr_1_6.py` (modify activation assertions; owner T2), `tests/package_contract/test_adr_1_5.py` (modify only predecessor activation assertions: 1.5 retained/exact-selectable and mutable current/default navigation on 1.6; owner T2)]
- **parallel_safe:** no
- **conflicts_with:** [T1]
- **supersedes:** []
- **superseded_by:** []
- **evidence:** [ephemeral]
- **recovery:** resume only after revision 4 is active and the exact ten-path reconciliation fixed point plus `31 passed, 2 failed` focused receipt are confirmed; update only the two stale ADR 1.5 activation assertions, then rerun the same focused command; if the corpus set is unsettled or its T4 checkpoint is missing, do not proceed; if pre-commit activation, reconciliation, corpus, focused, or full-gate proof fails, restore the source catalog, mutable navigation, config, tracked catalog snapshot, lock, shared catalog, and ADR 1.5/1.6 activation assertions together to the T1 checkpoint or forward-repair under an appended correction task—never change predecessor/provider behavior or frozen ADRs merely to silence proof
...
- **sub-tasks:**
  - **T2.1 PRECHECK** — on recovery, verify revision 4 owns the predecessor test, retain the exact ten-path fixed-point receipt and the focused `31 passed, 2 failed` result, and confirm no unclaimed edit or full gate followed; obtain the release coordinator's final ADR-writing-set and checkpoint inventory; verify the corpus-corrections T4 checkpoint and T1 checkpoint/digest, no concurrent owner of shared activation files, and a current candidate runtime containing 1.6.
  ...
  - **T2.3 APPLY** — add the 1.6 default catalog row, retain 1.5, repoint mutable navigation, set only `validate_amendments = true` in the ADR config, rebuild the candidate runtime, run direct-local candidate-wheel reconciliation with `--apply` so it refreshes the tracked Catalog 5 snapshot and lock from that runtime, regenerate the catalog document, and update both the ADR 1.6 activation assertions and the two predecessor activation assertions without changing predecessor behavior.
  - **T2.4 VERIFY** — inspect resolved config/tracked catalog snapshot/lock and provider request; rerun the exact focused ADR 1.6/1.5 command and require all 33 tests to pass, with 1.5 retained/exact-selectable and mutable current/default navigation on 1.6; require zero frozen-corpus findings, exact catalog/family identities, a candidate-rendered snapshot limited to activation/provenance facts, unchanged scaffold/predecessor behavior/unrelated tests/config/managed bytes, and green package/documentation checks.
  ...
  - **T2.6 Verify Task** — rerun remote bootstrap after final catalog/config/payload state; execute PV-T2-001 with the focused ADR 1.6/1.5 tests, all five package validators, and Ruff/BasedPyright on rexec, candidate-wheel `project-standards validate`, permitted direct-local Git-tracked Markdown/diff/tag/corpus checks, and one final direct-local `PYTHONPATH="$PWD/build/wheel-runtime" scripts/verify.sh --full`; inspect the exact eleven-path authorized diff, including only the claimed tracked-catalog snapshot and two predecessor activation-assertion deltas within their files, validate this plan/checkpoint, and commit with required trailers.
...
3. After revision 4 activates, T2 resumes from the exact ten-path reconciliation fixed point, updates only the two stale predecessor activation assertions, reruns the focused ADR 1.6/1.5 proof to green, and then runs the final full gate direct-local.
...
- Rollback / forward repair: before the T2 checkpoint, restore all activation/config/tracked-catalog-snapshot/lock/navigation/ADR 1.5 and 1.6 activation-test files together to T1; after a checkpoint, preserve history and append a correction. Never edit ADR 1.5 payload/provider behavior, immutable tags, or consumer ADRs to manufacture green.
- Recovery proof: PV-T1-001 preserves predecessor/default compatibility and PV-T2-001 proves the exact ten-path fixed point, the bounded predecessor-test correction, pre-activation corpus validity, atomic source-catalog/config/tracked-snapshot/lock agreement, idempotent reconciliation, and final qualification.

### 10.3 Late Failure and Correction

A failure in T1 prevents candidate completion and activation. Revision 2 left the valid unadvertised candidate and seven claimed T2 authored edits intact while T2 blocked before reconciliation apply; revision 3 authorized the sole previewed tracked-catalog target and reached an exact ten-path fixed point; revision 4 authorizes the two stale predecessor activation assertions exposed by the `31 passed, 2 failed` focused receipt. A final corpus or integration failure creates an append-only correction task owned by the responsible surface, with `corrects` and `discovered_from`; after its checkpoint, rerun the failed provider/package proof and the direct-local full T2 gate. An authority change to relationship semantics returns to issue/ADR ownership through a plan revision rather than silently changing stable codes or option defaults.
...
- Catalog 5 marks 1.6 default and 1.5 retained/exact-selectable; mutable family and standards navigation names 1.6; the predecessor activation assertions agree without changing ADR 1.5 provider/payload behavior; `.standards` config/tracked Catalog 5 snapshot/lock resolves 1.6 and both enabled checks; the snapshot contains only candidate-rendered activation/provenance deltas, and the existing create-only scaffold is byte-identical.
...
- Reconciliation's second preview/apply is a no-op, the focused ADR 1.6/1.5 command reports 33 passed, the final diff contains only plan-authorized implementation paths including the tracked Catalog 5 snapshot and two predecessor activation assertions, and no unrelated config/catalog/test/scaffold byte, release metadata/tag/GitHub/handoff/publication mutation, or unresolved correction remains.
...
| PV-T2-001 | REQ-001, REQ-005, REQ-006, REQ-008, REQ-009 | T2 | configuration | T1 checkpoint/digest; release coordinator's final writing-set and corpus-correction T4 checkpoint; Catalog 5 source and tracked consumer snapshot; ADR 1.5 predecessor contract; candidate-wheel provider; immutable tag bytes | Confirm the exact ten-path fixed point and `31 passed, 2 failed` focused receipt; update only the stale predecessor role/navigation assertions and require the same focused command to report 33 passed; field-check the tracked snapshot and exact eleven-path diff; run package/static checks through rexec and candidate-wheel/Markdown checks in their declared locations; direct-local rerun tags/diff and `PYTHONPATH="$PWD/build/wheel-runtime" scripts/verify.sh --full`. | 1.6 default/current/self-hosted; 1.5 retained/exact-selectable with provider/predecessor behavior exact; source catalog, tracked snapshot, config, lock, and mutable navigation agree; both checks effective; frozen/tagged corpora zero; scaffold/unrelated state exact; second reconcile no-op; all gates pass. | Keep either stale 1.5 default/navigation assertion; change any ADR 1.5 provider/payload behavior test; omit freeze; omit option/lock/root/snapshot update; mutate an unrelated snapshot field/test or add an unclaimed path; inject one-way or superseded edge in isolated fixture; mutate scaffold/unrelated config; stale runtime; break no-op. Each gate blocks before checkpoint. | authoritative local checkout for activation/Git/full gate; bootstrapped rexec worker for synchronized-tree-compatible CPU checks | ephemeral |
