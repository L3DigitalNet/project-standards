...
size: full
status: complete
source: 'SPEC-MS01 revision 1.5 and SPEC-RD01 revision 1.6'
...
- **files:** `docs/STATUS.md` (modify), `docs/handoff/state.md` (modify), `docs/handoff/specs-plans.md` (modify), `docs/handoff/sessions/{YYYY-MM}.md` (modify), `docs/TODO.md` (modify — close T12, re-scope the MCP hold), `docs/research/2026-07-28-project-standards-mcp-protocol-sdk-client-matrix.md` (modify — §13 mandates appending the final-candidate smoke evidence), `docs/specs/2026-07-07-project-standards-mcp-server-implementation-spec.md` (modify — owner-authorized revision 1.5 correcting the §9 field names and §10.3 EC-005 wording, 2026-07-31), `docs/specs/README.md` (modify — revision index), `docs/plans/2026-07-24-project-standards-mcp-server-plan.md` (close-out only), `.project-pipeline/2026-07-24-project-standards-mcp-server/` (close-out only)
...
- **Completed:** 2026-07-31 · final commit: the commit introducing this close-out · final candidate wheel SHA-256: `8ed0b2e8838fcc67a13bc62a82b52791e5b6b37104b7494c8dad16d04b17e07f` · state: **implementation verified; release unprepared and unpublished**.

  All 15 tasks are done as of this close-out commit — the 12 authored tasks plus discovered work T13, T15, and T14. The candidate is identified by its wheel digest and this commit, never by its version string: the candidate and the already-published MCP-less release both report `5.11.0`, so the version alone cannot distinguish them (freeze S-B). No version was bumped or finalized, no tag created, no versioned `CHANGELOG` section opened, and nothing was published; the feature sits under `Unreleased` only. This state is distinct from a separately authorized _prepared but unpublished_ release and from a _published_ release, neither of which exists for the MCP server.

  T12.0's traceability audit (`logs/T12.0.txt`) returned CLEAN: 137 of 137 Appendix B test targets resolved, 64 of 66 `TC-*` rows evidenced (the two open rows being T12's own), 61 of 61 source requirements covered by a done task that claims them, 88 of 90 checklist sub-task rows done each with a resolving evidence pointer, 74 lettered freezes across ten groups all dispositioned, nine of nine spec open questions resolved, and six of six §11 risks closed. `FR-010` is satisfied by its test-backed explicit omission (TC-T10-005), not by a waiver.

- **Deviations / decisions harvested from notes:**
  - **Reviewed byte ceilings are test constants, not a contract freeze.** The `MAX_*_BYTES` values in `tests/mcp_server/contract/test_protocol_conformance.py` and `tests/mcp_server/integration/test_server.py` are reviewed test constants at roughly twice the measured baselines, regression-shaped, and ratified as such by the owner on 2026-07-30. They are **explicitly not a contract freeze**: NFR-002 and NFR-012 authorize _a bound_, and no binding document names a number. Changing a ceiling is a reviewed test change, not a contract amendment, and no ADR or spec revision is owed for them.
  - **File-list deviations.** Every task that touched a file outside its declared `files:` row recorded the reason before doing so. All are one recurring class — the frozen facade methods live in `catalog.py`, shared DTO bases cannot stay private across modules under BasedPyright's `reportPrivateUsage`, and new test directories need `__init__.py` markers for the `tests.*` import convention. No control-plane file was touched by a services task and no authoritative semantics changed. The T9, T10, and T11 rows were reconciled with the tree at `b3bcfd5`.
  - **`.github/workflows/check.yml` was deliberately not modified.** The plan permits modification only if current discovery does not already reach the MCP suites. It does, proven by `test_ci_discovers_the_mcp_suites_without_a_second_workflow` running the exact CI command under `--collect-only`. The T10 Codex RED review re-classed this as a non-deviation.
  - **Error taxonomy frozen in the record.** The `ServiceError` taxonomy and its per-revision JSON-RPC mapping were queued three separate times (T3.6, T5.3, T6.3) and returned as T10 finding F2 before being frozen in ADR 0026 at `2c75bea`, then corrected from 22 to the full 32-code inventory at `0323bb1` — where `internal-error`, minted at T10 GREEN for the unexpected-handler class, is explicitly ratified.
  - **The provider-dispatch-input authority seam.** T11's live smoke found composite `validate_repo`/`drift_check` dispatching every provider with empty input: four of eleven providers crashed and seven returned silently wrong answers, masked since T4 by echo-provider fixtures that tolerate any snapshot. The defect was routed out of the documentation task to its owning work — T15 published the seam at `control_plane/provider_inputs.py` (`75c9653`) and T14 moved construction worker-side with typed per-result failure isolation (`1abf8d9`), because measured authoritative inputs of 290 KB–4.8 MB exceed the frozen 256 KiB IPC request bound. **Address correction for any reader of the T15 freezes:** freezes J-A through J-M were written against the pre-rework address `command_resolution.provider_dispatch_input`; every such mention must be read as `provider_inputs.provider_dispatch_input`.
  - **Coverage map.** T10's acceptance clauses are discharged partly by suites that predate it: protocol conformance and capability truthfulness by the T5 transport contract, resource projection and refusal codes by T6, prompt absence and the `standard_read` fallback by T7, tool schemas and metadata compactness by T8 and T9, no-write reachability by TC-T9-003, and provider determinism by T4 — with T10 adding cross-process hash-seed determinism, the real-catalog byte budget, the stable wire-namespace contract, and the unexpected-handler probe. Named here rather than implied, so a later reader does not mistake T10's node count for the whole proof.

- **Risks closed / accepted:**
  - **Closed.** All six §11 risks are closed with evidence: R-001 by the T1 decision gate and ADR 0025; R-002 by the import-boundary suites (TC-T5-003, TC-T10-006); R-003 by the frozen client matrix, the `standard_read` fallback, and both client smokes; R-004 by the digest, containment, and content-exclusion suites; R-005 by the determinism suites with two fixed hash seeds; R-006 by the effect allowlist and the no-write registry and filesystem tests.
  - **Accepted — the no-write proof is bounded.** TC-T9-003 proves no write or apply surface is reachable, but the guarantee has stated limits rather than being absolute: the audit hook lives in the _server_ process, so worker-side writes are proven only through `tree_state` comparison; a `dir_fd` plus relative-name write would reach the hook without a resolvable root; and the sweep is bounded by inode facts rather than file content, excluding `.git`, `.venv`, `build`, `dist`, `node_modules`, and caches. These bounds were recorded deliberately at T9 and T14 rather than papered over, and the trust model behind them is settled: provider payloads are digest-verified trusted bytes.
  - **Accepted — `FR-019` is dormant, not unmet.** "Future writes reuse executor safety" has no v1 write tool to exercise. TC-T9-003 proves the negative only. Adding a write tool requires an approved ADR 0025 amendment, and that amendment is where the substantive obligations become testable. A green TC-T9-003 must not be mistaken for a positive write-safety proof.
  - **Accepted — measured residuals.** The provider result limit runs at 71% headroom (187,057 of 262,144 bytes) and in-worker planning costs about 1.26 s per verify provider and scales with repository size. Both are recorded as measurements rather than closed; the explicit-headroom assertion the orchestrator proposed was rejected at review as brittle capacity policy. `ProviderOperationResult.status` has a frozen field name but a deliberately unfrozen value set, and `transport.py`'s per-file coverage is a measurement artifact of absent subprocess instrumentation, not a test gap.

- **Deferred work filed:**
  - **Payload-declared provider input shapes** as the retirement path for `control_plane/provider_inputs.py` (owner resolution A, 2026-07-30; freeze J-N). Queued in `docs/TODO.md` as post-hold work.
  - **Three architectural candidates**, each declined during implementation with a stated reason rather than silently dropped, and filed here so they outlive the checklist: an fd-bound consumer root, provider isolation or rollback, and a supervising serializer boundary. All three were rejected because the task rules forbid adding a sandboxing layer and because the trust model treats provider payloads as digest-verified trusted bytes. Revisit only if that trust model changes.
  - **Fixture-reach gaps** that no current fixture can exercise: a conflict-bearing reconciliation preview with non-empty findings and provider notices, a `documentation-only` provider declaration, prompt-role resources, and a second catalog generation over the protocol. Each would need a new fixture, which is an owner decision rather than a task-local one.
  - **Client-advertised root ordering** as a fifth semantically unordered input for the determinism suite; reaching it needs the roots back-channel harness.
  - **The concrete-membership policy** for resource enumeration, left open by decision at T6.
  - **`AdapterConfiguration` consolidation** as a single object for conformance assertions — filed as low value; T10 asserted without it.
  - **Tooling: `scripts/plan.py` cannot see `IR-*` or `DR-*` requirement ids.** `REQ_ID_RE` matches only `(?:FR|NFR|REQ)-\d+`, so the validator's coverage check reports 43 of this plan's 61 requirement rows and is structurally blind to all nine `IR-*` and nine `DR-*` rows. A green `plan.py validate` is therefore not proof of their coverage; the manual Appendix B audit is, and it is recorded in `logs/T12.0.txt`. Widening the pattern is deferred tooling work.
  - **Owner-optional future freezes, reported not actioned.** No tool-to-CLI-command mapping is frozen (`FR-016`/`FR-020` are discharged against their normative acceptance sentence, and any stronger mapping must come from the owner or the spec). `ProviderOperationResult.status` values and absolute provider finding-path handling would each need a §5.5 sentence before they could be called frozen. The two spec-side reconciliations this register previously carried are **done, not pending**: the owner authorized them by bounded question on 2026-07-31, and `SPEC-MS01` revision 1.5 landed in this same close-out change set, correcting §9's `RepoInspectionSnapshot` sketch from `warnings` to the frozen `state` plus ordered bounded `findings` with explicitly-absent parsed slots, and replacing §10.3's `EC-005` wording with the implemented closed two-slot envelope. No requirement, scope, tool schema, or server behavior changed, and no code was touched for that revision.
  - **Run discipline for any whole-battery leg, and it is not optional.** The file-count-heavy MCP fixtures exhaust the 1,048,576-inode `/tmp` tmpfs, which killed an earlier overnight session mid-review. Export `TMPDIR` off `/tmp` and pass `--basetemp` under `/home` for every full battery, cleaning that basetemp between legs. Measured at this gate: `/tmp` inode use _fell_ from 56,480 to 16,672 while the basetemp grew past 270 MB, so the redirect demonstrably moves the churn. The cost is wall-clock — the ordinary leg ran 22:30 disk-backed against roughly 16:00 on tmpfs — and that trade is correct for a gate that must not die at 90%. A suite-level fix that makes this automatic is still pending.
  - **Tooling: the markdownlint runner can walk into `.git/`.** `.markdownlint-cli2.jsonc` sets `"globs": ["**/*.md"]` and relies on `.gitignore` for exclusions, but `.gitignore` cannot exclude `.git/` because git never considers its own directory. A stray Markdown file left inside `.git/` therefore fails a bare `npx markdownlint-cli2` even though it is not repository content and cannot exist in a clone or a CI checkout. This surfaced at the T12 gate and was resolved by relocating the artifact; the durable fix is an `ignores` entry, which is owned by the Markdown Tooling standard and locked until v5.12.0 ships.
  - **Appendix C** already carries the standing v1 exclusions: `FR-010` deterministic recommendations, controlled writes, remote transport, and fleet reporting.
