...
revision: 3
revises_revision: 2
revision_reason: 'claim the exact GitHub Workflow 1.2 producer reconciliation outputs discovered by T2 preview'
pause_reason: ''
...
| PV-T2-001 | REQ-003–REQ-005 | T2 | catalog/reconcile/selection, release-record, and exact Git-index mode-classification contract | catalog roles, resolver behavior, immutable 1.2 skill/reference bytes, reconciliation-owned state, changelog, complete executable equality, T1 binary modes/blobs | Preserve the one-path focused hygiene RED; add exactly 1.2 to the immutable-projection set; run focused/full hygiene, targeted activation tests, `uv run project-standards reconcile --apply`, compare both deployed prose outputs byte-for-byte with their immutable 1.2 sources, inspect rendered policy version/organization, run Tier 1 validators and scoped Markdown checks, inspect exact binary modes/blobs, and reserve the final integrated release-prep full gate. | 1.2 default, 1.1/1.0 retained/exact-selectable; deployed `SKILL.md` and `field-vocabulary.md` equal their immutable 1.2 sources; policy records 1.2 with configured organization unchanged; generated files converge; changelog retires MCP-first without implying support; and only the 1.2 immutable projection classification changes while all three binary modes/bytes remain exact. | Keep 1.1 default, remove retained row, leave either deployed prose output at 1.1, retain `package_version = "1.1"`, alter configured organization, hand-edit generated output, create ADR/MCP path, chmod/rewrite a binary, classify another path, alter anchor/predecessor policy, or weaken complete equality. | local checkout; Git/index/history and final gate direct local; no intermediate repository-wide gate | ephemeral |
