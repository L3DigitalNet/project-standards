...
revision: 2
revises_revision: 1
revision_reason: 'fold the exact GitHub Workflow 1.2 immutable projection executable classification into T2 activation proof'
pause_reason: ''
...
- **files:** [`catalogs/5.toml` (modify; owner T2), `.standards/catalog.toml` (generated; owner T2), `.standards/lock.toml` (generated; owner T2), `standards/github-workflow/{README.md,adopt.md,agent-summary.md}` (modify; owner T2), `standards/README.md` (modify if current-package inventory changes; owner T2), `standards/catalog.md` (generated; owner T2), `CHANGELOG.md` (modify; owner T2), `tests/package_contract/test_current_catalog_activation.py` and `tests/test_release_consistency.py` (modify only if their current-selection assertions require 1.2; owner T2), `tests/test_repository_hygiene.py` (modify only the exact 1.2 post-anchor immutable-projection executable classification and focused proof; owner T2)]
- **parallel_safe:** no
- **conflicts_with:** [T1]
- **supersedes:** []
- **superseded_by:** []
- **evidence:** [ephemeral]
- **recovery:** Preserve T1 checkpoint `144c1954` and the isolated one-path current-index RED. If reconcile, selection, or hygiene proof fails, retain the T1 candidate but revert only T2-owned activation/generated/classification changes; do not edit predecessor entries, chmod/rewrite a binary, broaden the executable set, weaken complete equality, or force a default. A focused RED naming another path or changed mode/blob is a new blocker, not authority to expand T2.
...
- **sub-tasks:**
  ...
  - **T2.2 RED** — extend selection/release-consistency assertions to expect 1.2 default with 1.1/1.0 retained and to require the MCP-retirement changelog statement; preserve the observed focused hygiene RED that names only the indexed 1.2 binary as an extra `100755` path; run the targeted activation tests and capture their expected old-default failure.
  - **T2.3 GREEN** — update the Catalog 5 role rows and mutable current pointers; add the changelog entry; run producer `reconcile --apply` so `.standards` and generator-owned catalog output are regenerated rather than hand-edited; add exactly the 1.2 binary beside 1.0/1.1 in `_POST_ANCHOR_IMMUTABLE_PROJECTION_EXECUTABLES`.
  - **T2.4 Verify GREEN** — run targeted package/catalog tests, the focused complete-inventory node, and the full repository-hygiene file; inspect exact selector/default resolution, generated-file ownership, complete executable equality, and identical 1.1/1.2 bytes plus unchanged 1.0/1.1/1.2 modes.
...
| PV-T2-001 | REQ-003–REQ-005 | T2 | catalog/reconcile/selection, release-record, and exact Git-index mode-classification contract | catalog roles, resolver behavior, reconciliation-owned state, changelog, complete executable equality, T1 binary modes/blobs | Preserve the one-path focused hygiene RED; add exactly 1.2 to the immutable-projection set; run focused/full hygiene, targeted activation tests, `uv run project-standards reconcile --apply`, Tier 1 validators, scoped Markdown checks, exact binary mode/blob inspection, and the final integrated release-prep full gate only. | 1.2 default, 1.1/1.0 retained/exact-selectable, generated files converge, changelog retires MCP-first without implying support, and only the 1.2 immutable projection classification changes while all three binary modes/bytes remain exact. | Keep 1.1 default, remove retained row, hand-edit generated output, create ADR/MCP path, chmod/rewrite a binary, classify another path, alter anchor/predecessor policy, or weaken complete equality. | local checkout; Git/index/history and final gate direct local; no intermediate repository-wide gate | ephemeral |
