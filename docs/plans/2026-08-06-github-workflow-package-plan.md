...
- **Completed:** all 24 non-superseded tasks are terminal — T1–T18 and T20–T25 — landed on `testing` across `51dbb3c8`…`093a63ba` (T11 close-out follows). T19 is the only superseded task (by T21, plan revision 2). Delivered: the family scaffold (T1), six operating-model references (T2), the skill plus its Codex companion (T3), this repository's first Go packages `cmd/gh-workflow` and `internal/ghworkflow` with nine subcommands (T4–T6), the reproducibly built committed binary `sha256:b0a8712d…` with gate wiring (T7), the complete payload with providers, blocks, and policy (T8), catalog advertisement plus the eight-layer integration cascade (T15–T18, T21–T23), self-hosting adoption in this repository under `L3DigitalNet` for both harnesses (T20), the plain-space baseline and digest-chain repin (T24–T25), 53 package/dogfood/guard tests with `scripts/verify.sh` green (T9), and release-readiness documentation with SPEC-GHW1 revisions 1.3–1.5 and EV-001 (T10). Five owner-directed `golang-skills` review passes produced the T12–T14 corrections.
- **Decisions / deviations harvested:** owner decisions — packaged field values use plain spaces rather than em-dashes to match the live organization schema (T24–T25); risks R1–R4 stay as written; every Go surface passes a `golang-skills` review before freeze. Execution decisions — R-001 was treated by pinning `GOEXPERIMENT=dwarf5` in the build invocation rather than accepting environment-recorded variance; self-adoption in this repository moved from a separate operational decision into plan scope when the MCP seam canary made the family's absence a live failure; T19 was superseded by T21 through plan revision 2 rather than reworked in place; acceptance clauses deferred forward twice along T16→T17→T18 and T20→T22→T23 as each correction exposed the next integration layer; issue field values are written with `POST`, not `PUT`; reopen-then-close is reclassified rather than reported as divergence.
- **Risks closed / accepted:** R-001 (build reproducibility) treated and proven — clean-worktree double builds are byte-identical under the pinned invocation, gated by `make go-check`. R-002 (Issue Fields GA surface) closed by verification against the live API; OQ-002 records REST-only access. R-003 (ledger output versus consumer markdown gates) proven — generated fixtures pass this repository's actual Prettier and markdownlint. R-004 (binary size) accepted per design D7; the reopen trigger stays recorded in the brief.
- **Deferred/discovered work filed:** nothing below is resolved here; each is surfaced for the owner and mirrored into `docs/handoff/`.
  1. OQ-001 — release-train placement of `github-workflow@1.0`; owner decision, post-v5.17.0. Remains **Open** in SPEC-GHW1 §18.3.
  2. Spec deferrals WH-001–WH-007 (Issue Forms, phase-3 enforcement, `migrate` provider, extra platforms, scheduled ledger refresh) unchanged; triggers in SPEC-GHW1 §2.3.
  3. `.github/workflows/go.yml` path filters omit `scripts/build-gh-workflow.sh`, the committed binary, and `payload.toml` (T7 finding) — a small CI fix.
  4. Twenty untracked `.agents/skills/go-*` directories materialized by the `golang-skills` plugin; owner chooses to gitignore, relocate, or track them.
  5. Go review backlog, all minor or nit, recorded in `.project-pipeline/2026-08-06-github-workflow-package/execution/logs/go-review-*.md`: the `Get*`/`Fetch*` naming split; the `cli.MarshalJSON` name; placement of `repo.go` and `atomic.go` in render; a golden `-update` flag; parse the base URL once; `Commands()` defer-unlock; the schema-test failure message; eager emit in `render/command.go`; the linker test's GOROOT fallback and a `t.Cleanup` nit; a 2^53 number-acceptance table case; the stale em-dash comment at `mutate/fields.go:44`. The Go `orgschema` testdata deliberately keeps em-dash values as a self-consistent parser fixture — recorded as a decision, not drift.
  6. Ordinary-lane timing flakes (`test_cooperative_shutdown…`, `test_slow_provider…`, plus three subprocess tests seen once under a full tmpfs) — a lane-design question, since the ordinary lane runs `-n 16` unserialized.
  7. Version-research residual: no single GitHub document states that issue-field-values is covered by `2022-11-28`; three converging indirect proofs stand in. An optional one-off live `POST` smoke is an owner call, and a `GET /meta` version probe is an optional feature idea.
  8. The agent-summary byte-limit test covers only `_REAL_MANIFESTS` and therefore not this family — a test-coverage gap.
  9. `standards/README.md` version-column staleness for two other families — pre-existing, untouched here.
  10. Issue #133 (process efficiency, Research, Inbox) was filed during execution with the delivered tool and is untriaged.
  11. Consumer adoption in repositories other than this one remains a per-repository owner decision.
- **Source/ADR/handoff reconciliation:** SPEC-GHW1 advanced to revisions 1.3–1.5 — §17.3 rows all Passing, OQ-002 Answered, OQ-001 deliberately still Open — and EV-001 is recorded at `docs/research/2026-08-06-github-workflow-live-run-evidence.md` with a 12/12 audit match. No ADR changed: ADR 0027 already governs the Go lane, and no new architectural decision was taken. `docs/handoff/architecture.md`, `specs-plans.md`, `state.md`, the 2026-08 session log, and the root `README.md` index are reconciled to the delivered package by this task.
- **Scratch teardown:** no irreplaceable evidence remains outside the repository — EV-001 and the golden fixtures are committed. The four `/tmp/ghw-probe*` scratch consumer checkouts used for the live and reconcile runs are removed, and the gate scratch under `/mnt/pytesttmp` is cleared (5.4 GiB and 584k inodes released). Execution worktrees and the `.project-pipeline/` records are retired by the orchestrator at integration; the review logs are read-only inputs to item 5 above until then.
...
| Consumer adoption in repositories other than this one | separate operational decision per repo; this repository adopted the package in T20 | owner selects the package in a consumer's `.standards/config.toml` |
| Execution-discovered items 3–10 in §13 (CI path filter, `go-*` skill directories, Go review backlog, lane flakes, version-research residual, agent-summary coverage, `standards/README.md` staleness, issue #133) | found during execution; none blocks the package | owner triages from the §13 close-out register |
