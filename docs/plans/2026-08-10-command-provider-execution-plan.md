...
revision: 3
revises_revision: 2
revision_reason: 'preserve InstalledPayload seams with a parent-prepared execution capsule and update the obsolete compatibility process fixture'
pause_reason: ''
...
| `repo:tests/package_compatibility/test_performance.py` | current-state evidence | Existing 5-second planning and 30-second deterministic-order budgets that remain authoritative unless post-capsule profiling returns a conflict to owner authority. | gate failure verified 2026-08-10 | §§7, 9–12; T2 |
...
- **sub-tasks:**
  ...
  - **T2.8 Verify Task** — rerun bootstrap; focused suites; five package checks; Markdown/Ruff; `rexec -- uv run basedpyright`; `git diff --check`; run the two existing performance proofs repeatedly in isolation and retain their current budgets; if either remains red, return measured/profiled evidence to owner authority without editing thresholds; otherwise run direct-local `scripts/verify.sh` and create the checkpoint.
...
| A-003 | The shared runner can preserve all existing Python providers by executing a parent-prepared integrity-checked capsule without retaining MCP's worker-side whole-manifest rediscovery. | If capsule parity or repeated isolated performance profiling disproves it, pause T2 and return exact evidence to owner authority rather than retain a duplicate, normalize qualified synthetic payloads, or raise bounds without authority. |
