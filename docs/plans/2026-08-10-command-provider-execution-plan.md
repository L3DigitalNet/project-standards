...
revision: 4
revises_revision: 3
revision_reason: 'adopt the measured 7.5-second real-catalog budget while retaining the 30-second deterministic-order budget'
pause_reason: ''
...
| --- | --- | --- | --- | --- |
| `request` | normative | Continue autonomously through v5.19.0 and adopt the recommended 7.5-second real-catalog budget after measured one-child startup cost proved the former 5-second contract infeasible; retain the independent 30-second deterministic-order budget. | 2026-08-10 | §§3, 6–7, 9–12; T2 |
...
| Python dispatch | `compile`/`exec` in caller. | Parent prepares an integrity-checked capsule; Python bytes execute in a bounded child through the shared ABI without whole-manifest rediscovery. | Qualified synthetic/monkeypatched `InstalledPayload` seams, input/resource immutability, all effects, schema checks, output notice, safe failure text, and generic routing. |
...
- **consumes:** [command-provider-execution-adr-v1, current Python provider input/resource/output contract, qualified InstalledPayload object seams, ADR 0025 bounded transport behavior, approved 7.5-second and 30-second performance budgets]
- **produces:** [bounded-provider-runner-v1, python-provider-execution-capsule-v1, bounded-python-provider-dispatch-v1]
- **preserves:** [all V2 Python operations/effects, schemas, typed results, output notice semantics, live-path integrity, qualified synthetic and monkeypatched InstalledPayload behavior, public MCP DTO/service results, request qualification, finding ordering, content-safe errors, V1 runner behavior, 7.5-second real-catalog budget, 30-second deterministic-order budget]
- **invariants:** [one owner for selector pump/signals/caps/frame/diagnostics; direct and MCP callers do not wrap the shared runner in another provider process; parent verifies exact capsule bytes and child does not rediscover the whole payload manifest; compatibility permits the one required runner child but rejects nested/provider-created children through focused controls; result fd is distinct from stdout/stderr; every exit path tears down/reaps the group; authoritative serial performance proofs retain their 7.5-second and 30-second ceilings]
...
- **sub-tasks:**
  ...
  - **T2.5 GREEN** — extract the transport and Python worker ABI into the control-plane boundary; have the parent build an execution capsule from already-qualified, integrity-checked provider/schema/resource bytes; execute that capsule in the child without whole-manifest rediscovery; make MCP call the same bounded dispatcher without nested spawn; narrow the compatibility-wide process denial to permit exactly this required child.
  - **T2.6 Verify GREEN** — run direct/MCP parity, every ADR 0025 process-path assertion, qualified synthetic/monkeypatched payload regressions, exact typed effects, real-provider compositions, the compatibility matrix, import-boundary assertions, and a process-count assertion that rejects double spawn.
  ...
  - **T2.8 Verify Task** — rerun bootstrap; focused suites; five package checks; Markdown/Ruff; `rexec -- uv run basedpyright`; `git diff --check`; run the two authoritative performance proofs repeatedly in serial isolation and require less than 7.5 seconds for real-catalog planning and less than 30 seconds for the 100-order proof; run direct-local `scripts/verify.sh` and create the checkpoint.
...
| `python-provider-execution-capsule-v1` | T2 | all existing V2 Python providers | Python bytes compile/execute in caller from an already-qualified `InstalledPayload`. | Parent verifies and serializes exact provider/schema/resource bytes, declaration metadata, validated input, and effect contract; child executes the declared symbol without whole-manifest rediscovery and writes one JSON object to the result fd. | Same runner limits; pre-spawn byte/digest/schema mismatch fails in parent; non-JSON/non-object/exception becomes bounded fixed failure. | Qualified synthetic/monkeypatched payload seams, existing input/output schemas, effects, output notices, resource closure, and V1 behavior remain exact. | current `invoke_provider`; T1 ADR; T2 gate correction |
...
| PV-T2-001 | REQ-003, REQ-004, REQ-008, REQ-012 | T2 | characterization, refactor parity, and subprocess regression | existing direct Python outputs and MCP DTOs; ADR 0025 clauses; independent child/fd/process inspection; repeated post-capsule timing/profile evidence | Run focused control-plane and MCP provider suites, import/source ownership checks, real packaged provider compositions, the two serial budget regressions, package checks, type/lint, and fast repository gate after bootstrap. | Direct and MCP Python calls share one bounded child path, spawn once, preserve exact effect/result objects, satisfy timeout/caps/truncation/fd/cancel/crash/descendant/cleanup/safe-failure assertions, complete real-catalog planning in less than 7.5 seconds, and complete the 100-order proof in less than 30 seconds. | Leave `_run_worker` in MCP; direct call runs in parent; MCP spawns twice; write result to stdout; overflow at N+1; stubborn child survives; expose a canary path/detail; restore the 5-second ceiling or exceed 30 seconds. | local and rexec synchronized Linux amd64; Git-dependent gate local; serial `-n 0` budget execution | ephemeral |
