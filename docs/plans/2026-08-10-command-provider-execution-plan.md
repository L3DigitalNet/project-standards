...
revision: 11
revises_revision: 10
revision_reason: 'authorize T5 authoritative composite-input test-double correction'
pause_reason: ''
...
| --- | --- | --- | --- | --- |
| `request` | normative | Continue autonomously through v5.19.0; retain the approved 7.5-second/30-second budgets; correct only the gate-discovered T2 exception-cause and inherited-file-descriptor audit parity gaps; close T3's discovered MCP command-routing/input-authority omission; and append the bounded T5 correction discovered by T4 without changing caps, architecture, or accepted product outcome. | 2026-08-11 | §§3–7, 9–12; T2–T5 |
...
- **Authoritative commands:** focused `tests/control_plane/test_providers.py`, shared-runner/command-provider tests including T5's real plan-bound runner-selection regression, `tests/mcp_services/test_providers.py`, payload execution/schema tests, and command-provider end-to-end tests; the five package validators; generated schema/projection checks; Git-tracked Prettier/markdownlint; Ruff, BasedPyright, and coverage; `make go-check`; `git diff --check`; fast `scripts/verify.sh` at intermediate engine checkpoints and `scripts/verify.sh --full` after resumed T4.
...
- **Evidence:** command output is ephemeral. Retain the revision-4 failed-gate receipt in execution notes: statics green 4:10; ordinary 17 failed/4,858 passed 13:41; compatibility green 34:04; performance green 0:39; total 34:45. Retain T3's nine-path/299-focused-test green receipt and terminal checkpoint. Retain T4's correct real-route receipt: reproducible binary digest `6ab733ac5d7aa9c8949f0d0e6711fe4d262bbd5fb5bc72cb943beef90c7cda3f`, payload aggregate `6ca00708e0dea8c36a3370c72aa6c1b7266b456e4966dc5d80b889c69d58c296`, six fixture tests green, direct command one-child green, then `validate-fixture` success followed by `verify-fixture` `control-plane-unavailable` before spawn because planning attempted command `render-fixture` through the Python-only runner. The committed ADR, tests, fixture source/binary, build script, and checkpoint trailers retain the durable contract; no separate evidence file is required.
...
7. Stop at `command-provider-core-checkpoint-v1`. A separately authored Agent Handoff migration plan may consume T4 only after verifying EG-156; this plan cannot begin that cut.
...
- Recovery proof: T5 starts from the integrated T3 checkpoint and leaves the blocked T4 worktree untouched. Its focused proof counts the planning render and target verify separately and rejects target execution during construction; resumed T4 repeats the exact failed route before broader end-to-end flows and verifies a clean temporary namespace and unchanged production catalogs/payloads.
...
- Bridge 3.5.0 validates this active master and every T1–T5 checkpoint/trailer identity; EG-ADR was delivered before #162. EG-156 remains an explicit deferred-successor prerequisite, not a core-plan completion condition.
...
- **Completed:** record T1–T5 commits, including T5's permanent correction provenance, command-provider-core-checkpoint-v1, EG-ADR delivery to #162, and the unconsumed EG-156 requirement in the deferred-migration handoff.
...
- **Risks closed / accepted:** close R-001 through R-012 from proof or file a bounded follow-up owned outside core acceptance.
