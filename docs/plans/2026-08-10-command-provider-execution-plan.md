...
revision: 10
revises_revision: 9
revision_reason: 'append a bounded correction for plan-bound command input authority before T4 resumes'
pause_reason: ''
...
- **files:** [`src/project_standards/control_plane/provider_worker.py` (modify explicit planner-runner seam only; owner T2), `src/project_standards/mcp_services/providers.py` (modify `_dispatch_command` call to supply the parent kind-aware runner only; owner T2), `tests/control_plane/test_command_providers.py` (modify focused plan-bound runner-selection regressions only; owner T3)]
- **parallel_safe:** no
...
- **discovered_from:** [T4]
- **evidence:** [ephemeral]
- **recovery:** begin from the integrated T3 checkpoint, not the dirty T4 worker. If the correct-reason RED does not reproduce Python-only planning-runner refusal for a command render, stop and reconcile the test with T4's captured route. If the explicit runner seam changes Python construction, executes target VERIFY early, wraps a planning command in a Python child, requires a package-ID branch, or reconstructs snapshots outside provider_dispatch_input, block T5 and preserve T4 unchanged. After T5 checkpoints, resume the existing T4 worktree and rerun its failed real route before any full gate.
...
- **sub-tasks:**
  - **T5.1 CHARACTERIZE** — preserve the T4 receipt and assert the exact failing chain: `verify-fixture` has no family input; plan construction calls command `render-fixture` for `.standards/command-provider-fixture.txt`; the hard-coded Python in-child runner rejects it before target verify spawn. Record the render declaration metadata and snapshot inputs so the regression cannot pass through a hollow provider.
  - **T5.2 RED** — in the existing T3-owned control-plane command test surface, drive plan-bound authoritative input with a qualified mixed command payload. Require real render content to affect the completed plan, capture the input projected for verify, count provider IDs/operations and process boundaries, and retain Python, family-first, and generic-None controls. The correct RED is the current `provider kind is not executable in the Python worker` refusal, not a missing fixture, import, schema, or platform failure.
  - **T5.3 Verify RED** — prove `render-fixture/render` is the planning invocation that fails, `verify-fixture/verify` has not run, and substituting fixed render bytes or empty snapshots would fail the oracle.
  ...
  - **T5.5 Verify GREEN** — prove the required render command executes once through one direct bounded child, its output determines the completed plan, `_verification_input` produces the executor-equivalent snapshots, the target verify is absent during construction and executes once afterward through one direct bounded child, and Python planning/execution remains in the existing worker. Re-run family-first, generic-None, failure-isolation, caps, no-package-ID, and no-nested-worker controls.
  - **T5.6 Verify Task** — rerun bootstrap after source edits; run the focused command/provider-worker/MCP routing regressions through `rexec -- uv run pytest ...`; run five package checks and scoped Markdown locally where Git corpus selection is required; run Ruff and `rexec -- uv run basedpyright`; run `git diff --check`; run direct-local `scripts/verify.sh`; validate the plan/checkpoint; create the correction checkpoint. Do not run T4's final full gate here.
