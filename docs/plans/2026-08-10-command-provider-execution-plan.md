...
revision: 9
revises_revision: 8
revision_reason: 'bind MCP command routing to the sole control-plane runner and preserve composite input authority'
pause_reason: ''
...
| --- | --- | --- | --- | --- |
| `request` | normative | Continue autonomously through v5.19.0; retain the approved 7.5-second/30-second budgets; correct only the gate-discovered T2 exception-cause and inherited-file-descriptor audit parity gaps; and close T3's discovered MCP command-routing/input-authority omission without changing caps, architecture, or accepted product outcome. | 2026-08-11 | §§3–7, 9–12; T2–T3 |
...
| `repo:src/project_standards/control_plane/provider_worker.py::authoritative_provider_input` | current-state evidence | Existing family-first then plan-bound composite input-authority helper, including the explicit generic empty-input outcome when no seam authority exists. | `78fb946b` | §§4–7, 9–12; T3 |
| `repo:src/project_standards/mcp_services/providers.py::_dispatch` | current-state evidence | Parent-side MCP dispatch currently always launches the Python worker, while `_composite_dispatch` supplies the seam directive and single-provider dispatch supplies caller input. | `cf90c88c` | §§4–7, 9–12; T3 |
...
T2 is now terminal at `78fb946b`, and T3 has a preserved nine-path direct-runtime candidate with 299 focused tests green. The remaining T3 blocker is the MCP parent: `_dispatch` still unconditionally starts `python_worker_argv`. Sending a command declaration through that worker and then through the command path would create two children, contrary to ADR 0030. The current composite route also depends on that Python worker to interpret `_SEAM_DIRECTIVE_INPUT` plus `SEAM_AUTHORITY` and call `authoritative_provider_input`; a parent-side command route therefore cannot forward the empty directive as though it were the built input. Single-provider MCP dispatch already supplies caller input and must preserve it unchanged.
...
| MCP command dispatch | `_dispatch` always starts the Python worker; composite input is built only inside that worker. | Parent branches by provider kind: Python keeps its existing route; command invokes the control-plane boundary directly after resolving composite seam input when applicable. | Exactly one child; single-provider caller input remains unchanged; composite family/plan-bound/generic authority and per-result failures remain unchanged; no package-ID routing. |
...
| MCP provider dispatcher | `_dispatch` always starts `python_worker_argv`; `_composite_dispatch` relies on that worker to interpret the seam directive. | Branch on provider kind in the parent: retain the Python path; send command declarations directly to `invoke_provider` after preserving caller input or building authoritative composite input. | `mcp_services/providers.py::_dispatch`; `control_plane/provider_worker.py::authoritative_provider_input` | T3 contributor, shared owner T2 |
...
| Architecture / dependency direction | yes | `control_plane` owns the process runner; MCP consumes it exactly once, including command dispatch, and no longer owns or creates a nested dispatch boundary. | PV-T1-001, PV-T2-001, PV-T3-001 | T1–T3 |
...
- **Evidence:** command output is ephemeral. Retain the revision-4 failed-gate receipt in execution notes: statics green 4:10; ordinary 17 failed/4,858 passed 13:41; compatibility green 34:04; performance green 0:39; total 34:45. Also retain T3's preserved nine-path/299-focused-test green candidate receipt and the recorded MCP routing blocker until the parent-side branch is proved. The committed ADR, tests, fixture source/binary, build script, and checkpoint trailers retain the durable contract; no separate evidence file is required.
...
- Command process tests prove direct and MCP input/resource byte equivalence, single-provider caller-input preservation, composite family/plan-bound/generic input authority, exactly one MCP command child, output schema/effect validation, result-fd isolation, exact/N+1 caps, timeout/cancellation/crash/nonzero/descendant cleanup, unsupported-platform pre-spawn refusal, closed environment, installed-path nonexecution, on-disk digest/mode verification, and cleanup after every path.
...
- **Risks closed / accepted:** close R-001 through R-010 from proof or file a bounded follow-up owned outside core acceptance.
...
| `mcp-command-dispatch-v1` | T3 | MCP single-provider and composite routes | `_dispatch` always enters `python_worker_argv`; the worker interprets composite seam authority. | Parent branches on provider kind. Python retains the existing route. Command invokes `control_plane.providers.invoke_provider` directly; caller input passes unchanged, while composite input is first built through `authoritative_provider_input` against the qualified selection and remains `{}` only when that helper returns `None`. | Authority/selection failures preserve bounded service-error mapping and composite per-result isolation; command process/transport failures retain the shared runner taxonomy and caps. | Exactly one command child; no nested Python worker, package-ID routing, caller-input rebuilding, or empty-directive substitution for authority-built input. | ADR 0030 shared-runner decision; `_dispatch`; `authoritative_provider_input` |
