...
Migration preview exits `0` when the emitted plan is applicable with no error findings — ready to apply — and `1` when the plan is blocked; the JSON `ok` and `applicable` fields carry the same readiness signal as the exit code. Supplying `--apply` to a blocked plan emits the same plan and findings without writing and exits `1`; it is not a partial apply. Human findings include their severity, path, semantic identity, and remediation hint. JSON preview is the stable machine-readable form. Neither form publishes proposed file content; inspect the reviewed post-apply diff before committing.
...
Exit status: `0` plain init succeeded, migration already complete, migration apply succeeded, or the preview plan is applicable and error-free · `1` migration preview is blocked by error findings, apply was refused, or apply failed · `2` invalid arguments, unsafe paths, unavailable catalog, unsupported authority state, or inconsistent existing state.
...
- **Remote execution workers.** A worker that receives repository content but not the workstation's uv tool state has no `project-standards` on its `PATH`. Run the CLI there from the same immutable tag the repository adopted, which requires only `uv` on the worker and network reach to GitHub:

  ```bash
  uvx --from "git+https://github.com/L3DigitalNet/project-standards@vX.Y.Z" project-standards <command>
  ```

  Replace `vX.Y.Z` with the exact release tag recorded for the repository (the `<release-tag>` of [`docs/adoption-prompt-full.md`](adoption-prompt-full.md)); a bare `uvx project-standards` resolves to a cached or latest build and silently decouples the check from that release, and `@main` is never a supported selector. `validate`, `reconcile --check --json`, `agent-handoff validate`, and `agent-handoff drift-check` are read-only and may run on such a worker. Everything that mutates the checkout — `init`, `fix`, `reconcile --apply`, `reconcile --repair-state --apply`, and `reconcile --restore-managed <path> --apply` — stays local: apply authority belongs to the authoritative working tree, and a worker's tree is disposable. Distinguish the failure shapes: exit `127` with `exec: project-standards: not found` means the CLI was absent and nothing was checked, while exit `1` or `2` from the CLI itself is a standards result to read. Without network reach to GitHub the `uvx` form cannot resolve, and the checks stay local.
