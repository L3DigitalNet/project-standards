...
- An unreleased 5.3.0 candidate on `testing` (`e69831f`) corrects issues #12/#13 by extending consumer-owned relinquishment to every package CI target through immutable payloads Python Tooling 1.4, Markdown Tooling 1.5, and CLI Documentation 1.3 (in-tree catalog defaults advance accordingly). The 17-scenario adoption-mechanics audit over copies of every real v4 consumer (`docs/reviews/2026-07-20-adoption-mechanics-audit.md`) confirmed 3 blockers and 8 majors with two-model verification; the owner held the release so a Codex session can implement the fixes and roll them into 5.3.0.
