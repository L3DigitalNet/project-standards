...
- All 20 open issues except #168 are triaged as of 2026-08-09, each with a verified comment. Fourteen carried a materially wrong or stale premise, so several are now smaller or differently shaped than filed: #140's retirement defect is obsolete for V5-native installs (the live defect is `agent-handoff` 1.11 `adopt.md`), #164's acceptance criteria are unsatisfiable because the sweep reports rather than rewrites and keys on the release version, #166's performance-lane remedy already shipped, #158's bound is ADR 0025 and is not frozen, #153's prefix option is blocked by scope-as-lock-key, #162 touches 43 references rather than five, and #163's corpus already passes with zero findings.
- Owner decisions are owed on #142, #153, #157, #158, #159, #160, and #161 before those can be specified. #129 (`adr-conformance` foundation) stays open and deferred, with both prerequisites closed.
- Release shape for the backlog is undecided: `ROADMAP.md` scopes 5.19.0 to `project-toolbox` alone, and #168's title says v5.20.0 while its body says v5.19.0.
...
- The open-issue program has T16, T19, and T36 terminal (checkpoints `50d0c364`, `229a4bc1`, `e13e1a66`; EV-008); the feature phase T24–T29 remains, with T24 ready. The `.project-pipeline` executor state still records those three as `not-started` and needs reconciling through the execute-plan skill.
