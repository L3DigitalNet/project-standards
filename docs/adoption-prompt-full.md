...
- [x] `github-workflow` — installs the GitHub work-discipline skill and the static `linux/amd64` `gh-workflow` binary for a repository owned by a GitHub organization whose Issue Fields schema it audits against.
- [x] `project-toolbox` — installs two managed workflow checklists (a repository-housekeeping sweep and a drift-detection sweep) plus a routing skill; it has no options, ships no executable code, and requires no other package.
