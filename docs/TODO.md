...
- [ ] Preserve optimized coverage settings through the atomic Python Tooling migration.
  ...
  Release replay proved that Python Tooling 1.1 cannot represent the required parallel/subprocess coverage settings and that the optimized consumer-owned workflow needs a generic, target-bound ownership-relinquishment path. The reviewed 11-task TDD plan is in progress on an isolated feature branch. Tasks 1–6 are complete: the generic ownership engine, Python coverage options/rendering, and paired offline subprocess-capture oracle pass independent review and verification. The executable oracle also exposed a release blocker in the pre-existing checker-selection contract: the payload materializes both `[tool.basedpyright]` and `[tool.pyright]`, while BasedPyright 1.39.6 rejects that configuration before analysis. The conditional nested-option-pointer design is owner-approved after a five-round contract audit converged, and its six-task TDD implementation plan is committed (`docs/superpowers/plans/2026-07-12-python-tooling-checker-table-materialization.md`). Execute that plan before coverage Task 9; amend and freshly audit coverage Tasks 9 and 11 (Pyright carry-through, guarded dev-group pre-alignment, frozen predecessor overlay); then complete Tasks 7–8, refresh release replay/evidence, and perform the atomic root migration.
