...
- [ ] Complete the Consumer Standards Control Plane (`SPEC-CP01`) follow-on for v5.
  ...
  The 18-task follow-on plan at `docs/superpowers/plans/2026-07-11-consumer-standards-control-plane-package-migration-release.md` passed the scratch-path convergence audit and is owner-approved. Execution is underway on `feature/v5-package-migration`; Tasks 1-14 provide typed migration/apply and refresh/recovery, all nine reconstructed payloads, the real catalog 5 source, root V2 family indexes, V2-backed graph/catalog generation, complete-default adoption activation, and byte-identical source/wheel projections. Task 15 unifies public command configuration and provider routing next.
