...
Project Standards 5.4.0 requires Python 3.14 or newer. Install the exact release from its immutable Git tag, then verify the installed command before changing a repository:

```bash
uv tool install "git+https://github.com/L3DigitalNet/project-standards@v5.4.0"
project-standards --version
```

The version command must report `project-standards 5.4.0`. V5 consumers use one catalog/config/lock plane. Initialization is neutral and enables no package:
...
Reference reusable workflows by **major tag** (`@v5`), never `@main`. For an immutable pin, use a full version (`@v5.4.0`) or a commit SHA. [`UPGRADING.md`](UPGRADING.md) is the v4-to-v5 migration runbook.
...
repos:
  - repo: https://github.com/L3DigitalNet/project-standards
    rev: v5.4.0 # pre-commit requires an immutable rev — use a full release tag, not a moving major
    hooks:
