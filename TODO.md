...
- [ ] **NEXT SESSION — plan/spec the `project-spec` tooling.** The README §5 defines the target capability set (`validate`, `lint`, `extract`, `next`, `new`, `upgrade` v1-core + `status` planned + the agent review contract), each traced to guarantees G1–G8; that section is the requirements outline. Brainstorm → spec → plan the tooling and its distribution (a `project-standards` CLI subcommand group vs a reusable CI workflow). Building this **unblocks the README's §6 Adoption**, which is deliberately left as a placeholder until then (adoption == "run the tool," which does not exist yet).
