...
- [ ] **Project-spec follow-on:** write README §6 Adoption and decide/register the `project-spec` standard. Spec #1 (read-only `validate|lint|extract|next`), Spec #2 (`spec new` scaffold, `8d48c22`), and Spec #3 (`spec upgrade` tier promotion, implemented 2026-07-05 on `testing`) are all implemented — **the v1-core project-spec tool surface (`validate|lint|extract|next|new|upgrade`) is now complete.** When `project-spec` is registered/released, add a CHANGELOG line covering the full command surface, including `upgrade`.
...
- [x] **🟢 Pre-existing `Validate Specs` CI gap — FIXED 2026-07-05** (found during Spec #3 codex plan-review CR-002). The `Validate Specs` workflow would fail at any `main` merge because `.project-standards.yml` has no `spec:` block and `spec validate` exit-2s on its "never a vacuous green run" guard. Fixed: `.github/workflows/validate-specs.yml` now gates the this-repo validate/lint steps on a top-level `spec:` key existing (skip-with-notice otherwise), so CI is green until `project-spec` is registered and auto-enables when a `spec:` block appears. **Follow-up at registration:** add the `spec:` block pointing at the repo's real specs so the gated steps activate.
