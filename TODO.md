...
- [ ] **🟡 Pre-existing `Validate Specs` CI gap (found during Spec #3 codex plan-review, 2026-07-05).** `.github/workflows/validate-specs.yml` runs `project-standards spec validate --config .project-standards.yml` on this repo (`L3DigitalNet/project-standards`) for any push touching `src/**`/`*.md`/config, but `.project-standards.yml` has **no `spec:` block**, so the command exits 2 (`no spec: config block and no paths given` — the "never a vacuous green run" guard). Pre-existing since Spec #1 added the workflow; the repo authors no `project-spec`-format specs yet (`docs/superpowers/specs/**` are design docs, not project-spec documents). **Resolve alongside registering `project-spec`** (when real specs + a `spec:` block exist) — or make the workflow's "this repo" step tolerate an empty target until then. Not a blocker for the `spec upgrade` feature.
