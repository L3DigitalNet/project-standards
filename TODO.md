...
- [ ] **Codex to implement project-spec tooling Spec #1** from `docs/superpowers/plans/2026-07-04-project-spec-tooling-spec1.md` (12 TDD tasks) in a parallel session — spec + plan are codex-converged (design `403ea7c`, plan `9119039`, review fixes `47f5e04`). Scope = a `src/project_standards/specs/` subpackage: registry core + read-only `spec validate|lint|extract|next`; `new`/`upgrade` are Spec #2. The plan also retires `check_specs.py` into the pytest gate and reconciles README §5/§8/§9 + `tooling-notes.md` §8/§11. **On completion this unblocks README §6 Adoption** (write it, then register the standard) — that write-up remains the follow-on task.
