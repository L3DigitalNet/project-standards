...
- [ ] **Residual out-of-scope gaps from the 2026-06-12 verification** (adjacent files, outside the review target): `format-frontmatter` still silently defaults on a typo'd `--config` (and it _writes_ files) and can traceback on non-UTF-8 input (F1/F4 class; `format_frontmatter.py:597,651`); `project-standards validate --help` still says "Additional glob pattern" (`cli.py:188`, F42 class).
