...
- [ ] **MCP enablement program (specs ingested 2026-07-07; NOT started — do not begin implementation without explicit go-ahead).** Ordered, gated work; MCP server code must not start until **SPEC-MT01**'s readiness gate passes (**SPEC-RD01** Step 07). Specs + paths in `docs/handoff/specs-plans.md`. Before any SPEC-MS01 work, **recheck version-sensitive MCP references** — the MCP spec revision and the official Python SDK stable/pre-release line (REF-OQ-003 / SPEC-MS01 MS-0); do not rely on the July-2026 assumptions frozen in the reference pack.
  - **ADR backlog (→ `docs/adr/`, `adr-NNNN-...`).** The **13 SPEC-MT01 ADRs below (adr-0001…0013) are authored** as proposed (2026-07-07); the SPEC-MS01 _server_ ADRs at the end remain for the server phase. Each spec's §8.3 enumerates its full ADR set; the minimum required across the program is:
...
- [x] **Step 02 — Standard Bundle Authoring Standard** (the meta-standard) — done 2026-07-07 (`fbaf49d`…`83fe5fd`, executing-plans, inline): bundle authored under `standards/standard-bundle-authoring/` — README contract satisfying FR-001…FR-014 (bundle anatomy, annotated `standard.toml`, five-mode adoption enum, authority tuple + conflict rule, relationship taxonomy, dotted config-namespace ownership, providers, resources, manifest safety, `adopt.toml` linkage, lifecycle, conformance checklist) + its own worked `standard.toml` (`adoption = "none"`) + blank template. Repo-facing maps reconciled (`adopt.md` anatomy + standards index/count; fixed stale five→six drift; `CLAUDE.md` trimmed under budget). No machine-layer change (CR-004: registry/bundles/`spec.include` untouched). SPEC-BA01 §17.1 DoD complete; **owner sign-off — `status: approved`, rev 0.7** (Step 02 closed). Full gate green: frontmatter/id 39, spec validate/lint, markdownlint 0, prettier, ruff/basedpyright, 868 tests.
- [x] **Step 03 — `standard.toml` manifest** schema/model + valid/invalid fixtures. — done 2026-07-08 (commits 89464ea..44c290e, subagent-driven, 961 tests; pydantic runtime dep added).
- [x] **Step 04 — Standards graph validator** (authority / capability / resource / relationship, including hidden-dependency rejection) + CLI. — done 2026-07-09 (`8d23a0c`). Plan: `docs/superpowers/plans/2026-07-09-standards-graph-validator.md`.
- [x] **Step 05 — Retrofit** every existing standard with a manifest, authorities, and resources. — done 2026-07-09 (`82d1978`, review fix `ce11a54`): added `standard.toml` manifests for all eight standard bundles (including `python-coding` as `reference-only`) plus current-repo graph enforcement under `--require-all-manifests`; subagent review follow-up tightened provider operations to the generic enum and filled missing template resources.
- [ ] **Step 06 — Dogfood fixtures** + generated standards index + relationship catalog; reconcile the `standard.toml` ↔ packaged `adopt.toml` artifact-manifest linkage deferred from Step 05 because current resources are bundle-local while artifact manifests live under `src/project_standards/bundles/`.
